...
- The Buttons are contained in or owned by an element with `role` `radiogroup`.
- Each Button element has `role` `radio`.
...
- The Buttons are contained in or owned by an element with `role` `toolbar`. This can be overridden, with `role` `group` for example, if the ButtonGroup is contained within a parent with `role` `toolbar`.
...
With no selection using the `readOnly` prop, ButtonGroup implements the [Toolbar design pattern per WAI-ARIA 1.1](https://www.w3.org/TR/wai-aria-practices/#toolbar).
...
- The Buttons are contained in or owned by an element with `role` `toolbar`. This can be overridden, with `role` `group` for example, if the ButtonGroup is contained within a parent with `role` `toolbar`.
- Each Button element retains implicit role of `button`.
- Each Button is labelled by its content, has a visible label referenced by `aria-labelledby`, or has a label specified with `aria-label`.
- The `toolbar` element has a visible label referenced by `aria-labelledby` or has a label specified with `aria-label`.
