...
As space becomes limited, or as a user navigates deeper within a hierarchical structure, nested breadcrumbs provide a solution by displaying as many bottom level navigation items as possible while nesting all higher level navigation items within a menu.
In some cases, only the current page may be displayed, and in rare cases the entire breadcrumb may become nested within an icon only dropdown menu.
In certain cases, root context needs to be displayed, which is also an option for Breadcrumbs truncation behavior.
Breadcrumbs can also allow for a drag-and-drop interaction when they are used in a file management context.

Breadcrumbs in Spectrum have a multi-line or `title` variant, where the last item renders as a `Header`.
...
- The link to the current page has [`aria-current`](https://www.w3.org/TR/wai-aria-1.1/#aria-current) set to `page`. If the element representing the current page is not a link, [`aria-current`](https://www.w3.org/TR/wai-aria-1.1/#aria-current) is optional.
...
1. File items could be selected to be moved from a TableView or GridView.
...
2. File items could be selected to be moved from a TableView or GridView.
  ...
  - User could then press `Shift + F10`, or `MenuKey` on Windows, or perhaps `Enter` to bring up a context menu with options to either Copy or Move selected items to the Breadcrumb item location.
