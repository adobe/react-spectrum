...
- `PageDown` (Optional): When the list scrolls, moves focus to the item one page after the item with focus, or if the list does not scroll, the first item. Optionally, in a single-select listbox, selection may also move with focus.
