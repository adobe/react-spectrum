...
- If you cannot find the component css module name in @adobe/spectrum-css-temp/components, feel free to reach out to a team member on the git issue.
```

Upon answering all the prompts, the appropriate package(s) should be generated and ready for modification. Note: With regards to the generated docs files, please feel free to remove them if you won't be contributing docs. If you are contributing docs for a new component/hook, please submit them in a separate PR so that we can decide when to deploy them to our docs site.
