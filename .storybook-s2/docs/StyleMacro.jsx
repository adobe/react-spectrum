  ...
  return (
    <div className={'sb-unstyled ' + style({marginX: 'auto'})}>
      <header
        ...
        <h1 className={style({font: 'heading-2xl'})}>Style Macro</h1>
      </header>
      <main className={style({marginX: 48})}>
        <P>
          The React Spectrum <Code>style</Code>{' '}
          <Link href="https://parceljs.org/features/macros/" target="_blank">
            macro
          </Link>{' '}
          generates atomic CSS at build time, which can be applied to any DOM element or Spectrum
          component. Style properties use Spectrum tokens such as colors, spacing, sizing, and
          typography, helping you work more quickly with TypeScript autocomplete, reduce the design
          choices you need to make, and improve consistency between Adobe applications.
        </P>
        <Pre>
...
<div className={style({backgroundColor: 'red-400', color: 'white'})}>
  {/* ... */}
        ...
        <P>
          <Strong>Atomic CSS</Strong> scales as your application grows because it outputs a separate
          rule for each CSS property you write, ensuring there is no duplication across your whole
          app. The above example generates two CSS rules:
        </P>
        <Pre>
          {highlight(
...
.ac { color: #fff }`,
        ...
        <P>
          The <Code>style</Code> macro <Strong>colocates</Strong> your styles with your component
          code, rather than in separate CSS files. Colocation enables you to:
        </P>
        <ul className="sb-unstyled">
          <li className={style({font: 'body-lg', marginTop: 0, marginBottom: 8})}>
            <Strong>Develop more efficiently</Strong> – No need to switch between multiple files
            when working on a component, or spend time writing CSS selectors.
          </li>
          <li className={style({font: 'body-lg', marginY: 0})}>
            <Strong>Refactor with confidence</Strong> – Changing the styles in a component is
            guaranteed to never unintentionally affect any other parts of your application. When you
            delete a component, the corresponding styles are also removed, reducing technical debt.
          </li>
        </ul>
        <H2>Values</H2>
        <P>
          The <Code>style</Code> macro supports a constrained set of values for each CSS property,
          which conform to the Spectrum design system. For example, the <Code>backgroundColor</Code>{' '}
          property supports Spectrum colors, and does not allow arbitrary hex or rgb values by
          default. This helps make it easier to build consistent UIs that are maintainable over
          time.
        </P>
        <H3>Colors</H3>
        <P>
        ...
        <H3>Spacing</H3>
        <P>
          Spacing properties such as <Code>margin</Code> and <Code>padding</Code> support a limited
          set of values. The API is represented in pixels, however, only values conforming to a 4px
          grid are allowed. This helps ensure that spacing and sizing are visually consistent.
          Spacing values are automatically converted to rems, which scale according to the user’s
          font size preference.
        </P>
        <P>In addition to numeric values, the following spacing options are available:</P>
        <ul className="sb-unstyled">
          <li className={style({font: 'body-lg', marginTop: 0, marginBottom: 8})}>
            <Code>text-to-control</Code> – The default horizontal spacing between text and a UI
            control, for example between a label and input. This value automatically adjusts based
            on the font size.
          </li>
          <li className={style({font: 'body-lg', marginTop: 0, marginBottom: 8})}>
            <Code>text-to-visual</Code> – The default horizontal spacing between text and a visual
            element, such as an icon. This value automatically adjusts based on the font size.
          </li>
          <li className={style({font: 'body-lg', marginTop: 0, marginBottom: 8})}>
            <Code>edge-to-text</Code> – The default horizontal spacing between the edge of a UI
            control and text within it. This value is calculated relative to the height of the
            control.
          </li>
          <li className={style({font: 'body-lg', marginTop: 0, marginBottom: 8})}>
            <Code>pill</Code> – The default horizontal spacing between the edge of a pill-shaped UI
            control (e.g. a fully rounded button) and text within it. This value is calculated
            relative to the height of the control.
          </li>
        </ul>
        <H3>Sizing</H3>
        <P>
          Sizing properties such as <Code>width</Code> and <Code>height</Code> accept arbitrary
          pixel values. Internally, sizes are converted to rems, which scale according to the user’s
          font size preference. Additionally, size values are multiplied by 1.25x on touch screen
          devices to help increase the size of hit targets.
        </P>
        <H3>Typography</H3>
        <P>
          Spectrum 2 does not include specific components for typography. Instead, you can use the
          style macro to apply Spectrum typography to any HTML element or component.
        </P>
        <P>
          The <Code>font</Code> shorthand applies default values for the <Code>fontFamily</Code>,{' '}
          <Code>fontSize</Code>, <Code>fontWeight</Code>, <Code>lineHeight</Code>, and{' '}
          <Code>color</Code> properties, following Spectrum design pairings. These individual
          properties can also be set to override the default set by the shorthand.
        </P>
        <Pre>
          {highlight(`<main>
...
`)}
        ...
        <P>
          The <Code>style</Code> macro also supports conditional styles, such as media queries, UI
  ...
  className={style({
    padding: {
      ...
      lg: 32
        ...
        <P>
          Conditions are mutually exclusive, following object property order. The <Code>style</Code>{' '}
          macro uses{' '}
          <Link
        ...
        <P>
          The <Code>style</Code> macro also supports conditions that are resolved in JavaScript at
          runtime, such as variant props and UI states. When a runtime condition is detected, the{' '}
          <Code>style</Code> macro returns a function that can be called at runtime to determine
          which styles to apply.
        </P>
        <P>
          Runtime conditions can be named however you like, and values are defined as an object.
          This example changes the background color depending on a <Code>variant</Code> prop:
        </P>
        <Pre>
...
function MyComponent({variant}) {
  return <div className={styles({variant})} />
        ...
        <P>
          Boolean conditions starting with <Code>is</Code> do not need to be nested in an object:
        </P>
        <Pre>
          {highlight(`let styles = style({
  backgroundColor: {
...
<div className={styles({isSelected: true})} />`)}
        ...
        <P>
          ...
          <Link
            href="https://react-spectrum.adobe.com/react-aria/styling.html#render-props"
            target="_blank">
            render props
          </Link>{' '}
...
<div className={styles({isSelected, isEmphasized, isDisabled})} />`)}
        ...
        <P>
          The above example has three runtime conditions (<Code>isSelected</Code>,{' '}
          <Code>isEmphasized</Code>, and <Code>isDisabled</Code>), and uses the{' '}
          <Code>forcedColors</Code> condition to apply styles for{' '}
          <Link
          ...
          object, with the <Code>isSelected</Code> + <Code>isDisabled</Code> +{' '}
          <Code>forcedColors</Code> state having the highest priority.
        </P>
        <H2>Reusing styles</H2>
        <P>
          Styles can be reused by extracting common properties into objects, and spreading them into{' '}
          <Code>style</Code> calls. These must either be constants (declared with <Code>const</Code>
          ) in the same file, or imported from another file as a macro (
          <Code>{"with {type: 'macro'}"}</Code>). Properties can be overridden just like normal JS
...
const styles = style({
  ...
  backgroundColor: 'base'
        ...
        <P>
          The <Code>focusRing</Code> utility generates styles for the standard Spectrum focus ring,
...
`)}
        ...
        <ul className="sb-unstyled">
          <li className={style({font: 'body-lg', marginY: 8})}>
            Ensure that the styles are extracted into a CSS bundle and not injected at runtime by{' '}
            <Code>{'<style>'}</Code> elements.
          </li>
          <li className={style({font: 'body-lg', marginY: 8})}>
            Use a CSS minifier such as{' '}
            <Link href="https://lightningcss.dev" target="_blank">
              Lightning CSS
            </Link>{' '}
            to deduplicate common rules used between components. Consider running this during
            development as well to reduce style duplication in developer tools for improved
            debugging.
          </li>
          <li className={style({font: 'body-lg', marginY: 8})}>
            Configure your bundler to combine all CSS for S2 components and style macros into a
            single bundle instead of code splitting. Atomic CSS results in a lot of overlap between
            components. With code splitting, common rules are duplicated between bundles by default.
            To avoid this, load the CSS for all used S2 components in a single bundle. Because of
            the high degree of overlap between components, this initial bundle will be quite small.
          </li>
        </ul>
        ...
        <Disclosure isQuiet>
          ...
          <DisclosurePanel>
            <P>
              ...
              . You can configure it to bundle all CSS for S2 components and style macros into a
              single file using the{' '}
              <Link
                href="https://parceljs.org/features/code-splitting/#manual-shared-bundles"
                target="_blank">
        ...
        <Disclosure isQuiet>
          ...
          <DisclosurePanel>
            <ul className="sb-unstyled">
              <li className={style({font: 'body-lg', marginY: 8})}>
                ...
                to extract the generated styles into a CSS bundle. Do not use{' '}
                <Code>style-loader</Code>, which injects individual <Code>{'<style>'}</Code> rules
                at runtime.
              </li>
              <li className={style({font: 'body-lg', marginY: 8})}>
                Use{' '}
                <Link
                  href="https://webpack.js.org/plugins/css-minimizer-webpack-plugin/"
                  target="_blank">
                  CssMinimizerWebpackPlugin
                </Link>{' '}
                to optimize the generated CSS using{' '}
                <Link href="https://lightningcss.dev" target="_blank">
                  Lightning CSS
                </Link>
                . You can also configure this to run in development to remove duplicate rules and
                improve debugging.
              </li>
              <li className={style({font: 'body-lg', marginY: 8})}>
                Use{' '}
                <Link href="https://webpack.js.org/plugins/split-chunks-plugin/" target="_blank">
                  SplitChunksPlugin
                </Link>{' '}
                to bundle all S2 and style-macro generated CSS into a single bundle.
              </li>
            </ul>
            <P>
              See our{' '}
              <Link
                href="https://github.com/adobe/react-spectrum/blob/main/examples/s2-webpack-5-example/webpack.config.js"
                target="_blank">
                webpack example
              </Link>{' '}
          ...
          <Disclosure isQuiet>
            ...
            <DisclosurePanel>
              <ul className="sb-unstyled">
                <li className={style({font: 'body-lg', marginY: 8})}>
                  Configure the <Code>cssMinify</Code> option to use{' '}
                  <Link href="https://lightningcss.dev" target="_blank">
                    Lightning CSS
                  </Link>
                  , which produces much smaller output than the default minifier.
                </li>
                <li className={style({font: 'body-lg', marginY: 8})}>
                  Configure Rollup to bundle all S2 and style-macro generated CSS into a single
                  bundle using the{' '}
                  <Link
                    href="https://rollupjs.org/configuration-options/#output-manualchunks"
                    target="_blank">
                    manualChunks
                  </Link>{' '}
                  feature.
              ...
              <P>
                See our{' '}
                <Link
                  href="https://github.com/adobe/react-spectrum/blob/main/examples/s2-vite-project/vite.config.ts"
                  target="_blank">
                  Vite example
                </Link>{' '}
                for full configuration options.
