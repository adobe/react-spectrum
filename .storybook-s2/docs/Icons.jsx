  ...
  return (
    <div className={'sb-unstyled ' + style({marginX: 'auto'})}>
      <div className={style({marginX: 48})}>
        <h1 className={style({font: 'heading-2xl', marginBottom: 48})}>Workflow icons</h1>
        <P>
          Spectrum 2 offers a subset of the icons currently available in React Spectrum v3. These
          icons can be imported from <Code>@react-spectrum/s2/icons</Code>.
        </P>
              ...
              <ActionButton
                ...
                onPress={() =>
                  navigator.clipboard.writeText(
                    `import ${importName} from '@react-spectrum/s2/icons/${name}';`
                  )
                }>
                <Icon />
                <Text>{name}</Text>
        ...
        <H2>Styling</H2>
        <P>
          The <Code>iconStyle</Code> macro can be used to set the size and color of a workflow icon.
          Icons support five t-shirt sizes, and a subset of the Spectrum colors.{' '}
          <Link href="?path=/docs/intro--docs#supported-css-properties-on-spectrum-components">
            Other style properties
          </Link>{' '}
          available across components are also supported on icons.
        </P>
        <Pre>
          {highlight(`import {iconStyle} from '@react-spectrum/s2/style' with {type: 'macro'};
import CheckmarkCircle from '@react-spectrum/s2/icons/CheckmarkCircle';

<CheckmarkCircle styles={iconStyle({size: 'L', color: 'positive'})} />`)}
        </Pre>
        <H3>Icon colors</H3>
        <IconColors />
        <H3>Icon sizes</H3>
        ...
        <H2>Custom icons</H2>
        <P>
          To use custom icons, you first need to convert your SVGs into compatible icon components.
          This depends on your bundler.
        </P>
        <H3>Parcel</H3>
        <P>
          If you are using Parcel, the <Code>@react-spectrum/parcel-transformer-s2-icon</Code>{' '}
          plugin can be used to convert SVGs to icon components. First install it into your project
          as a dev dependency:
        </P>
  ...
  "transformers": {
    "icon:*.svg": ["@react-spectrum/parcel-transformer-s2-icon"]
        ...
        <P>
          Now you can import icon SVGs using the <Code>icon:</Code>{' '}
        ...
        <P>
          ...
          folder of SVG icons into TSX files.
        </P>
        <Pre>
