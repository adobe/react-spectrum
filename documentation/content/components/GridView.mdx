...
<div style={{display: 'flex', justifyContent: 'space-between'}}>
  <GridView
    aria-label="Select images to send"
    layout={GridLayout}
    cardSize="S"
    ...
    canDragItems />
  <GridView
    aria-label="Images selected to send"
    layout={GridLayout}
    cardSize="S"
    dataSource={new TestDS([])}
    renderItem={renderItem}
...
GridView implements the [WAI-ARIA 1.1 Layout Grid](https://www.w3.org/TR/wai-aria-practices/#grid) design pattern. It handles multiselection, item focusing, and is keyboard accessibile out of the box.
The GridView does not have a keyboard or screen reader accessible equivalent for drag and drop operations, and drag and drop should never be the only way to interact with a feature, such as manipulating the order of items in the GridView or moving items between GridViews.

### Labelling

A GridView can be labelled to provide context for Accessibility using either the `aria-label` or `aria-labelledby` prop.
...
```jsx
<Heading size={5} id="gridview-heading-id">
  Image Gallery
</Heading>
<GridView
...
To internationalize a GridView, you should provide localized content to the items displayed in the grid.
