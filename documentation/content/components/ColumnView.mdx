...
ColumnView implements accessibility using the [WAI-ARIA 1.1 Tree View](https://www.w3.org/TR/wai-aria-practices-1.1/#TreeView) design pattern. It handles multiselection, item focusing, and is fully keyboard accessible out of the box.

### Labelling

A ColumnView can be labelled to provide context for Accessibility using either the `aria-label` or `aria-labelledby` prop.
...
```jsx
<Heading size={5} id="columnview-heading-id">
  Column View
</Heading>
<ColumnView
  aria-labelledby="columnview-heading-id"
  dataSource={ds}
  renderItem={renderItem}
  renderDetail={renderDetail}
  allowsSelection
