...
There is a base class for data sources that can be used by TableView called `ListDataSource`, which supports asynchronous
...
if the only drop targets you want to support are other TableViews, but you may want to support a custom data format for your
...
To enable dropping files or other objects in a TableView, pass the `allowsDrops` prop. This can be a boolean to accept all drops,
or a list of allowed drag types (e.g. `Files`, `text/plain` or your custom format name).

#### Handling drops

By default, TableView will handle drops from other TableViews, but you may need to support dropping files or other custom types
