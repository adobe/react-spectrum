...
The `isDateDisabled` callback prop makes it possible to disable custom dates from the `RangeCalendar`. It is being called for each date and if it returns `true`, the date cell is disabled.
...
<DateRangePicker label="Date range" isDateDisabled={isDateDisabled} />
```
