...
describe("DatePicker", function () {
  ...
  describe("calendar popover", function () {
    ...
    it("should display a time field when a CalendarDateTime value is used", async function () {
      ...
      expect(onChange).toHaveBeenCalledWith(
        new CalendarDateTime(2019, 2, 4, 8, 45)
      );
      ...
      expect(onChange).toHaveBeenCalledWith(
        new CalendarDateTime(2019, 2, 4, 9, 45)
      );
    ...
    it("should not throw error when deleting values from time field when CalendarDateTime value is used", async function () {
      ...
      expect(onChange).toHaveBeenCalledTimes(1);
      expect(onChange).toHaveBeenCalledWith(
        new CalendarDateTime(2019, 2, 4, 10, 45)
      );
      ...
      expect(onChange).toHaveBeenCalledTimes(2);
      expect(onChange).toHaveBeenCalledWith(
        new CalendarDateTime(2019, 2, 4, 1, 45)
      );
  ...
  describe("labeling", function () {
    ...
    it("should support labeling with aria-label", function () {
      let { getByRole, getAllByRole, getByTestId } = render(
        <DatePicker aria-label="Birth date" />
      );
      ...
      for (let segment of segments) {
        expect(segment).toHaveAttribute("id");
        expect(segment.getAttribute("aria-label").endsWith(" Birth date")).toBe(
          true
        );
    ...
    it("should support labeling with aria-labelledby", function () {
      let { getByRole, getAllByRole, getByTestId } = render(
        <DatePicker aria-labelledby="foo" />
      );
  ...
  describe("focus management", function () {
    ...
    it("should focus the previous segment when the era is removed", async function () {
      let { getByTestId, queryByTestId } = render(
        <DatePicker
          label="Date"
          defaultValue={new CalendarDate("BC", 2020, 2, 3)}
        />
      );
      let field = getByTestId("date-field");
      ...
      expect(document.activeElement).toBe(
        within(field).getAllByRole("spinbutton").pop()
      );
    });
  ...
  describe("editing", function () {
    describe("arrow keys", function () {
      ...
      describe("day", function () {
        ...
        it("should wrap around when incrementing and decrementing the day", async function () {
          await testArrows(
            "day,",
            new CalendarDate(2019, 2, 28),
            new CalendarDate(2019, 2, 1),
        ...
        it("should support using the home and end keys to jump to the min and max second", async function () {
          await testArrows(
            ...
            new CalendarDateTime(2019, 2, 3, 8, 5, 0),
            {
              upKey: "End",
              downKey: "Home",
              props: { granularity: "second", hourCycle: 24 },
            }
          );
        });
      });
    ...
    describe("backspace", function () {
      ...
      it("should support backspace in the minute segment", async function () {
        await testBackspace(
          "minute,",
          new CalendarDateTime(2019, 2, 3, 5, 8),
          null
        );
        await testBackspace(
          "minute,",
      ...
      it("should support second in the minute segment", async function () {
        await testBackspace(
          "second,",
          new CalendarDateTime(2019, 2, 3, 5, 5, 8),
          null,
          { granularity: "second" }
        );
        await testBackspace(
  ...
  describe("placeholder", function () {
    ...
    it("should confirm the placeholder on blur and only AM/PM is un-entered", function () {
      ...
      expect(onChange).toHaveBeenCalledWith(
        new CalendarDateTime(2022, 4, 5, 5, 45)
      );
    });
  });
    ...
    describe("validation", () => {
      describe("validationBehavior=native", () => {
        it("supports isRequired", async () => {
          ...
          expect(document.activeElement).toBe(
            within(group).getAllByRole("spinbutton")[0]
          );
        ...
        it("supports minValue and maxValue", async () => {
          ...
          let getDescription = () =>
            group
              ...
              .join(" ");
          expect(input.validity.valid).toBe(false);
          expect(getDescription()).not.toContain(
            "Value must be 2/3/2020 or later."
          );
          ...
          expect(group).toHaveAttribute("aria-describedby");
          expect(getDescription()).toContain(
            "Value must be 2/3/2020 or later."
          );
          expect(document.activeElement).toBe(
            within(group).getAllByRole("spinbutton")[0]
          ...
          expect(getDescription()).not.toContain(
            "Value must be 2/3/2024 or earlier."
          );
          expect(input.validity.valid).toBe(false);
          await user.tab();
            ...
            <Provider theme={theme}>
              <Form data-testid="form">
                <DatePicker
                  ...
                  validationBehavior="native"
                  errorMessage={(e) =>
                    e.validationDetails.valueMissing
