...
<Flex flexDirection="column" maxWidth="275px">
  ...
  <Meter aria-label="Label" showValueLabel={false} value={25} variant="warning" />
</Flex>
```

The value label can be updated via the `valueLabel` prop.
```tsx example
<Meter aria-label="Progress" marginBottom="25px" value={25} valueLabel="1 of 4" variant="warning" />
```
