...
import DropZoneAnatomy from './images/DropZoneAnatomy.svg';
...
A standard DropZone has the following anatomy:
...
In order to internationalize a drop zone, a localized string should be passed to the `Text` element with a `label` slot or to the `aria-label` prop, as well as the `replaceMessage` prop.
