...
Of the various hook options above, `getAllowedDropOperations` and `getDropOperation` maybe of particular interest since it allows you to specify what kinds of drag and drop operations you want to allow.
...
to your collection's `onDragEnd` handler, letting it know that it shouldn't remove the dragged items from the list. Alternatively, a drop target that allows `move` operations, like in the
