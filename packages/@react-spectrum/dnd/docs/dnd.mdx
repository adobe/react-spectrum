...
Due to the possible complexity of a droppable collection's behavior, `useDnDHooks` accepts various [high level drop event handlers and options](#high-level-drop-handlers-and-options) that help you enable common drop behaviors like
...
For a more general explanation of these low level options, see the `useDroppableCollection` aria [docs](/react-aria/useDroppableCollection.html#low-level-api).

#### Using high level options

For the second ListView we want to make it droppable, accepting drops between items and onto folders but not at the root of the list. Similar to the draggable ListView, we'll start by initializing the default item list
...
Since we only want to accept items from the first ListView, we setup an array containing our custom type that will be provided to `useDnDHooks`'s `acceptedDragTypes` option.

```tsx
let acceptedDragTypes = ['adobe-app'];
```

To handle the processing of the drop item data, we create a `itemProcessor` utility function that will be used in each of the drop handlers. See the [aria docs](/react-aria/useDroppableCollection.html#drop-data)
...
Next, we can add support for insertion drops by providing a `onInsert` handler. This handler uses the relative drop position to insert the dropped
...
Similarly, item drops are enabled by providing a `onItemDrop` handler. The folder's contents are updated by using `list.update` to add the dropped items to
...
Below is what our droppable ListView would look like after combining everything together. By not supplying a `onRootDrop` handler to `useDnDHooks`, we automatically
...
Note that `acceptedDragTypes` can still be used alongside `getDropOperation` where it acts as a pre-filter that rejects any drag operations that do not contain any of the drag types defined within `acceptedDragTypes`. We will be
using the same `acceptedDragTypes` from the previous example so that we don't need to perform any drag types checks within our `getDropOperation`. If you were to implement a more complex scenario, such as only allowing on folder drops if
the dragged items were of a specific mime type, you can use the `types` in `getDropOperation` to check the drag types of the current drag operation.
...
Below is what our droppable ListView would look like after combining everything together.
...
`useDnDHooks` will then provide you with a set of hooks that you can pass to the component via its `dndHooks` prop, thus enabling drag and drop operations for the component. This approach allows the drag and drop
...
you don't provide a `onDrop` handler or any of the high level drop handlers (`onInsert`/`onItemDrop`/`onReorder`/`onRootDrop`) to `useDnDHooks` then the component will not be droppable.

### High level drop handlers and options
<TypeContext.Provider value={dndDocs.links}>
  <InterfaceType properties={dndDocs.exports.UtilityOptions.properties} />
