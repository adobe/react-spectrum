...
text input drop target. The item's kind can be one of two types: `"string"` if the item is inline data in the form of a plain Unicode string or `"file"` if the item is a reference to a file from
the user's computer.
