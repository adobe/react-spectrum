...
It is a flexible, efficient, and intuitive way for users to perform a wide variety of tasks, and is widely supported across both desktop and mobile operating systems.
...
text input drop target. The item's kind can be one of two types: `"string"` if the item is inline data in the form of a plain Unicode string or `"file"` if the item is a reference to file(s) from
the user's computer.
...
In this case, we look up the information for each dragged item and serialize it, mapping it to the item's type (e.g. `folder` or `file`). This information will be processed and provided to the drop target's drop handlers on drop.
...
async function onDrop(e) {
  let itemsToAdd = await Promise.all(
    e.items
      .filter(item => item.kind === 'text' && item.types.has('file'))
      .map(async item => JSON.parse(await item.getText('file')))
  );
