...
```tsx example export=true render=false
import type {TextItem} from '@react-spectrum/dnd';
...
function DroppableList() {
  ...
  let {dragAndDropHooks} = useDragAndDrop({
    ...
    onInsert: async (e) => {
      ...
      let processedItems = await Promise.all(
        items.map(async (item: TextItem) => JSON.parse(await item.getText('adobe-app')))
      );
...
```tsx example
import type {FileItem, DirectoryItem} from '@react-spectrum/dnd';
    ...
    onDrop: async (e) => {
      let items = await Promise.all(
        e.items
          ...
          .map(async (item: FileItem | DirectoryItem) => {
