...
The above example makes use of several high-level events and options supported by `useDnDHooks` to define the droppable ListView's behavior. However, you may have complex operations
...
As demonstrated in this [example](#creating-the-droppable-list), `useDnDHooks` accepts a set of high-level drop handlers and options. Providing any of these high-level drop handlers will enable the corresponding
...
The full list of options for `useDnDHooks` is available below.
