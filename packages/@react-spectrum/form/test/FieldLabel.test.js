...
describe('FieldLabel', () => {
  ...
  describe('with label', () => {
    ...
    it.each`
    Name                                 | Component       | numChildren | props
    ${'v3 FieldLabel no children'}       | ${FieldLabel}   | ${0}        | ${{isRequired: true, necessityIndicator: 'label'}}
    ${'v3 FieldLabel one child'}         | ${FieldLabel}   | ${1}        | ${{isRequired: true, necessityIndicator: 'label'}}
    ${'v3 FieldLabel multiple children'} | ${FieldLabel}   | ${2}        | ${{isRequired: true, necessityIndicator: 'label'}}
    ${'v3 FieldLabel no children'}       | ${FieldLabel}   | ${0}        | ${{isRequired: false, necessityIndicator: 'label'}}
    ${'v3 FieldLabel one child'}         | ${FieldLabel}   | ${1}        | ${{isRequired: false, necessityIndicator: 'label'}}
    ${'v3 FieldLabel multiple children'} | ${FieldLabel}   | ${2}        | ${{isRequired: false, necessityIndicator: 'label'}}
  `('$Name will show a necessityLabel when necessityIndicator is \'label\'', ({Component, numChildren, props}) => {
    let {findByText} = renderFieldLabel(Component, {label, labelFor, ...props}, numChildren);
    let fieldLabel = findLabel();
    let necessityLabel;
    if (props.isRequired) {
      necessityLabel = findByText(/\((required)\)/g);
      expect(necessityLabel).toBeTruthy();
      if (numChildren === 1) {
        expect(fieldLabel.querySelector('span')).toHaveAttribute('aria-hidden', 'true');
      }
    } else {
      necessityLabel = findByText(/\((optional)\)/g);
      expect(necessityLabel).toBeTruthy();
      expect(fieldLabel.querySelector('span')).toBeNull();
    }
  });
