...
>(props: SpectrumButtonProps<T>, ref: FocusableRef<HTMLElement>) {
  ...
  let hasAriaLabel =
    !!buttonProps["aria-label"] || !!buttonProps["aria-labelledby"];
  ...
  const isPendingAriaLiveLabel = `${
    hasAriaLabel ? buttonProps["aria-label"] : ""
  } ${stringFormatter.format("pending")}`.trim();
  ...
  return (
    ...
    >
      <Element
        ...
        aria-label={
          isPending ? isPendingAriaLiveLabel : buttonProps["aria-label"]
        }
        aria-labelledby={
          isPending
