...
function Example(demo) {
  ...
  let obj = {one: 1, two: 2};
  let {one, two} = obj;
  let nextObj = {...obj, three: 3};
  let theString = "a string";
  let theLiteral = `a string ${theString}`;

  // comment!
  return (
    <Button {...nextObj} variant="primary" onPress={(something) => setCount(c => c + 1)}>{count} Dogs</Button>
  );
}
```
