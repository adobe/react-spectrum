...
Buttons can indicate that a quick progress taking place (e.g., saving settings on a server). After a 1 second delay an indeterminate spinner will be displayed in place of the button label and icon. You can trigger this behavior with the `isPending` prop.
