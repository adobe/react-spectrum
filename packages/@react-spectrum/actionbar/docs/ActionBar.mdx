...
An ActionBar is a popover that consists of an ActionGroup, a clear button, and a count of selected items. It accepts a list of <TypeLink links={docs.links} type={docs.exports.Item} />
...
function DynamicActionBar(props) {
  let columns = [
    // Same table columns and rows as above...
    ///- begin collapse -///
    {name: 'Name', uid: 'name'},
    {name: 'Type', uid: 'type'},
    {name: 'Level', uid: 'level'}
  ...
  return (
    <ActionBarContainer height={containerHeight}>
      <TableView aria-label="Table with controlled selection" selectionMode="multiple" selectedKeys={selectedKeys} onSelectionChange={setSelectedKeys} height="size-3000">
        <TableHeader columns={columns}>
          {column => (
            <Column key={column.uid} align={column.uid === 'level' ? 'end' : 'start'}>
...
<DynamicActionBar containerHeight="size-5000" />
```
...
### isEmphasized
{/* TODO: add spectrum link and remove blurb when it moves out of contributions */}
