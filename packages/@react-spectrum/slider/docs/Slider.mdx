...
the Slider's minimum and maximum values, each of which default to 0 and 100 respectively.
...
function Example() {
  let [value, setValue] = React.useState(25);
  return <Slider label="Cookies to buy" marginBottom="size-300" value={value} onChange={setValue} valueLabel={`${value} of 100`} />
}
```
