...
function Autocomplete<T extends object>(props: SpectrumAutocompleteProps<T>, ref: FocusableRef<HTMLElement>) {
  props = useProviderProps(props);

  let isMobile = useIsMobileDevice();
  if (isMobile) {
    // menuTrigger=focus/manual don't apply to mobile autocomplete
    return <MobileAutocomplete {...props} menuTrigger="input" ref={ref} />;
  } else {
    return <AutocompleteBase {...props} ref={ref} />;
  }
}

const AutocompleteBase = React.forwardRef(function AutocompleteBase<T extends object>(props: SpectrumAutocompleteProps<T>, ref: FocusableRef<HTMLElement>) {
  props = useProviderProps(props);
  ...
  let {inputProps, listBoxProps, labelProps} = useAutocomplete(
    {
      ...props,
...
interface AutocompleteInputProps extends SpectrumAutocompleteProps<unknown> {
...
const AutocompleteInput = React.forwardRef(function AutocompleteInput(props: AutocompleteInputProps, ref: RefObject<HTMLElement>) {
  let {
    isQuiet,
    isDisabled,
    validationState,
...
/**
 * An Autocomplete component is a field that supports a dynamic list of suggestions.
 */
let _Autocomplete = forwardRef(Autocomplete);
