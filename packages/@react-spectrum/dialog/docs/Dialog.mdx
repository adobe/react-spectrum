...
The layout of a Dialog is made up of the following elements and sections, mapped to the corresponding default slot names:
...
Each of these sections can be populated by providing a semantic element paired with a corresponding `slot` prop
to the Dialog as children. Note that the order of the children provided does not affect the visual layout of the Dialog. 

The example below illustrates how a Dialog with a hero image could be rendered.
...
of rendering a close button. If your Dialog is dismissable, make sure that you do not include a footer section. 
While the Dialog will hide any elements occupying the `footer` slot, it will not remove them from the DOM, 
