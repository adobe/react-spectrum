...
a ButtonGroup. While the Dialog will hide any elements occupying the `footer` slot, it will not remove them from the DOM, 
resulting in possible tab order issues. In the example below, note that the dismissable Dialog does not need
to be wrapped in a `close` function unlike the non-dismissable example before.
