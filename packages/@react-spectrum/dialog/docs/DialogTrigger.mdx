...
portion of the page behind the underlay. 
...
background covers the entire screen. The page behind the dialog is not visible. This variant
should be reserved for workflows where displaying a second dialog on top of the first one is 
to be expected.
...
If your modal, fullscreen, or fullscreen takeover Dialog doesn't require the user 
to make a confirmation, you can set `isDismissable` on the DialogTrigger. This adds
an exit button that the user can click to dismiss the Dialog.
...
function Example() {
  ...
  return (
    <div ref={ref} style={{border: '5px solid red', height: "500px", width: "500px"}}>
      <DialogTrigger placement="top" shouldFlip type="popover" boundaryElement={ref.current}>
        <ActionButton>Trigger</ActionButton>
        <Dialog>
          <Heading>The Heading</Heading>
          <Divider />
...
<DialogTrigger hideArrow type="popover">
  ...
  <Dialog>
    ...
    <Divider />
    <Content>
      This is a popover with its tip hidden.
    </Content>
  </Dialog>
</DialogTrigger>
```
