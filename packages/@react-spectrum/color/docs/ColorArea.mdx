...
`xChannel` and `yChannel` props may must also be provided to specify which color channels the color area should should display, and the direction of each channel in the color gradient.
...
"Background color", which ColorArea will return as the `aria-label` prop "Background color, Color picker"
for each of the `<input>` elements, or on mobile devices, the group containing them. If you provide your own
...
To build a fully functional color picker, the ColorArea control, that adjusts two channels of a color value stored in state,
should be combined with a separate control, like a [ColorSlider](ColorSlider.html) or [ColorWheel](useColorWheel.html), for
...
This example shows how you could build an RGB color picker using a color area and a color sliders bound to the same
  ...
  return (
    <fieldset style={{border: 0}}>
      ...
      <Flex
        ...
        alignItems="start"
        UNSAFE_style={{
          marginTop: 'calc( .25 * var(--spectrum-global-dimension-size-2400))'
        }}>
        <Flex
          alignItems="center"
          direction="column">
...
function Example() {
  ...
  let [hueChannel, saturationChannel, brightnessChannel] = color.getColorChannels();
  return (
    <fieldset style={{border: 0}}>
      <legend>{color.getColorSpace().toUpperCase()}A Example</legend>
      <Flex
        ...
        <ColorSlider channel="alpha" value={color} onChange={setColor} />
        <p>Current value: {color.toString(`${color.getColorSpace()}a`)}</p>
      </Flex>
    </fieldset>
  );
}
