...
Note, this will not bring up link context menus in mobile, it's recommended that you use an anchor tag or something that will render one.

```tsx example
function TrackLink () {
  let [visited, setVisited] = React.useState(false)
  return (
    <>
        <Link onPress={() => setVisited(true)}>A label</Link>
        <br />
        {visited && 'You visited the link!'}
    </>
  );
}
```
...
function GetPressTarget () {
  ...
  return (
    <>
        <Link onPress={e => setText(`clicked "${e.target.textContent}" Link`)}>
