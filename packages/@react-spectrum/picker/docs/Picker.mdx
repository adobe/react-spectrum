...
```
### Deselection
Unsetting a selected option can be done by using `selectedKey` and `onSelectionChange` handler
...
function Example() {
  ...
  return (
    <Picker
      label="Pick an animal (deselection allowed)"
      items={options}
      selectedKey={animal}
      onSelectionChange={selected => setAnimal(animal === selected ? null : selected)}>
      {item => <Item key={item.name}>{item.name}</Item>}
    </Picker>
  );
}
