...
Alternatively, providing a `value` prop creates a TextArea with a controlled value. 

```tsx example
<TextArea defaultValue="Default text" label="Uncontrolled TextArea" marginEnd="20px" />
<TextArea value="Default text" label="Controlled TextArea" />
```
...
Placeholder text will only appear when the TextArea is empty, and should not be used as a substitute for labeling the control with a visible label.
