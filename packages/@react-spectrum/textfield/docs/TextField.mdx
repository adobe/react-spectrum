...
```tsx example
import {Form, Button, ButtonGroup} from '@adobe/react-spectrum';
...
```tsx example
<Form validationBehavior="native">
  <TextField label="Tracking number" validationState={value => value.length === 8 ? 'valid' : 'invalid'} errorMessage="Please enter an 8-digit tracking number." />
  <ButtonGroup>
    <Button type="submit">Submit</Button>
  </ButtonGroup>
</Form>
...
function Example() {
  ...
  return (
    <Form validationBehavior="native" onSubmit={onSubmit}>
      <TextField label="Tracking number" validationState={validationState} />
      <ButtonGroup>
        <Button type="submit">Submit</Button>
      </ButtonGroup>
    </Form>
