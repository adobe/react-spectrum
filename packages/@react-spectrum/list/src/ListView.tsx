...
export interface SpectrumListViewProps<T>
  extends Omit<AriaGridListProps<T>, "keyboardNavigationBehavior">,
    StyleProps,
    SpectrumSelectionProps,
...
export const ListViewContext =
  React.createContext<ListViewContextValue<unknown> | null>(null);
...
export const ListView = React.forwardRef(function ListView<T extends object>(
  ...
  useEffect(() => {
    if (
      dragHooksProvided.current !== isListDraggable &&
    ...
    if (
      dropHooksProvided.current !== isListDroppable &&
      process.env.NODE_ENV !== "production"
    ) {
      console.warn(
      ...
      {DragPreview && isListDraggable && dragAndDropHooks && dragState && (
        <DragPreview ref={preview}>
          {() => {
            ...
            let itemHeight =
              layout.getLayoutInfo(dragState.draggedKey)?.rect.height ?? 0;
            return (
              <SpectrumDragPreview
                item={item}
                itemCount={itemCount}
  ...
  props: SpectrumListViewProps<T> & { ref?: DOMRef<HTMLDivElement> }
) => ReactElement;
