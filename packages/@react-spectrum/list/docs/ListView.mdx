...
function ReorderableList() {
  ...
  // Same acceptedDragTypes as last example...
  ///- begin collapse -///
...
function DragBetweenListsExample() {
  // Similar set list item data as previous examples
  ...
  // Same acceptedDragTypes as previous examples...
  ///- begin collapse -///
  // Append a generated key to the item type so they can only be reordered within this list and not dragged elsewhere.
  let dragType = React.useMemo(() => `keys-${Math.random().toString(36).slice(2)}`, []);
  let acceptedDragTypes = [`${dragType}-folder`, `${dragType}-file`];
  ...
  let {dndHooks: list1Hooks} = useDnDHooks({
    getItems(keys) {
      // Same configuration as DragIntoListMultipleTypes but using list1
      ///- begin collapse -///
      return [...keys].map(key => {
