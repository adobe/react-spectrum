...
You can disable specific rows by providing an array of keys to ListView via the `disabledKeys` prop. By default, this will disable all interactions on disabled rows.
...
You can use the `disabledBehavior` prop to either disable all interactions, or just selection on disabled rows. With `disabledBehavior` set to `selection`, interactions such as focus, dragging, or actions can still be performed on disabled rows.
