...
import type {DragAndDropOptions, TextItem} from '@react-spectrum/dnd';
  ...
  let {dragAndDropHooks} = useDragAndDrop({
    ...
    onInsert: async (e) => {
      ...
      let processedItems = await Promise.all(
        items.map(async (item: TextItem) => JSON.parse(await item.getText('custom-app-type')))
      );
...
function DroppableListViewFolder(props: DndListViewProps) {
  ...
  let {dragAndDropHooks} = useDragAndDrop({
    ...
    onInsert: async (e) => {
      ...
      let processedItems = await Promise.all(
        items.map(async (item: TextItem) => JSON.parse(await item.getText('custom-app-type-folder-drop')))
      );
...
function BidirectionalDnDListView(props: DndListViewProps) {
  ...
  let {dragAndDropHooks} = useDragAndDrop({
    ...
    onInsert: async (e) => {
      ...
      let processedItems = await Promise.all(
        items.map(async (item: TextItem) => JSON.parse(await item.getText('custom-app-type-bidirectional')))
      );
      if (target.dropPosition === 'before') {
        list.insertBefore(target.key, ...processedItems);
      } else if (target.dropPosition === 'after') {
