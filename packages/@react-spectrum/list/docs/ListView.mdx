...
function DragIntoListExample() {
  ...
  let dropHooks = useDropHooks({
    ...
    getDropOperation(target, types) {
      ...
      // Cancel the drop operations if any of drag items aren't a matching type
      if (!draggedTypes.every(type => acceptedDragTypes === type)) {
        return 'cancel';
      }
  ...
  return (
    <Flex wrap gap="size-300">
      <ListView
        aria-label="Draggable list view in drag into list example"
        selectionMode="multiple"
        width="size-3600"
        height="size-2400"
      ...
      <ListView
        aria-label="Droppable list view in drag into list example"
        width="size-3600"
         height="size-2400"
...
function DragIntoListFilesOnly() {
  ...
  let dropHooks = useDropHooks({
    ...
    getDropOperation(target, types) {
      let typesSet = types.types ? types.types : types;
      let draggedTypes = [...typesSet.values()];
      // Cancel the drop operations if any of drag items aren't a file
      if (!draggedTypes.every(type => acceptedDragTypes === type)) {
        return 'cancel';
      }
  ...
  return (
    <Flex wrap gap="size-300">
      <ListView
        aria-label="Draggable list view in drag into list files only example"
        selectionMode="multiple"
        width="size-3600"
        height="size-3600"
        items={sourceList.items}
      ...
      <ListView
        aria-label="Droppable list view in drag into list files only example"
        width="size-3600"
        height="size-3600"
        items={targetList.items}
        dropHooks={dropHooks}>
...
function ReorderableList() {
  ...
  let dropHooks = useDropHooks({
    ...
    getDropOperation(target, types) {
      let typesSet = types.types ? types.types : types;
      let draggedTypes = [...typesSet.values()];
      // Cancel the drop operations if any of drag items aren't of the acceptedTypes or if the drop target is root
      if (!draggedTypes.every(type => acceptedDragTypes.includes(type)) || target.type === 'root') {
        return 'cancel';
      }
...
the ability to drop into a folder and onto the root of the list.
<details>
  <summary style={{fontWeight: 'bold'}}><ChevronRight size="S" /> Show code</summary>
...
function DragBetweenListsExample() {
  ...
  let dropHooks = useDropHooks({
    ...
    getDropOperation(target, types) {
      ...
      if (!draggedTypes.every(type => acceptedDragTypes.includes(type)) || target.type === 'root' || target.dropPosition === 'on') {
        return 'cancel';
      }
