...
function DragIntoList() {
  ...
  let {dragAndDropHooks: dropListHooks} = useDragAndDrop({
    acceptedDragTypes: [`${dragType}-file`],
    onInsert: async (e) => {
      let {
        items,
        target
      ...
      let processedItems = await Promise.all(
        items.map(async item => JSON.parse(await item.getText(`${dragType}-file`)))
      );
...
function DragIntoListMultipleTypes() {
  ...
  let {dragAndDropHooks: dragListHooks} = useDragAndDrop({
    // Same configuration as DragIntoList example...
    ///- begin collapse -///
    getItems: (keys) => [...keys].map(key => {
      let item = sourceList.getItem(key);
      // Setup the info for each item that will be propagated upon drag
  ...
  let {dragAndDropHooks: dropListHooks} = useDragAndDrop({
    ...
    acceptedDragTypes,
    onInsert: async (e) => {
      // Same configuration as DragIntoList example's dropListHooks...
      ///- begin collapse -///
      let {
        items,
        target
...
function ReorderableList() {
  let list = useListData({
    // Similar configuration as DragIntoListMultipleTypes example source list...
    ///- begin collapse -///
    initialItems: [
      {id: '1', type: 'file', name: 'Adobe Photoshop'},
      {id: '2', type: 'file', name: 'Adobe XD'},
  ...
  // Same acceptedDragTypes as DragIntoListMultipleTypes example...
  ///- begin collapse -///
  ...
  let {dragAndDropHooks} = useDragAndDrop({
    // Same getItems as DragIntoList example and same acceptedDragTypes as DragIntoListMultipleTypes example...
    ///- begin collapse -///
    getItems(keys) {
      return [...keys].map(key => {
...
the ability to drop into a folder. Each list is internally reorderable and only support `move` drop operations.
<details>
  <summary style={{fontWeight: 'bold'}}><ChevronRight size="S" /> Show code</summary>

```tsx example export=true render=false
function DragBetweenListsExample() {
  // Similar set of list item data as DragIntoListMultipleTypes example
  ...
  let {dragAndDropHooks: list1Hooks} = useDragAndDrop({
    ...
    /*- end highlight -*/
    getItems(keys) {
      // Same configuration as DragIntoListMultipleTypes example but using list1
      ///- begin collapse -///
      return [...keys].map(key => {
    ...
    onReorder: async (e) => {
      // Same configuration as ReorderableList but using list1
      ///- begin collapse -///
      let {
        keys,
        target
...
function DragIntoListDefaultCopy() {
  let sourceList = useListData({
    // Similar configuration as DragIntoList example...
    ///- begin collapse -///
    initialItems: [
      {id: '1', type: 'file', name: 'Adobe Photoshop'},
      {id: '2', type: 'file', name: 'Adobe XD'},
  ...
  // Same dragType generation as DragIntoList example...
  ///- begin collapse -///
  // Append a generated key to the item type so they can only be dragged between these two lists
  let dragType = React.useMemo(() => `keys-${Math.random().toString(36).slice(2)}`, []);
  ///- end collapse -///
  let {dragAndDropHooks: dragListHooks} = useDragAndDrop({
    // Same getItems and onDragEnd as DragIntoList example...
    ///- begin collapse -///
    getItems: (keys) => [...keys].map(key => {
      let item = sourceList.getItem(key);
      // Setup the info for each item that will be propagated upon drag
  ...
  let {dragAndDropHooks: dropListHooks} = useDragAndDrop({
    ...
    /*- end highlight -*/
    onInsert: async (e) => {
      // Similar onInsert as DragIntoList example...
      ///- begin collapse -///
      let {
        items,
        target
      ...
      let processedItems = await Promise.all(
        items.map(async item => (
          {...JSON.parse(await item.getText(`${dragType}-file`)), id: Math.random().toString(36).slice(2)}
        ))
      );
      /*- end highlight -*/
      // ...
    ...
    onRootDrop: async (e) => {
      // Similar onRootDrop as DragIntoList example...
      ///- begin collapse -///
      let {
        items
      } = e;
