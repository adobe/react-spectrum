...
<Flex width={500}>
  <TagGroup items={items} disabledKeys={[2,4,5,8]} isRemovable aria-label="Disabled TagGroup example">
    {(item) => (
      <Item key={item.key}>
        {item.name}
      </Item>
...
<Flex width={500}>
  <TagGroup items={items} isRemovable aria-label="Removable TagGroup example">
    {(item) => (
      <Item key={item.key}>
        {item.name}
      </Item>
