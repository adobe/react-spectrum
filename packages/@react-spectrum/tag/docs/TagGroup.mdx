...
### Removing tags
Adding the `allowsRemoving` prop will allow the tags to be removed from the group. The `onRemove` callback is then
...
function Example() {
  ...
  let removeItem = (key) => {
    let updatedItems = items.filter((item) => key !== item.id);
