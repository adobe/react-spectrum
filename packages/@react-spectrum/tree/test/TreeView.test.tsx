...
describe("Tree", () => {
  ...
  it("should support DOM props", () => {
    let { getByRole, getAllByRole } = render(
      <StaticTree
        treeProps={{ "data-testid": "test-tree" }}
        rowProps={{ "data-testid": "test-row" }}
      />
    );
  ...
  it("should not label an expandable row as having an action if it supports selection", () => {
    let { getAllByRole } = render(
      <StaticTree treeProps={{ selectionMode: "single" }} />
    );
  ...
  it("should not render checkboxes for selection with selectionStyle=highlight", async () => {
    let { getByRole } = render(
      <StaticTree
        treeProps={{ selectionMode: "multiple", selectionStyle: "highlight" }}
      />
    );
    let treeTester = testUtilUser.createTester("Tree", {
      user,
  ...
  describe("general interactions", () => {
    it("should support hover on rows", async () => {
      let tree = render(
        <StaticTree treeProps={{ selectionMode: "multiple" }} />
      );
    ...
    it("should support focus", async () => {
      let tree = render(
        <StaticTree
          treeProps={{ selectionMode: "multiple", disabledKeys: ["Projects"] }}
        />
      );
    ...
    it("should support actions on rows", async () => {
      ...
      await treeTester.triggerRowAction({
        row: expandableRow,
    ...
    describe("links", function () {
      describe.each(["mouse", "keyboard"])("%s", (type) => {
        ...
        it('should support links with selectionMode="none"', async () => {
          ...
          expect(onClick.mock.calls[0][0].target.href).toBe(
            "https://google.com/"
          );
        });
    ...
    describe("keyboard interactions", () => {
      it("left and right arrows should navigate between interactive elements in the row", async () => {
        let { getAllByRole } = render(
          <DynamicTree treeProps={{ selectionMode: "multiple" }} />
        );
  ...
  describe("expanding and collapsing", () => {
    describe.each(["mouse", "keyboard"])("%s", (type) => {
      ...
      it("should not expand when clicking/using Enter on the row if the row has a link", async () => {
        let { getAllByRole } = render(
          <DynamicTree rowProps={{ href: "https://google.com" }} />
        );
        let rows = getAllByRole("row");
    ...
    it("should support controlled expansion", async () => {
      function ControlledTree() {
        ...
        return (
          <DynamicTree
            treeProps={{ expandedKeys, onExpandedChange: setExpandedKeys }}
          />
        );
      }
