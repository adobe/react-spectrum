...
It is highly suggested the `children` are Item components from the `@react-stately/collections` library.
...
It is possible (but not suggested) to use alternative components as children,
as long as those components have a `uniqueKey` prop and render children.

```tsx example
function Example() {
  let ExampleItem = ({children}) => <span>{children}</span>;
  return (
    <Breadcrumbs>
      <ExampleItem uniqueKey="Folder 1">Folder 1</ExampleItem>
      <ExampleItem uniqueKey="Folder 2">Folder 2</ExampleItem>
      <ExampleItem uniqueKey="Folder 3">Folder 3</ExampleItem>
    </Breadcrumbs>
  );
}
```
...
Breadcrumbs truncate by default when there is not enough room to display all items.

Truncation of breadcrumb items begins when either there is not enough room to display all items,
or if there are 5 or more breadcrumbs to display.
