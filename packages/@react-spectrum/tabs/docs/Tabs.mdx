...
<Tabs aria-label="History of Ancient Rome">
  <Item title="Founding of Rome" key="FoR">
    <Content marginTop="size-250" marginStart="size-125" marginBottom="size-250">
...
To internationalize Tabs, a localized string should be passed to the `title` or [`textValue`](#complex-items) of each `Item`. Any text content within the Tab's panel should also be localized accordingly.
...
function Example() {
  let unauthorizedPage = (heading) => (
    <Content marginTop="size-250">
      <Heading marginStart="size-125">{heading}</Heading>
      <IllustratedMessage>
  ...
  let tabs = [
    {
      name: 'Your files',
      icon: <AppleFiles />,
      children: unauthorizedPage('Files')
    },
    {
      name: 'Shared with you',
      icon: <UserShare />,
      children: unauthorizedPage('Shared Files')
    },
    {
      name: 'Libraries',
      icon: <Book />,
      children: unauthorizedPage('Available Libraries')
    }
  ];
...
```tsx example
<div style={{minWidth: '100px', width: '300px', height: '100px', padding: '10px', resize: 'horizontal', overflow: 'auto', backgroundColor: 'var(--spectrum-global-color-gray-50)'}}>
