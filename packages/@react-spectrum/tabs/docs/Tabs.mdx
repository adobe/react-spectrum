...
each with a `key` prop. The content of the Tab itself is defined via props passed to the `<Item>` element whereas the contents of the Tab's panel are defined by the `<Item>` element's children.
