...
This will prevent screen readers from announcing non-focused tabs, allowing for a more focused accessibilty experience.
...
### Internationalization
To internationalize a complex tab, any text content within the `title` and `textValue` of each item should be replaced with a localized string.
...
function Example() {
  ...
  return (
    <>
      ...
      <Button onPress={() => setCollapse((collapse) => !collapse)}>
        Flip tab container size.
      </Button>
    </>
  );
}
