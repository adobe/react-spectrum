...
controlled properties accordingly. Note that you should provide a `onSelectionChange` handler for a ComboBox with controlled input value and open state so that you can properly
control the menu's open state when the user selects a option or blurs from the field.
