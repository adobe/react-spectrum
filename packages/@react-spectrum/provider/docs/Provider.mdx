...
## useProvider
<TypeLink links={docs.links} type={docs.exports.useProvider} /> can be used to access the various properties applied by the nearest parent Provider. This can be useful for adapting a component's various features, such as its icon, to the current color scheme or
scale.

The example below uses the `colorScheme` sourced from `useProvider` to determine the top button's icon when its parent Provider's `colorScheme` is flipped. Note how the icon of the ActionButton within the nested Provider doesn't change since the `colorScheme` for its
parent Provider is fixed.
...
function ColorSchemeButton() {
  ...
  let label = colorScheme === 'dark' ? 'Moon icon button for dark theme' : 'Light icon button for light theme';
  return (
    <Flex gap="size-100" direction="column" alignItems="start" marginBottom="size-100">
      <View>Parent provider color scheme: {colorScheme}</View>
      <ActionButton aria-label={label}>
        {colorScheme === 'dark' ? <Moon /> : <Light />}
      </ActionButton>
    </Flex>
  );
}
...
function Example() {
  ...
  return (
    <Flex direction="column" gap="size-100" alignItems="start">
      <ToggleButton onChange={(selected) => setColorScheme(selected ? 'light' : 'dark')}>
        Toggle light mode
      </ToggleButton>
      <Provider colorScheme={colorScheme}>
        <View width="size-3600" borderWidth="thickest" padding="size-200">
          <ColorSchemeButton />
          <Provider colorScheme="light">
            <Flex direction="column" alignContent="center">
              <View width="100%" borderWidth="thickest" padding="size-200">
                <ColorSchemeButton />
              </View>
            </Flex>
          </Provider>
