...
The example below uses `useProvider` to dynamically change the button's icon when the docs theme is flipped. Note how the icon inside the nested Provider doesn't change when you toggle the theme switcher.
