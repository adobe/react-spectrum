...
The example below uses the `colorScheme` sourced from `useProvider` to determine the top button's icon when the docs theme is flipped. Note how the icon of the ActionButton within the nested Provider doesn't change when you toggle the theme switcher since the `colorScheme` for its
...
function ColorSchemeButton() {
  ...
  return (
    <ActionButton aria-label={label} margin="size-100">
...
function Example() {
  ...
  return (
    <Flex direction="column" gap="size-100" alignItems="start">
      ...
      <Provider colorScheme={colorScheme} UNSAFE_style={{padding: 20, textAlign: 'center', width: 300}}>
        <ColorSchemeButton />
        <Provider colorScheme="light" UNSAFE_style={{padding: 20, textAlign: 'center', margin: 20}}>
          <ColorSchemeButton />
        </Provider>
      </Provider>
    </Flex>
