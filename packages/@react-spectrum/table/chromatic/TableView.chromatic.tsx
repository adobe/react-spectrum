...
const SectionsTemplate = ({columns, items, ...args}) => (
  <Grid columns={repeat(3, '1fr')} autoFlow="row" gap="size-300">
    {combinations.map(c => {
      let key = Object.keys(c).map(k => shortName(k, c[k])).join(' ');
      if (!key) {
...
export const DefaultSections = {
  ...
  name: 'default items and columns (sections)',
  args: {columns: columnsWithSections, items: itemsWithSections}
};
...
export const SectionColumnDividers = {
  render: SectionsTemplate,
  name: 'column dividers (sections)',
  args: {columns: columnSectionDividers, items: itemsWithSections}
};
