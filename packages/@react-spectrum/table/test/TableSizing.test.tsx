...
describe("TableViewSizing", function () {
    ...
    describe("row heights", function () {
      ...
      it('should return the proper cell z-indexes for overflowMode="wrap"', function () {
        ...
        for (let [index, cell] of headerRow.childNodes.entries()) {
          ...
          expect(Number((cell as HTMLElement).style.zIndex)).toBe(
            4 - index + 1
          );
        }
    ...
    describe("column widths", function () {
      ...
      describe("bounded constraint on columns where dynamic columns exist before the bounded columns", () => {
        it("should fulfill the constraints of the static columns and give remaining width to previously defined dynamic columns", () => {
          ...
          for (let row of rows) {
            expect((row.childNodes[0] as HTMLElement).style.width).toBe(
              "600px"
            );
            expect((row.childNodes[1] as HTMLElement).style.width).toBe(
              "200px"
            );
            expect((row.childNodes[2] as HTMLElement).style.width).toBe(
              "200px"
            );
          }
        });
      ...
      it("should compute the correct widths for tiered headings with selection", function () {
        ...
        expect((rows[0].childNodes[0] as HTMLElement).style.width).toBe(
          "230px"
        );
        expect((rows[0].childNodes[1] as HTMLElement).style.width).toBe(
          "770px"
        );

        expect((rows[1].childNodes[0] as HTMLElement).style.width).toBe(
          "230px"
        );
        expect((rows[1].childNodes[1] as HTMLElement).style.width).toBe(
          "385px"
        );
        expect((rows[1].childNodes[2] as HTMLElement).style.width).toBe(
          "193px"
        );
        expect((rows[1].childNodes[3] as HTMLElement).style.width).toBe(
          "192px"
        );
  ...
  describe("resizing columns", function () {
    describe("pointer", () => {
      ...
      it("dragging the resizer works - desktop", () => {
        ...
        fireEvent.pointerLeave(resizableHeader, {
          pointerType: "mouse",
          pointerId: 1,
        });
        act(() => {
    ...
    describe("keyboard", () => {
      ...
      `(
        "arrow keys the resizer works - desktop $Name",
        async ({ renderer, getContainer }) => {
          jest
  ...
  describe("updating columns", function () {
    ...
    it("should update the row widths when removing and adding columns", async function () {
      ...
      let audienceCheckbox = tree.getByLabelText(
        "Audience Type"
      ) as HTMLInputElement;
      let budgetCheckbox = tree.getByLabelText(
        "Net Budget"
      ) as HTMLInputElement;
      let targetCheckbox = tree.getByLabelText(
        "Target OTP"
      ) as HTMLInputElement;
      ...
      expect(
        parseInt((rows[1].childNodes[1] as HTMLElement).style.width, 10)
      ).toBeLessThan(parseInt(oldWidth, 10));
    });
  });
  ...
  describe("headerless columns", function () {
    ...
    it("renders  table with headerless column with default scale", function () {
      ...
      expect(
        className.includes("spectrum-Table-cell--hideHeader")
      ).toBeTruthy();
      expect(headers[0]).toHaveTextContent("Foo");
      // visually hidden syle
      expect((headers[1].childNodes[0] as HTMLElement).style.clipPath).toBe(
        "inset(50%)"
      );
      expect((headers[1].childNodes[0] as HTMLElement).style.width).toBe("1px");
      expect((headers[1].childNodes[0] as HTMLElement).style.height).toBe(
        "1px"
      );
