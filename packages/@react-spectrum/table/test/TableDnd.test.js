  ...
  describe('drag and drop', function () {
    ...
    describe('via mouse', function () {
      ...
      it.skip('should allow moving one row into another table', async function () {
        ...
        let rowgroups1 = within(grid1).getAllByRole('rowgroup');
        let rowgroups2 = within(grid2).getAllByRole('rowgroup');
        ...
        let dataTransfer = new DataTransfer();
        fireEvent.pointerDown(table1rows[0], {pointerType: 'mouse', button: 0, pointerId: 1, clientX: 0, clientY: 0});
        fireEvent(table1rows[0], new DragEvent('dragstart', {dataTransfer, clientX: 0, clientY: 0}));
        act(() => {jest.runAllTimers();});
        fireEvent(table1rows[0], new DragEvent('drag', {dataTransfer, clientX: 0, clientY: 0}));
        fireEvent(table1rows[0], new DragEvent('dragenter', {dataTransfer, clientX: 0, clientY: 0}));
        fireEvent(grid2, new DragEvent('dragover', {dataTransfer, clientX: 0, clientY: 0}));
        expect(onDragStart).toHaveBeenCalledTimes(1);

        fireEvent.pointerMove(table1rows[0], {pointerType: 'mouse', button: 0, pointerId: 1, clientX: 200, clientY: -1});
        fireEvent(table1rows[0], new DragEvent('drag', {dataTransfer, clientX: 200, clientY: -1}));
        fireEvent(table1rows[0], new DragEvent('dragenter', {dataTransfer, clientX: 200, clientY: -1}));
        ...
        fireEvent.pointerUp(table1rows[0], {pointerType: 'mouse', button: 0, pointerId: 1, clientX: 200, clientY: -1});
      ...
      it.skip('should allow moving multiple rows into another table', async function () {
        ...
        act(() => userEvent.click(within(table1rows[0]).getByRole('checkbox')));
        act(() => userEvent.click(within(table1rows[2]).getByRole('checkbox')));
      ...
      it.skip('should automatically focus an item after if it has been dropped on', async function () {
        ...
        expect(within(rows[1]).getByRole('rowheader')).toHaveTextContent('One');
        expect(within(rows[2]).getByRole('rowheader')).toHaveTextContent('Two');
