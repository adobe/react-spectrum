...
storiesOf('TableView', module)
  ...
  .add(
    ...
    () => <FillWidths allowsResizing />
  )
  .add(
    'should fill cell actions',
    () => <FillWidths hasCustomActions />
  )
  .add(
...
function FillWidths({hasCustomActions = false, hasSection = false, allowsResizing = false}) {
  let menus = columns.map(column => {
    let menu = hasCustomActions ? (
      <Menu onAction={action(`${column} action`)}>
        <Item key="hide">Hide</Item>
  ...
  return (
    <TableView aria-label="TableView with filled cells" selectionMode="multiple" width={500} height={200} onSelectionChange={s => onSelectionChange([...s])}>
      <TableHeader>
        <Column allowsResizing={allowsResizing} menu={menus[0]}>File Name</Column>
        <Column allowsResizing={allowsResizing} menu={menus[1]} align="center">Type</Column>
        <Column allowsResizing={allowsResizing} menu={menus[2]} align="end">Size</Column>
        <Column allowsResizing={allowsResizing} menu={menus[3]}>Description</Column>
      </TableHeader>
      <TableBody>
        <Row>
          <Cell>2018 Proposal</Cell>
