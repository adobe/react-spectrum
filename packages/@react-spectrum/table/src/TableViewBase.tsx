...
import {
  ...
  FocusScope,
  useFocusRing,
} from "@react-aria-nutrient/focus";
import {
...
import {
  useVisuallyHidden,
  VisuallyHidden,
} from "@react-aria-nutrient/visually-hidden";
...
export const TableContext =
  React.createContext<TableContextValue<unknown> | null>(null);
export function useTableContext(): TableContextValue<unknown> {
  return useContext(TableContext)!;
}

export const VirtualizerContext = React.createContext<{
  width: number;
  key: Key | null;
} | null>(null);
...
function TableViewBase<T extends object>(
...
function TableVirtualizer<T>(props: TableVirtualizerProps<T>) {
  ...
  let resizerAtEdge =
    resizerPosition >
    Math.max(
      state.virtualizer.contentSize.width,
      state.virtualizer.visibleRect.width
    ) -
      3;
  // this should be fine, every movement of the resizer causes a rerender
  // scrolling can cause it to lag for a moment, but it's always updated
  let resizerInVisibleRegion =
    resizerPosition < state.virtualizer.visibleRect.maxX;
...
function TableColumnHeader(props) {
  ...
  useEffect(() => {
    if (
      ...
      console.warn(
        `Column key: ${column.key}. Columns with child columns don't allow resizing.`
      );
    }
    ...
    <FocusRing focusRingClass={classNames(styles, "focus-ring")}>
      ...
      >
        {columnProps.allowsSorting && (
          <ArrowDownSmall
            UNSAFE_className={classNames(styles, "spectrum-Table-sortedIcon")}
          />
        )}
...
let ForwardTableColumnHeaderButton = (
  props,
  ...
  let { buttonProps } = useButton(
    { ...otherProps, elementType: "div", isDisabled: isEmpty },
    domRef
  );
        ...
        <MenuTrigger onOpenChange={setHeaderMenuOpen} align={menuAlign}>
          ...
          >
            ...
            {columnProps.hideHeader ? (
              ...
              <div
                className={classNames(styles, "spectrum-Table-headerCellText")}
              >
                {column.rendered}
              </div>
            )}
    ...
    <FocusRing focusRingClass={classNames(styles, "focus-ring")}>
      ...
      >
        ...
        <Checkbox
          ...
          isEmphasized
          UNSAFE_style={
            isSingleSelectionMode ? { visibility: "hidden" } : undefined
          }
...
function DragButton() {
  let { dragButtonProps, dragButtonRef, isFocusVisibleWithin } =
    useTableRowContext();
...
function TableRow({
  ...
  if (isLastRow) {
    if (
      ...
      (layout.virtualizer?.visibleRect.height ?? 0)
    ) {
  ...
  return (
    <TableRowContext.Provider
      value={{ dragButtonProps, dragButtonRef, isFocusVisibleWithin }}
    >
...
function TableCell({ cell }) {
  ...
  if ("expandedKeys" in state) {
    isRowExpandable =
      ...
      state.keyMap.get(cell.parentKey)?.props?.children?.length >
    ...
    levelOffset =
      (cell.level - 2) * LEVEL_OFFSET_WIDTH[scale] +
  ...
  return (
    <FocusRing focusRingClass={classNames(styles, "focus-ring")}>
      <div
        ...
        ref={ref}
        style={
          isExpandableTable && isFirstRowHeaderCell
        ...
        className={classNames(
          ...
          "spectrum-Table-cell",
          {
            "spectrum-Table-cell--divider":
              columnProps.showDivider && cell.column.nextKey !== null,
            "spectrum-Table-cell--hideHeader": columnProps.hideHeader,
            "spectrum-Table-cell--hasExpandCollapseButton":
        ...
        () =>
          classNames(
            ...
            classNames(stylesOverrides, {
              ...
              "react-spectrum-Table-cellWrapper--dropTarget":
