...
By default, TableView doesn't allow expandable rows, but this can be enabled using the `hasExpandableRows` prop.
You'll first need to enable a feature flag in your app to use this feature.
...
Use `defaultExpandedKeys` to provide a default set of expanded rows. Note that the value of the expanded keys must match the `key` prop of the Row and that a
expanded child row will only be visible if its parent row key is also included within the expanded key set.

A expandable row can be statically defined by including one or more Rows as children. Note that all child Rows must be placed after the last Cell within the Row.

The example below enables expandable rows, and uses `defaultSelectedKeys` to expand rows with keys "1" and "2".

```tsx example
<TableView aria-label="Example table with expandable rows" hasExpandableRows defaultExpandedKeys={['1', '2']} height="size-4600">
  <TableHeader>
    <Column>Name</Column>
...
To programmatically control row expansion, use the `expandedKeys` prop paired with the `onExpandedChange` callback. The `key` prop from the expanded rows will
...
function TableExample(props) {
  ...
  let [expandedKeys, setExpandedKeys] = React.useState<'all' | Set<React.Key>>('all');

  return (
    <TableView aria-label="Example table with controlled expandable rows" hasExpandableRows expandedKeys={expandedKeys} onExpandedChange={setExpandedKeys} height="size-4600">
      <TableHeader columns={columns}>
        {column => (
