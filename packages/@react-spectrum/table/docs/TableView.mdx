...
Use `defaultExpandedKeys` to provide a default set of expanded rows. Note that the value of the expanded keys must match the `key` prop of the Row.
...
Here is how you would control expanded rows for the above example.
...
function TableExample(props) {
  ...
  let nestedRows = [
    {key: '1', name: 'United States', capital: 'Washington, D.C.', population: '331.9 million', childRows: [
      ...
      {key: '4', name: 'Texas', capital: 'Austin', population: '29.53 million'}
    ]}
  ];

  let [expandedKeys, setExpandedKeys] = React.useState<'all' | Set<Key>>('all');
  ...
  return (
    <TableView aria-label="Example table with controlled expandable rows" hasExpandableRows expandedKeys={expandedKeys} onExpandedChange={setExpandedKeys} height="size-3000">
      <TableHeader columns={columns}>
        {column => (
          <Column childColumns={column.children} align={column.key === 'population' ? 'end' : 'start'}>
            {column.name}
          </Column>
        )}
      </TableHeader>
