...
## Example
```tsx example
<Flex height="size-2000" width="size-6000">
...
## Content
Table expects a TableHeader and a TableBody as children. In turn, TableHeader and TableBody follow the [Collection Components](../react-stately/collections.html) API, accepting both static and dynamic collections.
...
which dramatically improves performance. Be sure to match each Column's key with an identical property in the Row object.
...
To internationalize a Table, all text content within the Table should be replaced with localized strings. This includes all `aria-labels` provided to the Table, Columns, Rows, and Cells.
...
### Selection
Table supports multiple selection modes. By default, selection is disabled, however this can be changed using the `selectionMode` prop.
...
The example below demonstrates how one would use `defaultSelectedKeys`.
...
<PropTable component={{
  props: {
    properties: Object.fromEntries(Object.entries(tableTypes.exports.SpectrumTableProps.properties))
  }
...
// TODO maybe don't need this, explain through examples?
...
// TODO maybe don't need this, explain through examples? Also might need to comment out some of the column types if they aren't supported yet
...
<Flex height="size-2000" width="size-3000">
...
function TableExample(props) {
  ...
  let rows = [
    ...
    {id: '5', firstName: 'Longgggg Wrapping', lastName: 'Name', age: '56'}
  ];
...
### Disabled
Use the `disabledKeys` prop to specify which rows to disable in the Table.

```tsx example
// Using same setup as hide header example
<TableExample disabledKeys={['2', '3']} />
```
...
<Flex height="size-6000" width="size-6000">
...
TODO: // Other column specific props like isRowHeader labeling, perhaps accessibility
