...
## Unavilalbe Menu Item <VersionBadge version="alpha" style={{marginLeft: 4, verticalAlign: 'bottom'}} />

ContextualHelpTrigger shows a user extra information about the state of an unavilalbe Menu Item.
It provides context for the state and may point users to more information elsewhere.

ContextualHelpTrigger serves as a wrapper around a Dialog and an Item which is its associated trigger,
linking the Dialog's open state with the trigger's press state. The ContextualHelpTrigger accepts exactly
...
ContextualHelp, the layout of the Dialog is very diliberate. See [ContextualHelp](ContextualHelp.html#content) for further explanation.

By default, a ContextualHelpTrigger's Dialog is opened by hovering or pressing the trigger element or activating
...
When a ContextualHelpTrigger is used to make Menu Items unavailabe, the Menu `onAction` and `onSelectionChange`
callbacks will not occur.

A ContextualHelpTrigger disables a Menu Item when you can set `isUnavailable` on the ContextualHelpTrigger.
