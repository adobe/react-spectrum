...
## Unavailable Menu Items  <VersionBadge version="alpha" style={{marginLeft: 4, verticalAlign: 'bottom'}} />

Sometimes, Menu Items need to be disabled, but still shown so people know that those options exist. For example, maybe a user needs to get additional permissions to perform an action.

```tsx example
import {MenuDialogTrigger} from '@react-spectrum/menu';
import {Dialog, Heading, Content, Footer, Link} from '@adobe/react-spectrum';
<MenuTrigger>
  <ActionButton>
    Edit
  </ActionButton>
