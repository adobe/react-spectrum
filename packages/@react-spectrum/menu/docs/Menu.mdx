...
To disable Menu items, pass an array of keys into the `disabledKeys` prop.
If an item has both `key` and `id` attributes, the `key` attribute should be used for disabling.
See the [Content](#events) section for more detail on how an item's `key` may be determined.
...
<MenuTrigger>
  ...
  <Menu
    items={[
      {name: 'tiff', key: 'tiff', id: 'a1b2c3'},
      {name: 'png', key: 'png', id: 'g5h1j9'},
      {name: 'jpg', key: 'jpg', id: 'p8k3i4'},
      {name: 'PDF', key: 'pdf', id: 'j7i3a0'}
    ]}
    disabledKeys={['tiff', 'jpg']}>
    {item => <Item>{item.name}</Item>}
  </Menu>
</MenuTrigger>
```
