...
export function MenuSection<T>(props: MenuSectionProps<T>): ReactNode {
  ...
  let lastSectionKey = [...state.collection]
    .filter((node) => node.type === "section")
    .at(-1)?.key;
  let sectionIsFirst =
    firstSectionKey === item.key &&
    state.collection.getFirstKey() === firstSectionKey;
  ...
  return (
    <Fragment>
      {item.key !== state.collection.getFirstKey() && (
        <div
          {...separatorProps}
          className={classNames(styles, "spectrum-Menu-divider")}
        />
      )}
      <div {...itemProps}>
        {item.rendered && (
          <span
            {...headingProps}
            className={classNames(styles, "spectrum-Menu-sectionHeading")}
          >
            {item.rendered}
          </span>
        )}
