 ...
 */
import {RenderProps, RuntimeStyleFunction} from './types';

// taken from: https://stackoverflow.com/questions/51603250/typescript-3-parameter-list-intersection-type/51604379#51604379
type ArgTypes<T> = T extends (props: infer V) => any ? NullToObject<V> : never;
type NullToObject<T> = T extends null | undefined ? {} : T;
type BoxedTupleTypes<T extends any[]> = {[P in keyof T]: [ArgTypes<T[P]>]}[Exclude<
  keyof T,
  keyof any[]
>];
type BoxedReturnTypes<T extends any[]> = {[P in keyof T]: [InferReturn<T[P]>]}[Exclude<
  keyof T,
  keyof any[]
>];
...
export function merge<R extends RenderProps<string> = never, X = {}>(
  ...args: Arg<NoInfer<X>, NoInfer<R>>[]
): RuntimeStyleFunction<X, R>;
export function merge<T extends Arg<any, any>[]>(
  ...args: T
): RuntimeStyleFunction<
  InferReturnType<T>,
  UnboxIntersection<UnionToIntersection<BoxedTupleTypes<T>>>
>;
