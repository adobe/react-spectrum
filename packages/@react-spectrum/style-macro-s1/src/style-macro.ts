...
export function createArbitraryProperty<T extends Value>(
  fn: (value: T, property: string) => CSSProperties
): PropertyFunction<T> {
  return (value, property) => {
    let selector = Array.isArray(value)
      ? value.map(v => generateArbitraryValueSelector(String(v))).join('')
      : generateArbitraryValueSelector(String(value));
    return {default: [fn(value, property), selector]};
  };
}

export function createMappedProperty<T extends CSSValue>(
  fn: (value: string, property: string) => CSSProperties,
  values: PropertyValueMap<T> | string[]
): PropertyFunction<T> {
  let valueMap = createValueLookup(
    Array.isArray(values)
      ? values
      : Object.values(values).flatMap((v: any) => (typeof v === 'object' ? Object.values(v) : [v]))
  );
...
export function createColorProperty<C extends string>(
  colors: PropertyValueMap<C>,
  property?: keyof CSSProperties
): PropertyFunction<Color<C>> {
  let valueMap = createValueLookup(
    Object.values(colors).flatMap((v: any) => (typeof v === 'object' ? Object.values(v) : [v]))
  );
        ...
        (value, priority, conditions) => {
          ...
          if (conditions.size > 0) {
            for (let condition of conditions) {
              selector +=
                themeConditionMap.get(condition) || generateArbitraryValueSelector(condition);
            }
          }
...
function printJS(rules: Rule[], indent = ''): string {
  ...
  let elseCases = rules
    .filter(rule => !rule.condition)
    .map(rule => printRuleChildren(rule, indent + '  '));
