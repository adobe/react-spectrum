...
export type PropertyFunction<T extends Value> = (
  value: T,
  property: string
) => PropertyValueDefinition<[CSSProperties, string]>;
...
export type RuntimeStyleFunction<S, R> =
  Keys<R> extends never ? () => string & S : (props: R) => string & S;
...
>(
  style: S
) => RuntimeStyleFunction<IncludedProperties<S>, InferProps<R, C | ArbitraryCondition, S>>;
...
export type CSSProp<S, P extends string | number> =
  S extends StyleFunction<infer T, any> ? string & LimitTheme<T, P> : never;
