...
<SearchField
  ...
  onSubmit={text => alert(`Search for ${text}`)} />
```
...
The example below uses `onChange`, `onSubmit`, and `onClear` to update a separate elements with the text entered into the SearchField.
```tsx example
function Example() {
