...
### Custom icon
The icon in the SearchField can be changed to suit the theme of a search. For instance, if the SearchField was for searching users users, an icon relating to that would help convey meaning. See the [Icons page](./Icon.html) for more.
```tsx example
