...
Accordion follows the [ARIA Accordion pattern](https://www.w3.org/WAI/ARIA/apg/patterns/accordion/) and accepts multiple [Disclosure](Disclosure.html) elements as children.

Each disclosure accepts an `id` prop which is passed to the `onExpandedChange` handler to identify the expanded disclosure within the accordion. See the [Events](#events) section for more details about expansion.
