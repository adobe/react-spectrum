...
ActionGroups accept as list of `children`, each with a `uniqueKey` prop. Generally `children` will be `Item` components from the `@react-stately/collections` library.
...
In order to identify children when a user takes an action on a item, each child needs to have a `uniqueKey` prop. If using a function, an `itemKey` prop is required on the ActionGroup.
