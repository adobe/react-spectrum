...
import {HeaderInfo, PropTable, PageDescription, ClassAPI, TypeContext, InterfaceType} from '@react-spectrum/docs';
...
<PageDescription>Toasts are transient notifications of actions, errors, or other events in an application.</PageDescription>

<HeaderInfo
  packageData={packageData}
  componentNames={['ToastProvider']}
  sourceData={[
...
<ToastProvider />
<Button
  onPress={() => ToastProvider.positive('Toast is done!')}
  variant="primary">
  Show toast
</Button>
```
...
Toasts are triggered using one of the methods of `ToastProvider`. A `<ToastProvider />` element must be rendered at the root of your app in order to display the queued toasts.
...
<ButtonGroup>
  <Button
    ///- begin highlight -///
    onPress={() => ToastProvider.neutral('Toast available')}
    ///- end highlight -///
    variant="secondary">
    Show Default Toast
  </Button>
  ...
  <Button
    ///- begin highlight -///
    onPress={() => ToastProvider.negative('Toast is burned!')}
    ///- end highlight -///
    variant="negative">
    Show Negative Toast
  </Button>
  <Button
    ///- begin highlight -///
    onPress={() => ToastProvider.info('Toasting…')}
    ///- end highlight -///
    variant="accent"
    style="outline">
    Show info Toast
...
```tsx example
<Button
  onPress={() => ToastProvider.info('An update is available', {
    ///- begin highlight -///
    actionLabel: 'Update',
...
<Button
  ///- begin highlight -///
  onPress={() => ToastProvider.positive('Toast is done!', {timeout: 5000})}
  ///- end highlight -///
  variant="primary">
  Show toast
</Button>
...
Toasts may be programmatically dismissed if they become irrelevant before the user manually closes them. Each method of `ToastProvider` returns a function which may be used to close a toast.
  ...
  return (
    <Button
      onPress={() => {
        if (!close) {
          ///- begin highlight -///
          let close = ToastProvider.negative('Unable to save', {onClose: () => setClose(null)});
          ///- end highlight -///
          setClose(() => close);
        } else {
          ///- begin highlight -///
...
<TypeContext.Provider value={docs.links}>
  <InterfaceType properties={docs.exports.ToastProvider.props.properties} showRequired showDefault isComponent />
</TypeContext.Provider>
