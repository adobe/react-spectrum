...
import {addComment, addComponentImport} from './utils';
...
function updatePropNameAndValue(
  ...
  path.traverse({
    JSXAttribute(path) {
      if (t.isJSXAttribute(path.node) && path.node.name.name === oldProp) {
        if (
          ...
          if (typeof newValue === 'string') {
            ...
            if (!newValue) {
              path.node.value = t.jsxExpressionContainer(t.booleanLiteral(newValue));
            } else {
          ...
          if (t.isIdentifier(path.node.value.expression)) {
            // @ts-ignore
            if (path.node.comments && [...path.node.commments].some((comment) => comment.value.includes('could not be automatically'))) {
              return;
            }
            addComment(path.node, ` TODO(S2-upgrade): Prop ${oldProp} could not be automatically updated because ${path.node.value.expression.name} could not be followed.`);
          } else {
...
function removeProp(path: NodePath<t.JSXElement>, options: RemovePropOptions) {
  const {propToRemove} = options;

  path.traverse({
    JSXAttribute(path) {
      if (t.isJSXAttribute(path.node) && path.node.name.name === propToRemove) {
        if (options.propValue) {
          // If prop value is provided, remove prop only if it matches the value
          if (t.isStringLiteral(path.node.value) && path.node.value.value === options.propValue) {
            path.remove();
          } else if (
            ...
            if (t.isIdentifier(path.node.value.expression)) {
              ...
              addComment(path.node, ` TODO(S2-upgrade): Prop ${propToRemove} could not be automatically removed because ${path.node.value.expression.name} could not be followed.`);
            } else {
              path.traverse({
                StringLiteral(stringPath) {
                  if (
                    t.isStringLiteral(stringPath.node) &&
                    stringPath.node.value === options.propValue
                  ) {
                    // Invalid prop value was found inside expression.
                    // TODO: We should leave a comment instead of removing the prop entirely
                  }
                }
              });
            }
...
function commentOutProp(
  ...
  const {propToComment} = options;

  path.traverse({
    JSXAttribute(path) {
      if (t.isJSXAttribute(path.node) && path.node.name.name === propToComment) {
        addComment(path.parentPath.node, ` TODO(S2-upgrade): ${propToComment} has not been implemented yet.`);
        path.remove();
      }
    }
  });
...
function updateComponentIfPropPresent(
  ...
  path.traverse({
    JSXAttribute(path) {
      if (t.isJSXAttribute(path.node) && path.node.name.name === propToCheck) {
        let node = path.findParent((p) => t.isJSXElement(p.node))?.node;
...
function moveRenderPropsToChild(
  ...
  if (
    ...
    renderFunction.expression.body.openingElement.name.name !== newChildComponent) {
  ...
  if (
    ...
    path.node.children[renderFunctionIndex] = t.jsxElement(
      t.jsxOpeningElement(t.jsxIdentifier(newChildComponent), dialogElement.openingElement.attributes),
...
function updateComponentWithinCollection(
  ...
  if (
    ...
    let closestParentCollection = path.findParent((p) =>  t.isJSXElement(p.node) && t.isJSXIdentifier(p.node.openingElement.name) && collectionItemParents.has(p.node.openingElement.name.name));
    if (
      ...
      closestParentCollection.node.openingElement.name.name === parentComponent
    ) {
  ...
  path.traverse({
    JSXElement(path) {
      if (
        t.isJSXElement(path.node) &&
        t.isJSXIdentifier(path.node.openingElement.name) &&
        path.node.openingElement.name.name === parentComponent
      ) {
...
function movePropToNewChildComponent(
  ...
  if (
    ...
    path.node.openingElement.name.name === childComponent &&
    path.parentPath.node.openingElement.name.name === parentComponent
  ) {
    let propValue;
...
function convertDimensionValueToPx(
  ...
  path.traverse({
    JSXAttribute(path) {
      if (t.isJSXAttribute(path.node) && path.node.name.name === propToConvertValue) {
        if (t.isStringLiteral(path.node.value)) {
          try {
            let value = convertDimension(path.node.value.value);
            if (value && typeof value === 'number') {
              path.node.value = t.jsxExpressionContainer(t.numericLiteral(value));
            } else if (value && typeof value === 'string') {
              // eslint-disable-next-line max-depth
              if ((/%|vw|vh|auto|ex|ch|rem|vmin|vmax/).test(value)) {
                addComment(path.node, ' TODO(S2-upgrade): Unable to convert CSS unit to a pixel value');
                ...
                if (!isNaN(value)) {
                  ...
                  path.node.value = t.jsxExpressionContainer(t.numericLiteral(pixelValue));
                }
            ...
            addComment(path.node, ` TODO(S2-upgrade): Prop ${propToConvertValue} could not be automatically updated due to error: ${error}`);
          }
        } else if (t.isJSXExpressionContainer(path.node.value)) {
          if (t.isIdentifier(path.node.value.expression)) {
            addComment(path.node, ` TODO(S2-upgrade): Prop ${propToConvertValue} could not be automatically updated because ${path.node.value.expression.name} could not be followed.`);
          }
        }
      }
    }
...
function updatePlacementToSingleValue(
  ...
  path.traverse({
    JSXAttribute(path) {
      if (t.isJSXAttribute(path.node) && path.node.name.name === propToUpdate) {
        if (t.isStringLiteral(path.node.value) && doublePlacementValues.has(path.node.value.value)) {
          path.replaceWith(t.jsxAttribute(t.jsxIdentifier(propToUpdate), t.stringLiteral(path.node.value.value.split(' ')[0])));
          return;
        } else if (t.isJSXExpressionContainer(path.node.value)) {
          path.traverse({
            StringLiteral(stringPath) {
