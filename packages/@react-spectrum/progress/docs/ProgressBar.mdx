...
Progress Bar requires a `label`, `aria-label`, or `aria-labelledby` attribute depending on the visualization being used (ie depending on the `showValueLabel` prop).
