...
Use an indeterminate ProgressBar when thhe wait time or effort to completion can not be determined.
...
To internationalize a Progress Bar, a localized string should be passed to the `label` prop, `aria-label` prop, or value of the associated `aria-labelledby` element.
