...
import Anatomy from './CardAnatomy.svg';
...
If you need to perform actions on a card, please use the [ActionBar component](./ActionBar.html) in lieu of adding an action menu to the Card itself. See the [Card Actions](#card-actions) section for an example.
...
the aspect ratio of each card's image. The customizable options specific to the WaterfallLayout are as follows:
...
function Folder(props) {
  ...
  return (
    <div className={classNames(assetStyles, styleProps.className)}>
      <svg
        viewBox="0 0 32 32"
        className={classNames(assetStyles, 'spectrum-Asset-folder')}
        aria-label={props.alt}
        aria-hidden={props.decorative || null}
        role="img">
        <path
          className={classNames(assetStyles, 'spectrum-Asset-folderBackground')}
        ...
        <path
          className={classNames(assetStyles, 'spectrum-Asset-folderOutline')}
...
function File(props) {
  ...
  let {styleProps} = useStyleProps(props);
  return (
    <div {...styleProps} className={classNames(assetStyles, styleProps.className)}>
      <svg
        viewBox="0 0 128 128"
...
function CardViewDisabledKeys() {
  let items = [
    {id: 1, name: 'Vacation ideas', type: 'file', size: '100 KB', format: 'DOCX'},
    {id: 2, name: 'Saved Photos 2022', type: 'folder', size: '1.3 GB'},
    // ...
    ///- begin collapse -///
    {id: 3, name: 'Stock market predictions 2022', type: 'file', size: '1.2 MB', format: 'PDF'},
    {id: 4, name: 'My Games', type: 'folder', size: '803 GB'},
    {id: 5, name: 'README', type: 'file', size: '3 KB', format: 'TXT'},
    {id: 6, name: 'TODOs week 1/15', type: 'file', size: '12 KB', format: 'TXT'}
    ///- end collapse -///
  ];

  return (
    <CardView items={items} height="500px" width="100%" layout={GridLayout} aria-label="Disabled selection CardView example" selectionMode="multiple" disabledKeys={[2, 4]}>
      {item => (
        <Card textValue={item.name} >
          {item.type === 'folder' ? <Folder alt="folder" slot="illustration" /> : <File alt="file" slot="illustration" />}
          <Heading>{item.name}</Heading>
...
### Card actions
A common use case is to allow a user to perform actions on one or more cards. The example below illustrates how one could use [ActionBar](./ActionBar.html) to trigger an action on the
