...
function DynamicCardView(props: SpectrumCardViewProps<object>) {
  ...
  let removeItem = () => {
    ...
    let newItems = items
      .slice(0, val)
      .concat(items.slice(val + 1, items.length));
    setItems(newItems);
  };
...
export type ControlledCardViewStory = ComponentStoryObj<
  typeof ControlledCardView
>;
...
function NoItemCardView(props: SpectrumCardViewProps<object>) {
  ...
  return (
    <>
      <ActionButton onPress={() => setShow((show) => !show)}>
        Toggle items
      </ActionButton>
      <CardView
        {...props}
...
export type AsyncLoadingCardViewStory = ComponentStoryObj<
  typeof AsyncLoadingCardView
>;
...
export function CustomLayout(
  props: SpectrumCardViewProps<object> & LayoutOptions
) {
  let { scale } = useProvider();
  let collator = useCollator({ usage: "search", sensitivity: "base" });
