...
As with most coding problems, we start with how do we want our API to look.
```snippet
<Dialog>
...
```tsx snippet
function Dialog(props) {
  return props.children({title: {id: props.id, className: props.classname}, body: {} /* body props ...etc */});
}
```
