...
as long as the input is empty. In Chrome, VoiceOver only announces changes to `aria-activedescendant` if `aria-selected="true"` is applied to the focused item. If your list of options is organized into separate sections, VoiceOver doesn't announce at changes to the focused item
for all browsers. In Firefox, VoiceOver only announces the first item you focused and well you get the picture.

Item focus wasn't the only
// List some other issues such as number options available aren't announced, section titles aren't announced, selection isn't announced

As a workaround, we made use of [ARIA live-regions](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/ARIA_Live_Regions). By setting up a visually hidden region in the DOM that has two `div`s with `aria-live="polite"` and `aria-live="assertive"`
we are able to have the screen reader announce custom messages by updating the contents of said `div`s when updates occur in the ComboBox.

to broadcast when a new ComboBox menu item is focused. It also announces when focus has been moved into a new menu section, when a item is selected,
