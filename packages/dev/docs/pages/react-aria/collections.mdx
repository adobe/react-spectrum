...
The `dependencies` prop can be used to invalidate the dynamic collection's item cache in response to other state changes.
// TODO come up with a useful time for this that isn't our Table example again...
