...
[Remix actions](https://remix.run/docs/en/main/route/action) handle form submissions on the server. The [useActionData](https://remix.run/docs/en/main/hooks/use-action-data) hook can be used to get the value returned by the server, which may include validation errors. Since Remix has its own [Form](https://remix.run/docs/en/main/components/form) component which must be used to submit data, use `FormValidationContext` from React Aria directly to provide validation errors instead of `Form`.

```tsx
// app/routes/signup.tsx
import type {ActionArgs} from '@remix-run/node';
import {Form, useActionData} from '@remix-run/react';
import {FormValidationContext, TextField, Label, Input, FieldError, Button} from 'react-aria-components';
...
export default function SignupForm() {
  ...
  return (
    <Form method="post">
      {/*- begin highlight -*/}
      <FormValidationContext.Provider value={actionData?.errors}>
      {/*- end highlight -*/}
        <TextField name="username" isRequired>
          <Label>Username</Label>
          <Input />
