...
with the relevant `AXE` rule set as its value. The example below demonstrates how you could do so for your [automated Storybook test runner](https://storybook.js.org/docs/7.1/react/writing-tests/accessibility-testing#a11y-config-with-the-test-runner).
A similar config can be setup for the [Storybook a11y addon](https://storybook.js.org/docs/7.1/react/writing-tests/accessibility-testing#global-a11y-configuration) as well.
...
module.exports = {
  ...
  async postRender(page, context) {
    ...
    // Apply global a11y rules on top of story-level a11y rules
