...
with builtin semantics (e.g. `<button>` or `<select>`) for styling reasons, or if there isn't a native element available,
...
will automatically associate the visible label with the control to so that assistive technology can describe it properly.

In case a visible label is not desired for some reason, or you're using a control that doesn't have a builtin label,
...
takes over the whole screen, and normal touch interactions for your application will not be available.
...
can press the <kbd>Escape</kbd> key. On desktop, this may be enough for screen reader users, but on mobile there is no
