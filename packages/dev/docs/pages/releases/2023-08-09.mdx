...
In today's release, the `InlineAlert` component is going to GA! Explore the options in the [React Spectrum InlineAlert docs](../InlineAlert.html). Our React Aria Components have new additions including [TextArea](react-aria:TextField#textarea), [FileTrigger](react-aria:FileTrigger), and [DropZone](react-aria:DropZone). Our date/time components have also had some usability updates,  `usePress` now supports an option to [continue propagation](react-aria:usePress), and we improved support for HTML forms across our components.

Thank you to our community for their support and contributions!

## New components
- [InlineAlert](../InlineAlert.html)
