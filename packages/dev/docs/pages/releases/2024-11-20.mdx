...
We’re excited to announce that our Accordion and Disclosure components are now generally available! This includes [Disclosure](react-aria:Disclosure) and [DisclosureGroup](react-aria:DisclosureGroup) in React Aria, as well as [Disclosure](../Disclosure.html) and [Accordion](../Accordion.html) in React Spectrum. These components now also use the `hidden="until-found"` API currently supported in Chrome to enable searching collapsed content with ctrl + F.

We are also introducing [ToggleButtonGroup](react-aria:ToggleButtonGroup) in React Aria Components, which supports arrow key navigation with single and multiple selection modes. In addition, Menu now supports the ability to create [Section-level selection groups](react-aria:Menu#section-level-selection), allowing you to mix actions, single, and multiple selection within the same menu. Menus can also be nested inside Dialog popovers to enable more complex use cases.
...
If you're using an older pre-release version of Accordion, see the [previous release notes](2024-09-30.html#accordion-updates) for instructions on how to migrate to the released API.
