...
Drop everything! Drag and drop support in React Aria Tree has been released! 🫳🎤 This supports moving items within a tree or between collection components, with full keyboard and screen reader accessibility. It's interoperable with our existing drag and drop support in components like Table and GridList, and with third party apps via native HTML APIs. Check out our [previous blog post](react-aria:blog/drag-and-drop) on drag and drop to learn about the interactions, and our new [Tree drag and drop documentation](react-aria:Tree#drag-and-drop).
