...
React Spectrum components are designed to be **consistent** across all Adobe applications. They include builtin
...
and positioning options. These options only affect things external to the component, not internal styles.
For example, margins can be set on all components since it is external to the component, but not padding,
which is internal to the the component (and included in the Spectrum design). Additionally, options like
colors, borders, text styles, etc. are not available to override.

All of these options are available as props on each React Spectrum component. The following example shows 
    ...
    <Example>
      ...
      <Provider scale={selectedKey}>
        <ul style={{listStyle: 'none', padding: 0, overflow: 'auto'}}>
          {console.log(dnaDocs.links[dnaDocs.exports.DimensionValue.id])}
...
using the `View` component from React Spectrum. `View` is like a `div` or `span`, but with additional style properties that
map to Spectrum defined variables in addition to the ones covered above.
...
While we encourage teams to utilize Spectrum design as it is, we do realize that sometimes it is too limited and some
product specific customizations may be needed. In these cases, we encourage you or your designers to **talk to us**.
...
escape hatches you can **use at your own risk**. These are `UNSAFE_className` and `UNSAFE_style`. Use of these props
...
The reasoning behind this is that `className` and `style` are too flexible. The goal of Spectrum is to unify the 
design of Adobe products,  and ensure consistency. Full access to the internal CSS implementation details allows 
too much flexibility to change the Spectrum design without taking changes through the proper channels.

In addition, future updates to Spectrum design may cause unintended breaking changes in products if the internal 
DOM structure or CSS properties of a React Spectrum component change, leading to conflicts with CSS overrides in 
products. For this reason, these properties are unsafe, and if you use them know that you are doing so at your own
risk.
