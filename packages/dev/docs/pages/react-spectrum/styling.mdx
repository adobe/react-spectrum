...
recommend using it for new projects. However, v5 is still the default for backward compatibility. You can
opt into v6 by setting the `colorVersion` prop on the [View](View.html) component to `6`. A [migration guide](https://spectrum.adobe.com/static/misc/Colors_v6.0_Migration-Guide.xd)
is also available on the Spectrum website.
