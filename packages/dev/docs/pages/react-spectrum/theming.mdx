...
consist of definitions for these variables for light and dark [color schemes](https://spectrum.adobe.com/page/color/#Color-themes-and-modes),
...
automatically (e.g. based on time of day), React Spectrum applications will update accordingly.

We recommend that all React Spectrum applications support both light and dark mode. It can be very jarring to 
use a light themed application when the rest of your applications are in dark mode, and visa versa. This is
generally quite simple to support if you use Spectrum color variables rather than hard coded color values.
...
There is a medium scale for devices with fine grained pointers (e.g. mice), and a large scale for devices with courser
...
use controls that are too large on desktop. That said, if you have some usecase that requires it, you can explicitly
...
[source code](https://github.com/adobe-private/react-spectrum-v3/tree/master/packages/%40adobe/spectrum-css-temp/vars)
