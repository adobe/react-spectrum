...
Thanks for choosing to contribute! We look forward to improving web applications together. Here you will find information on how propose bug fixes, suggest improvements and how to build react-spectrum yourself.

## Better together
We believe in sharing our process, results and journey with you all. The core team and all external contributors follow the same process in order maintain a high quality codebase.

## Code of conduct
We adhere to the Adobe [code of conduct](https://github.com/adobe-private/react-spectrum-v3/blob/CODE_OF_CONDUCT.md) and by participating, you are expected to uphold this code. Please report unacceptable behavior to Grp-opensourceoffice@adobe.com.

## Reporting issues
### Bugs
We use [GitHub issues](https://github.com/adobe-private/react-spectrum-v3/issues) to track work and log bugs. Please check existing issues before filing anything new. We do our best to respond to issues within a few days. If you would like to contribute a fix, please let us know on the issue.

The best way to reduce back and forth on a bug is to add code for a storybook example exhibiting the issue along with steps to reproduce it. If you would like to work on a bugfix yourself, make sure an issue exists first.

### Feature requests
Our components can always be improved upon. If you have a feature request, you can use our Feature Request issue template in issues. For larger scopes of work, it is a good idea to put forth a Request For Comments (RFC) first to gather feedback from the team. A [template](https://github.com/adobe-private/react-spectrum-v3/blob/master/rfcs/template.md) is available to build an RFC. Make a PR to add your RFC to the [rfcs folder](https://github.com/adobe-private/react-spectrum-v3/tree/master/rfcs) to give the team and the community a chance to discuss the proposal.

### Security issues
Security issues shouldn't be reported on this issue tracker. Instead, [file an issue](https://helpx.adobe.com/security/alertus.html) to our security experts.
...
```
git clone https://github.com/YOUR-USERNAME/react-spectrum-v3
cd react-spectrum-v3
yarn install
```
...
### Tests
We use [jest](https://jestjs.io/) for unit tests and [react-testing-library](https://testing-library.com/docs/react-testing-library/intro) for rendering and writing assertions. Please make sure you include tests with your pull requests. Our CI/CD will run the tests on PRs as well as lint code and TS. You can see on each PR whether you have passed all our checks. We split the tests into 2 groups.
...
### Storybook
We use [Storybooks](https://storybooks.js.org) for local development. You can use the command
```bash
yarn start
```
followed by opening [http://localhost:9003](http://localhost:9003) in your browser to play around with the components and test your changes.
...
Documentaton can be run locally by using
```bash
yarn start:docs
```
followed by opening [http://localhost:1234](http://localhost:1234) in your browser.
