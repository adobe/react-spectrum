...
function TypeOperator({operator, value}) {
  return (
    <span>
      <span className="token hljs-literal">{operator}</span> <Type type={value} />
    </span>
  );
}
...
function Keyword({type}) {
  ...
  if (link) {
    return (
      <a
        href={link}
        className={`${styles.colorLink} token hljs-keyword`}
...
export function JoinList({elements, joiner, minIndent = 2, newlineBefore, neverIndent}) {
  ...
  return elements
    ...
    .reduce(
      (acc, v, i) => [
        ...acc,
        <span className="token punctuation" key={`join${v.name || v.raw}${i}`}>
          {contents}
        </span>,
        <IndentContext.Provider
  ...
  return (
    <>
      ...
      {constraint && (
        <>
          {' '}
          <span className="token hljs-keyword">extends</span> <Type type={constraint} />
        </>
      )}
      {defaultType && (
        <>
    ...
    <section key={type.id} id={type.id} data-title={type.name} hidden>
      {type.description && (
        <Markdown
          ...
          className={styles['type-description']}>
          {type.description}
        </Markdown>
      )}
      {type.type === 'interface' && type.extends?.length > 0 && (
        <p style={{paddingLeft: 'var(--spectrum-global-dimension-size-200)'}}>
...
function SpectrumLink({href, children, title}) {
  return (
    <a
      className={clsx(linkStyle['spectrum-Link--secondary'], styles.link)}
      href={href}
      title={title}
      {...getAnchorProps(href)}>
      {children}
...
export function renderHTMLfromMarkdown(description, opts) {
  if (description) {
    const options = {
      forceInline: true,
      overrides: {a: {component: SpectrumLink}},
...
export function InterfaceType({
  ...
  return (
    <>
          ...
          <thead>
            <tr>
              ...
              {!hideType && (
                <td
                  role="columnheader"
                  className={tableStyles['spectrum-Table-headCell']}
                  style={{width: '30%'}}>
                  Type
                </td>
              )}
            ...
            {properties.map((prop, index) => (
              <tr key={index} className={clsx(tableStyles['spectrum-Table-row'], styles.tableRow)}>
                <td
                  ...
                  data-column="Name">
                  <code className={`${typographyStyles['spectrum-Code4']}`}>
                    <span className={`token ${isComponent ? 'hljs-attr' : 'hljs-variable'}`}>
                      {prop.name}
                    </span>
                  </code>
      ...
      {methods.length > 0 && (
        <table
          className={`${tableStyles['spectrum-Table']} ${styles.propTable} ${styles.methodTable}`}>
          <thead>
            <tr>
              <td role="columnheader" className={tableStyles['spectrum-Table-headCell']}>
                Method
...
function ConditionalType({checkType, extendsType, trueType, falseType}) {
  return (
    <>
      <Type type={checkType} /> <span className="token hljs-keyword">extends</span>{' '}
      <Type type={extendsType} />
      <span className="token punctuation">{' ? '}</span>
      <Type type={trueType} />
      <span className="token punctuation">
        {' :' + (falseType.type === 'conditional' ? '\n' : ' ')}
      </span>
      <Type type={falseType} />
    </>
  );
    ...
    <>
      ...
      {elements.map((element, i) => {
        if (element.type === 'string' && element.value) {
          return (
            <span className="token hljs-string" key={i}>
              {element.value}
            </span>
          );
        }
