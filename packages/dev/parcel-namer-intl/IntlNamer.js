...
module.exports = new Namer({
  name({bundle}) {
    ...
    if (!bundle.needsStableName && bundle.type === 'js' && bundle.bundleBehavior !== 'inline') {
      ...
      let bundleName = path.basename(mainAsset.filePath, path.extname(mainAsset.filePath)).replace(/\*/g, 'intlStrings');
      if (mainAsset.filePath.includes('@adobe/spectrum-css-temp')) {
        bundleName = mainAsset.filePath.split(path.sep).at(-2) + '_' + bundleName;
      }
      if (path.extname(mainAsset.filePath) === '.css') {
        bundleName += '_css';
      }
      bundleName += '.' + bundle.target.name + '.js';
      return bundleName;
    }
    return null;
  }
