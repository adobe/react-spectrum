...
//
// Why this matters: without externalization, Parcel's CJS dev bundle pulls
// `storybook/preview-api` and `storybook/internal/csf` into a circular
// dependency where csf reads `addons` at module top-level before
// preview-api has finished evaluating, crashing Storybook init with
// `TypeError: Cannot read properties of undefined (reading 'addons')`.
// Routing every preview-runtime specifier to a leaf module that only reads
// from globalThis breaks the cycle.
...
// Two strategies for the synthetic external module:
//   1. "named-reexport" (preferred): enumerate named exports via require() at
//      codegen, then emit
//        const { x, y } = globalThis.__STORYBOOK_MODULE_*__;
//        export { x, y };
//   2. "cjs-interop" (fallback): used only when require() can't enumerate the
//      source module (e.g. it's ESM-only with no enumerable own properties).
//      Emits `module.exports = globalThis.__STORYBOOK_MODULE_*__;`.

const _require = createRequire(__filename);

interface ExternalEntry {
  filePath: string;
  approach: 'named-reexport' | 'cjs-interop';
}
...
function buildExternalsCache(): Record<string, ExternalEntry> {
  ...
  for (const [specifier, globalName] of Object.entries(globalsNameReferenceMap)) {
    ...
    if (specifier === '@storybook/global') {
      ...
      result[specifier] = { filePath, approach: 'named-reexport' };
      continue;
    }

    // Try named-reexport: enumerate exports via require() and codegen
    // `const { ... } = globalThis.X; export { ... };`.
    ...
    if (namedExports && namedExports.length > 0) {
      ...
      // Fall back to cjs-interop when require() couldn't enumerate exports
      // (e.g. an ESM-only module with no enumerable own properties).
      const code =
        `// Generated by @parcel/resolver-storybook (cjs-interop fallback)\n` +
        `// Could not enumerate named exports for ${specifier}; using CJS interop.\n` +
        `module.exports = globalThis.${globalName};\n`;
      fs.writeFileSync(filePath, code);
      result[specifier] = { filePath, approach: 'cjs-interop' };
    }
  }
