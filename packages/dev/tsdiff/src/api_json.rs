...
///
/// `remote = "Self"` makes the derive emit an inherent `TypeNode::deserialize`
/// instead of the `Deserialize` impl, so the hand-written impl below can run
/// first and normalize tag-less nodes (see that impl for why).
...
pub enum TypeNode {
    ...
    Method {
        /// Optional because a computed member key (e.g. `[Symbol.iterator]()`)
        /// is emitted by the legacy extractor as a method node with no `name`.
        #[serde(default)]
        name: String,
        value: Box<TypeNode>,
        #[serde(default)]
...
impl<'de> Deserialize<'de> for TypeNode {
    /// Tolerate type-nodes that carry no `type` tag at all.
    ///
    /// `#[serde(other)]` only covers an *unrecognized* tag value; a node with
    /// no `type` field is a hard error for an internally-tagged enum, and
    /// because `ApiJson::load` propagates it that single node aborts the whole
    /// comparison rather than degrading one export.
    ///
    /// The legacy parcel-transformer-docs extractor really does emit these:
    /// `@react-stately/datepicker` exports `SegmentType` as a bare
    /// `{"name": "SegmentType"}` (an unresolved node that never got its type
    /// filled in). Treat that the same way as an unknown tag.
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        let value = serde_json::Value::deserialize(deserializer)?;
        if let serde_json::Value::Object(map) = &value {
            if !map.contains_key("type") {
                return Ok(TypeNode::Unknown);
            }
        }
        // Inherent method generated by `remote = "Self"`, not this impl.
        TypeNode::deserialize(value).map_err(serde::de::Error::custom)
    }
}
...
impl ApiJson {
    pub fn load(path: &std::path::Path) -> anyhow::Result<Self> {
        ...
        let mut json: Self = serde_json::from_str(&data)
            .with_context(|| format!("parsing {}", path.display()))?;
        // Applied to both sides of a comparison so the two stay symmetric.
        // It is a no-op for API JSON that already carries `component` nodes.
        crate::component_normalizer::normalize_components(&mut json);
        Ok(json)
    }
}
...
mod tests {
    ...
    /// A node with no `type` tag must degrade to `Unknown` rather than failing
    /// the whole file. The legacy extractor emits these (e.g.
    /// `@react-stately/datepicker`'s `SegmentType` is a bare `{"name": ...}`).
    #[test]
    fn test_missing_type_tag_degrades_to_unknown() {
        assert!(matches!(parse_node(r#"{"name":"SegmentType"}"#), TypeNode::Unknown));
        assert!(matches!(parse_node("{}"), TypeNode::Unknown));
    }

    /// A tag-less node nested inside a real file must not abort the parse of
    /// its siblings.
    #[test]
    fn test_missing_type_tag_does_not_abort_sibling_exports() {
        let api: ApiJson = serde_json::from_str(
            r#"{"exports":{
                 "SegmentType":{"name":"SegmentType"},
                 "Good":{"type":"interface","name":"Good","properties":{}}
               }}"#,
        )
        .expect("tag-less export should not fail the file");
        assert_eq!(api.exports.len(), 2);
        assert!(matches!(api.exports["SegmentType"], TypeNode::Unknown));
        assert!(matches!(api.exports["Good"], TypeNode::Interface { .. }));
    }

    /// Computed member keys (`[Symbol.iterator]()`) reach us as method nodes
    /// with no `name`; they must not fail the file.
    #[test]
    fn test_method_without_name_parses() {
        let node = parse_node(r#"{"type":"method","value":{"type":"function"},"static":false}"#);
        assert!(matches!(node, TypeNode::Method { ref name, .. } if name.is_empty()));
    }
