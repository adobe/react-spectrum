...
- Use `LabeledValue` to display a read-only labeled value (string, number, date, time, range, or list of strings) — prefer it over a disabled `TextField` or hand-rolled label-plus-text markup. It applies locale-aware formatting to numbers, dates, and times via `formatOptions`.
