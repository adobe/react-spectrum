...
Use the `value` or `defaultValue` prop to set the time value, using objects in the <Link href={`${getBaseUrl('react-aria')}/internationalized/date/`}>@internationalized/date</Link> package. `TimeField` accepts plain <Link href={`${getBaseUrl('react-aria')}/internationalized/date/Time`}>Time</Link>, <Link href={`${getBaseUrl('react-aria')}/internationalized/date/CalendarDateTime`}>CalendarDateTime</Link>, or <Link href={`${getBaseUrl('react-aria')}/internationalized/date/ZonedDateTime`}>ZonedDateTime</Link>, but only displays the time.
