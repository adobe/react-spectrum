...
Use `AIButton` to trigger an AI-powered action, such as opening a prompt field or starting a chat.

```tsx render type="s2" docs={docs.exports.AIButton} props={['size']}
"use client";
...
function Example(props) {
  return (
    <div style={{display: 'flex', gap: 8}}>
      <AIButton {...props}>Ask AI</AIButton>
      <AIButton {...props} aria-label="Ask AI" />
    </div>
  );
}
```
...
```tsx links={{AIButton: '#aibutton', Chat: '#chat', Thread: '#thread', ThreadItem: '#threaditem', ThreadScrollButton: '#threadscrollbutton', PromptField: '#promptfield'}}
<AIButton />
<Chat>
  <Thread>
    <ThreadItem />
...
<PropTable component={docs.exports.AIButton} links={docs.links} showDescription />
