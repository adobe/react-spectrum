        ...
        {message => {
          switch (message.type) {
            ...
            case 'alert':
              return (
                <ThreadItem textValue={message.text}>
                  <Alert variant={message.variant} onDismiss={() => dismissMessage(message.id)} >
                    {message.text}
                  </Alert>
                </ThreadItem>
              )
...
```tsx render type="s2" docs={docs.exports.PixelLoader} props={['size', 'isPlaying']} initialProps={{isPlaying: true}}
"use client";
import {PixelLoader} from '@react-spectrum/ai/loader';

function PixelLoaderExample(props) {
  return <PixelLoader {...props} />;
}
```
...
```tsx render type="s2"
"use client";
import {analyze, cc, cxo, dc, exp, mega, PixelLoader} from '@react-spectrum/ai/loader';
import {Picker, PickerItem} from '@react-spectrum/s2/Picker';
import {useState} from 'react';

let presets = {cc, dc, exp, cxo, analyze, mega};

function PresetLoader() {
  let [preset, setPreset] = useState('cxo');
  ...
  return (
    <div style={{display: 'flex', flexDirection: 'column', gap: 16, alignItems: 'center'}}>
      <Picker
        label="Animation"
        selectedKey={preset}
        onSelectionChange={key => setPreset(key as string)}>
        <PickerItem id="cc">Creative Cloud</PickerItem>
        <PickerItem id="dc">Document Cloud</PickerItem>
        <PickerItem id="exp">Experience Cloud</PickerItem>
        <PickerItem id="cxo">CXO</PickerItem>
        <PickerItem id="analyze">Analyze</PickerItem>
        <PickerItem id="mega">Mega</PickerItem>
      </Picker>
      {/*- begin highlight -*/}
      <PixelLoader key={preset} icon={presets[preset]} size={32} />
      {/*- end highlight -*/}
    </div>
  );
}
...
```tsx render type="s2"
"use client";
import {dc} from '@react-spectrum/ai/loader';
import {
  PromptField,
  PromptFieldSubmitButton,
  PromptFieldToolbar,
...
function GeneratingPrompt() {
  ...
  return (
    <PromptField
      ...
      {/*- begin highlight -*/}
      <PromptTokenField placeholder="Ask a question" pixelLoader={dc} shouldAnimatePixelLoader />
      {/*- end highlight -*/}
      <PromptFieldToolbar>
        <div style={{marginInlineStart: 'auto'}}>
          <PromptFieldSubmitButton />
