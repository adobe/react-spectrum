...
* [Tags](?path=/docs/tags--docs): Fix Tag collapse calculation for removeable tags
...
If you previously used `page.css` without a `Provider`, you'll need to add a `Provider` around your app to load the fonts. See the [updated docs](getting-started.html#setting-up-your-app) for details.
...
To fix these Safari issues, we have removed `all: revert-layer` in this release. This means that global CSS will now take precedence over S2's styles. **If you are using a CSS reset on the same page as S2 components, you will need to remove it** or put it in a `@layer` of its own. See [the docs](styling.html#css-resets) for more information.
