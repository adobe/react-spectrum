...
function Example() {
  ...
  return (
    <div className={style({width: 'full', display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'center'})}>
      <MenuTrigger>
  ...
  return (
    <div>
      <TreeView
        ...
        onSelectionChange={setSelectedKeys}
        ///- end highlight -///
        styles={style({height: 250, width: 360})}
      >
...
```tsx render type="s2"
"use client";
import {TableView, TableHeader, Column, TableBody, Row, Cell, type Selection, type Key} from '@react-spectrum/s2';
import {style} from '@react-spectrum/s2/style' with {type: 'macro'};
import {useState} from 'react';
