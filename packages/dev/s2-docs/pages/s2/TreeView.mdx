...
export const relatedPages = [
  {title: 'Testing', url: './TreeView/testing'}
];
...
```tsx render docs={docs.exports.TreeView} links={docs.links} props={['selectionMode', 'isDetached', 'isEmphasized']} initialProps={{selectionMode: 'multiple'}} type="s2" wide
...
Use the `href` prop on a `<TreeItem>` to create a link. See the [client side routing guide](routing) to learn how to integrate with your framework. See the [selection guide](selection?component=Tree#selection-behavior) for more details.
...
```tsx links={{TreeView: '#treeview', TreeViewItem: '#treeviewitem', TreeViewItemContent: '#treeviewitemcontent', TreeViewLoadMoreItem: '#treeloadmoreitem', ActionMenu: 'ActionMenu', ActionButtonGroup: 'ActionButtonGroup', Icon: 'icons'}}
<TreeView>
  <TreeViewItem>
    <TreeViewItemContent>
      <Icon />
