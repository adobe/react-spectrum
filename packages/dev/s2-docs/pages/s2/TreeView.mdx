...
Pass custom preview content to the exported `TreeViewDragPreview` from `renderDragPreview` to match the default visual.
...
function ReorderableTree() {
  ...
  return (
    <TreeView
      aria-label="Reorderable folders"
      selectionMode="multiple"
      items={tree.items}
      ...
      ///- end highlight -///
      styles={style({width: 'full', maxWidth: 320, height: 320})}>
      {function renderItem(item) {
        return (
