...
<RangeCalendar aria-label="Hotel booking dates" />
```
...
The RangeCalendar can display multiple months at once using the `visibleMonths` prop. This allows users to see a broader date range at a glance and makes it easier to select ranges that span multiple months.
