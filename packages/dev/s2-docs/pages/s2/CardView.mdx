...
export const description = 'A CardView displays a group of related objects, with support for selection and bulk actions.';
...
Use the `href` prop on a Card to create a link. See the **client side routing guide** to learn how to integrate with your framework. Link interactions vary depending on the selection behavior.  See the [selection guide](selection.html) for more details.
