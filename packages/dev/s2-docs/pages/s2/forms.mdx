...
function action(prevState: {values: Record<string, FormDataEntryValue>, errors: {username: string}}, formData: FormData) {
  return {
...
function Example() {
  let [{values, errors}, formAction] = useActionState(action, {values: {}, errors: {username: ''}});
  ...
  return (
    <Form
      ...
      <TextField
        ...
        defaultValue={typeof values?.username === 'string' ? values.username : undefined}
        isRequired />
      <TextField
        label="Password"
        name="password"
        placeholder="Enter your password"
        defaultValue={typeof values?.password === 'string' ? values.password : undefined}
        type="password"
        isRequired />
      <ButtonGroup>
        <Button type="submit" variant="primary">Submit</Button>
