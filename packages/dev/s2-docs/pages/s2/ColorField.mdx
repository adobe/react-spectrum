...
<VisualExample
  ...
  importSource="@react-spectrum/s2/ColorField"
  type="s2" />
...
Use the `prefix` prop to display an additional element before the input in the color field.
...
function Example() {
  let [color, setColor] = useState<Color | null>(parseColor('#e73623'));
  return (
    <ColorField
      value={color}
      onChange={setColor}
      label="User Email"
      /*- begin highlight -*/
      prefix={<ColorSwatch size="XS" color={color ? color.toString('hex') : undefined} />}
      /*- end highlight -*/
      placeholder="######" />
  );
}
