...
If you previously used `page.css` without a `Provider`, you'll need to add a `Provider` around your app to load the fonts. See the [updated docs](https://react-spectrum.adobe.com/s2/index.html?path=/docs/intro--docs#setting-up-your-app) for details.
...
### Updates
* [Button](Button.html): Fix focus visible styles from being applied on standard focus
* [ContextualHelp](ContextualHelp.html): Update width to match Spectrum designs
* [Tabs](Tabs.html): Update selection indicator when tab text changes
* [TagGroup](TagGroup.html): Fix focus visible styles from being applied on standard focus
...
To fix these Safari issues, we have removed `all: revert-layer` in this release. This means that global CSS will now take precedence over S2's styles. **If you are using a CSS reset on the same page as S2 components, you will need to remove it** or put it in a `@layer` of its own. See [the docs](https://react-spectrum.adobe.com/s2/index.html?path=/docs/intro--docs#css-resets) for more information.
...
* [FullscreenDialog](Dialog.html#fullscreen-dialog) – a fullscreen or takeover modal, similar to a Dialog but with different slots and layout. This corresponds to the previous `type="fullscreen"` and `type="fullscreenTakeover"` APIs.
...
[colors](https://react-spectrum.adobe.com/s2/index.html?path=/docs/style-macro--docs#colors) and [typography](https://react-spectrum.adobe.com/s2/index.html?path=/docs/style-macro--docs#typography) have been added to help clarify
these common use cases. Style macro properties like `width` and `height` now allow for arbitrary pixel size values, please see the [docs](https://react-spectrum.adobe.com/s2/index.html?path=/docs/style-macro--docs#sizing)
for more information. Finally, documentation on [optimizing CSS bundling](https://react-spectrum.adobe.com/s2/index.html?path=/docs/style-macro--docs#css-optimization) have also been
...
* [FullscreenDialog](Dialog.html#fullscreen-dialog)
...
* [AlertDialog](Dialog.html#alert-dialog)
...
* [Contextual Help](ContextualHelp.html)
* [ColorArea](ColorArea.html)
* [ColorField](ColorField.html)
* [ColorSlider](ColorSlider.html)
* [ColorSwatch](ColorSwatch.html)
* [ColorSwatchPicker](ColorSwatchPicker.html)
* [ColorWheel](ColorWheel.html)
...
* Fixed global styles such as CSS resets from applying to Spectrum 2 elements. Note that any CSS rule referenced from an `UNSAFE_className` prop must now be wrapped in `@layer UNSAFE_overrides`. See [the docs](https://react-spectrum.adobe.com/s2/index.html?path=/docs/intro--docs#unsafe-style-overrides) for more details.
