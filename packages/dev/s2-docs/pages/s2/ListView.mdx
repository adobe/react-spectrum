...
<div className={style({display: 'flex', gap: 24, width: 'full', flexWrap: 'wrap', justifyContent: 'center'})}>
  <ListView aria-label="Documents" items={documents} styles={style({flex: 1, minWidth: 280, maxWidth: 400, height: 320})}>
    {item => (
  ...
  <ListView aria-label="Images" items={images} styles={style({flex: 1, minWidth: 280, maxWidth: 400, height: 320})}>
    {item => (
      <ListViewItem id={item.id} textValue={item.name}>
        {/*- begin highlight -*/}
