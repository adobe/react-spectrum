...
You can customize the drag preview by passing <TypeLink links={docs.links} type={docs.exports.ListViewDragPreview} /> to `useDragAndDrop`'s `renderDragPreview` with custom content.

```tsx render type="s2"
"use client";
import {ListView, ListViewItem, ListViewDragPreview} from '@react-spectrum/s2/ListView';
...
function ReorderableList() {
  ...
  let {dragAndDropHooks} = useDragAndDrop({
    ...
    renderDragPreview: (items) => (
      <ListViewDragPreview items={items}>
        <File />
