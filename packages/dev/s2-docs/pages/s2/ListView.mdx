...
You can customize the drag preview content by providing that content to `ListViewDragPreview`, and then passing that to `useDragAndDrop`'s `renderDragPreview`.
...
function ReorderableList() {
  ...
  return (
    <ListView
      ...
      ///- end highlight -///
      styles={style({width: 'full', maxWidth: 320, height: 240})}>
