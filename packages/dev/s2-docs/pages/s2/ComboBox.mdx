...
```tsx render docs={docs.exports.ComboBox} links={docs.links} props={['label', 'placeholder', 'size', 'labelPosition', 'description', 'contextualHelp', 'isDisabled']} initialProps={{label: 'Ice cream flavor', placeholder: 'Select a ice cream flavor'}} type="s2"
...
`ComboBox` follows the [Collection Components API](collections.html?component=ComboBox), accepting both static and dynamic collections. This example shows a dynamic collection, passing a list of objects to the `items` prop, and a function to render the children.
...
function Example() {
  ...
  return (
    /*- begin highlight -*/
    <ComboBox label="Animals" items={options} selectionMode="single" placeholder="Select a animal">
...
<ComboBox label="Preferred fruit or vegetable" placeholder="Select a preferred fruit or vegetable">
  {/*- begin highlight -*/}
  <ComboBoxSection>
    <Header>
  ...
  return (
    <div>
      <ComboBox
        {...props}
        label="Favorite Animal"
        placeholder="Select a favorite animal"
        /*- begin highlight -*/
        /* PROPS */
        inputValue={value}
  ...
  return (
    <div>
       <ComboBox
        ...
        placeholder="Select a engineering major"
        /*- begin highlight -*/
...
```tsx render docs={docs.exports.ComboBox} links={docs.links} props={['isRequired', 'necessityIndicator']} initialProps={{isRequired: true}} type="s2"
"use client";
...
```tsx render docs={docs.exports.ComboBox} links={docs.links} props={['menuTrigger', 'align', 'direction', 'shouldFlip', 'menuWidth']} initialProps={{label: 'Favorite Animal', placeholder: 'Select a favorite animal'}} type="s2"
"use client";
import {ComboBox, ComboBoxItem} from '@react-spectrum/s2';
...
```tsx links={{ComboBox: '#combobox', ComboBoxItem: '#comboboxitem', ComboBoxSection: '#comboboxsection'}}
<ComboBox>
  <ComboBoxItem>
    <Icon />
    <Text slot="label" />
