...
```tsx render docs={docs.exports.ComboBox} links={docs.links} props={['size', 'direction', 'align', 'menuWidth', 'loadingState']} initialProps={{}} expanded type="s2"
...
Provide additional information using ContextualHelp.

```tsx render expanded
"use client";
import {ComboBox, ComboBoxItem, ContextualHelp, Heading, Content, Footer, Link, Text} from '@react-spectrum/s2';

<ComboBox
  label="Ice cream flavor"
  contextualHelp={
    <ContextualHelp>
      <Heading>What is ice cream?</Heading>
      <Content>
        <Text>
...
- **Focus Management**: Focus is properly managed when opening/closing the menu
- **Filter Announcements**: Screen readers announce the number of filtered results
