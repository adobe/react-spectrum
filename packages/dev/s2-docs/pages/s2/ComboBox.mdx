...
Use the `href` prop on a `<ComboBoxItem>` to create a link. See the [client side routing guide](routing) to learn how to integrate with your framework. Link items in a `ComboBox` are not selectable.
