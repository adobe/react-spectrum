...
- <PendingBadge /> Comment out `density` (it has not been implemented yet)
- <PendingBadge /> Comment out `dragAndDropHooks` (it has not been implemented yet)
...
- For expandable rows, use `treeColumn` to designate your expandable column. `defaultExpanded`, `expandedKeys`, and `onExpandedChange` can be used to set and update your expanded rows. See the [TableView](TableView#expandable-rows) documentation for more details.
