...
`SidePanel` and `SideNav` animate in two steps, and never at the same time. The panel's width animates with a CSS transition, so the rest of your page reflows along with it and no extra markup is required. Its contents animate separately, with a [view transition](https://developer.mozilla.org/en-US/docs/Web/API/View_Transition_API): each `SideNavItem` row is wrapped in a [`<ViewTransition>`](https://react.dev/reference/react/ViewTransition) and each `SideNavHeader` is named in CSS, so rows slide to their new positions instead of jumping, and rows that come and go fade in and out.
