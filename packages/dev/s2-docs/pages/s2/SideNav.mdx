...
SideNav uses the `href` prop as a key for selection. 
`id` is used for the expansion state and disabled items as not every item will have an `href`.
...
<RoutedSideNav defaultSelectedRoute="/files">
  {({selectedRoute}) => (
    <SideNav aria-label="Files" defaultExpandedKeys={['your-libraries']} selectedRoute={selectedRoute} styles={style({height: 120, width: 210})}>
