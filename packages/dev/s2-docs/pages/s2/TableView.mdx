...
You can customize the drag preview by passing <TypeLink links={docs.links} type={docs.exports.TableViewDragPreview} /> to `useDragAndDrop`'s `renderDragPreview` with custom content.

```tsx render type="s2"
"use client";
import {TableView, TableHeader, TableBody, Column, Row, Cell, Collection, TableViewDragPreview} from '@react-spectrum/s2/TableView';
...
function ReorderableTable() {
  ...
  let {dragAndDropHooks} = useDragAndDrop({
    ...
    renderDragPreview: (items) => (
      <TableViewDragPreview items={items}>
        <Text>{`${items[0]['text/plain']} (${items[0].type})`}</Text>
