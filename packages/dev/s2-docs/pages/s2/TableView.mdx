...
You can customize the drag preview content by providing that content to `TableViewDragPreview`, and then passing that to `useDragAndDrop`'s `renderDragPreview`.
