...
This example combines `useSelect` with [useButton](../Button/useButton.html) for the trigger, [useListBox](../ListBox/useListBox.html) for the popup, and [usePopover](../Popover/usePopover.html) for positioning. A &lt;`HiddenSelect`&gt; renders a hidden native `<select>` to enable browser form autofill. The `Button`, `ListBox`, and `Popover` components shown collapsed below can be shared with other components such as a ComboBox or Menu.
