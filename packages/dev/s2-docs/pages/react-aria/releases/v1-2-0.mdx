...
We have a huge release today! Its a GA bonanza 🎊 – [Submenu](../Menu#submenus), [unavailable menu items](../v3/Menu#unavailable-items), [DropZone](../DropZone) and [FileTrigger](../FileTrigger) components are now in GA. Check our [blog post](react-aria:blog/creating-a-pointer-friendly-submenu-experience) on the intricacies of the submenu interactions and how we handled them.
...
We have also improved our integration with [client side routers](../frameworks), accepting options to control router-specific behaviors such as scrolling, replacing instead of pushing to the history, base path, etc. For convenience, all collection items in React Aria Components (e.g. `MenuItem`) now also support [`isDisabled`](../Menu#selection-and-actions) and [`onAction`](../Menu) props directly, rather than needing to be defined on the root collection.
...
* `gradientProps` was merged into `colorAreaProps` in [`useColorArea`](../ColorArea/useColorArea).
