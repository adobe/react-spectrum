...
We're excited to announce that our Disclosure components are now generally available! This includes [Disclosure](../Disclosure) and [DisclosureGroup](../DisclosureGroup) in React Aria. These components now also use the `hidden="until-found"` API currently supported in Chrome to enable searching collapsed content with ctrl + F.

We are also introducing [ToggleButtonGroup](../ToggleButtonGroup) in React Aria Components, which supports arrow key navigation with single and multiple selection modes. In addition, Menu now supports the ability to create [Section-level selection groups](../Menu#section-level-selection), allowing you to mix actions, single, and multiple selection within the same menu. Menus can also be nested inside Dialog popovers to enable more complex use cases.
