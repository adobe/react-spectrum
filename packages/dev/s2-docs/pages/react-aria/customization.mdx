...
export const tags = ['motion', 'framer', 'render', 'composeRenderProps', 'useRenderProps', 'Provider', 'useContextProps', 'slots', 'useSlottedContext', 'hooks'];
...
Many React Aria components allow you to customize their rendering based on their current state. The `className`, `style`, and `children` props accept a function that receives the component's states (its _render props_) and returns the value to render. This avoids the need for CSS attribute selectors in many cases, and enables dynamic rendering based on state.

For example, a `Checkbox` can render different content depending on whether it is selected:
...
To support the render props API in a fully custom component — for example one built with [Hooks](#hooks) — use <TypeLink links={docs.links} type={docs.exports.useRenderProps} />. It resolves the `className`, `style`, and `children` props (each of which may be a value or a function) against the `values` you provide, and returns the computed `className`, `style`, and `children` to spread onto your element.

```tsx
import {useRenderProps} from 'react-aria-components';

function MyComponent(props) {
  // Compute the rendered output from the user's props and the current state.
  /*- begin highlight -*/
