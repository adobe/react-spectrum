...
import {PokemonListBox as PokemonCOMPONENT} from './PokemonListBox';
import {DroppableListBox as DroppableCOMPONENT} from './DroppableListBox';
...
function DraggableCOMPONENT() {
  let {dragAndDropHooks} = useDragAndDrop<Pokemon>({
    ...
    getItems(keys, values) {
      return values.map((item: Pokemon) => {
        return {
...
"use client";
import {useDragAndDrop} from 'react-aria-components';
import {PokemonGridList as PokemonCOMPONENT} from './PokemonGridList';
...
import {PokemonCOMPONENT, type Pokemon} from './PokemonCOMPONENT';
...
function DroppableTree() {
  ...
  let {dragAndDropHooks} = useDragAndDrop({
    ...
    async onRootDrop(e) {
      ...
      let getFiles = async (dir): Promise<DirItem[]> => {
        let files: DirItem[] = [];
...
function DroppableCOMPONENT() {
  ...
  return <PokemonCOMPONENT items={list.items as any} dragAndDropHooks={dragAndDropHooks} />;
}
    ...
    async onInsert(e) {
      let processedItems = await Promise.all(
        e.items
          ...
          .map(async item => {
            let pokemon = JSON.parse(await item.getText('pokemon'));
            return {...pokemon, id: pokemon.id ?? Math.random()};
          })
      );
      if (e.target.dropPosition === 'before') {
