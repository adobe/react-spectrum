...
This example uses `useGridList` for the list and `useGridListItem` for each row, building the collection with the [Collection Components API](collections?component=GridList). It matches the React Aria Components GridList starter, including its grid layout, selection checkboxes, image cards, and text styles.
