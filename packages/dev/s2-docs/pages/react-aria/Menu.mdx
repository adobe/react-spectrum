...
<ExampleSwitcher>
  ...
  "use client";
  import {MenuTrigger, SubmenuTrigger, Menu, MenuItem, MenuSection, UnavailableMenuItemTrigger} from 'vanilla-starter/Menu';
  ...
  "use client";
  import {MenuTrigger, SubmenuTrigger, UnavailableMenuItemTrigger, Menu, MenuItem, MenuSection, MenuSeparator} from 'tailwind-starter/Menu';
...
Note that you can use a `<SubmenuTrigger>` and a `<Popover>` together without rendering a child `<Menu>` within.
The example below creates a `<UnavailableMenuItemTrigger>` wrapper around the `<SubmenuTrigger>` to provide additional props and
render a informational popover instead of a submenu.

```tsx render type="vanilla" files={["starters/docs/src/Menu.tsx", "starters/docs/src/Menu.css"]}
"use client";
import {Menu, MenuTrigger, MenuItem, UnavailableMenuItemTrigger, SubmenuTrigger} from 'vanilla-starter/Menu';
import {Button} from 'vanilla-starter/Button';
