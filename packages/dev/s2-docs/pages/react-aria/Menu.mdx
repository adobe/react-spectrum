...
You can use a `<SubmenuTrigger>` a `<Menu>` child to display arbitrary supplemental information about the `<MenuItem>`.
...
```tsx render type="vanilla" files={["starters/docs/src/Menu.tsx", "starters/docs/src/Menu.css"]}
"use client";
import {Menu, MenuTrigger, MenuItem, UnavailableMenuItemTrigger} from 'vanilla-starter/Menu';
import {Button} from 'vanilla-starter/Button';
