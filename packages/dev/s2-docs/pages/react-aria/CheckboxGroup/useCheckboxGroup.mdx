...
Checkbox groups can be built with the [&lt;fieldset&gt;](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/fieldset) and [&lt;input&gt;](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input) HTML elements, but this can be difficult to style. `useCheckboxGroup` and `useCheckboxGroupItem` help achieve accessible checkbox groups that can be styled as needed.

* Checkbox groups are exposed to assistive technology via ARIA, built with native HTML `<input>` elements
...
* Group and individual checkbox disabled states

## State management

`useCheckboxGroup` requires a `state` object created with the `useCheckboxGroupState` hook from `react-stately`, which tracks the set of selected values. The state is passed to each `useCheckboxGroupItem`, typically via React context.

## API

<FunctionAPI function={docs.exports.useCheckboxGroup} links={docs.links} /> <br />
