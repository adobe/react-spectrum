...
export const relatedPages = [
  ...
  {title: 'Testing', url: './Select/testing'}
];
...
`Select` reuses the `ListBox` component, following the [Collection Components API](collections?component=Select). It supports ListBox features such as static and dynamic collections, sections, disabled items, links, text slots, asynchronous loading, etc. See the [ListBox docs](ListBox) for more details.
...
`Select` can include additional components as siblings of the `ListBox`. This example uses an [Autocomplete](Autocomplete) with a [SearchField](SearchField) to let the user filter the items.
...
Use the `SelectValue` render prop function to display the selected items as a [TagGroup](TagGroup).
...
```tsx links={{Select: '#select', Button: 'Button', SelectValue: '#selectvalue', Popover: 'Popover', ListBox: 'ListBox'}}
<Select>
  <Label />
  <Button>
    <SelectValue />
