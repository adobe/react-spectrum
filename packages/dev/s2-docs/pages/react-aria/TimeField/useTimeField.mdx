...
function TimeField(props: AriaTimeFieldProps<TimeValue> & {label?: React.ReactNode}) {
...
function DateSegment({segment, state}: {segment: ReturnType<typeof useTimeFieldState>['segments'][number], state: ReturnType<typeof useTimeFieldState>}) {
