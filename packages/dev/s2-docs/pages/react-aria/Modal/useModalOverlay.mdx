...
* **Accessible** – Content outside the modal is hidden from assistive technologies while it is open. The modal optionally closes when interacting outside, or pressing the <kbd>Escape</kbd> key.
...
State is managed by the `useOverlayTriggerState` hook in `react-stately`. The state object should be passed as an argument to `useModalOverlay`.
