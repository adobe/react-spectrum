...
const animatedQueue = new ToastQueue({
  wrapUpdate(fn) {
...
<Toast style={{viewTransitionName: toast.key}} toast={toast}>
  {/* ... */}
</Toast>
```

Add CSS animations for the transitions:
...
<Anatomy role="img" aria-label="Toast anatomy diagram showing the ToastRegion containing a Toast with ToastContent (title and description) and close button." />
