...
<ExampleSwitcher>
  <VisualExample
    ...
    type="vanilla"
    files={["starters/docs/src/TimeField.tsx", "starters/docs/src/TimeField.css", "starters/docs/src/theme.css"]} />
  <VisualExample
...
A TimeField consists of a label, and a [Group](Group.html) of segments representing each unit of a time (e.g. hours, minutes, and seconds). Each segment is individually focusable and editable by the user, by typing or using the arrow keys to increment and decrement the value.

`TimeField` also supports optional description and `<FieldError>` slots. If there is no visible label, an `aria-label` or `aria-labelledby` must be provided to identify it to assistive technology.
...
Most of this anatomy is shared with [DateField](DateField.html), so you can reuse many components between them.
...
function Example() {
  ...
  let formatter = useDateFormatter({ timeStyle: 'full' });
