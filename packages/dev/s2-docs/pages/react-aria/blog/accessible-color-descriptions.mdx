...
Recently, we released a suite of color picker components in React Aria and React Spectrum. These components help users choose a color in various ways, including a 2D [ColorArea](../ColorArea), channel-based [ColorSlider](../ColorSlider), circular [ColorWheel](../ColorWheel), preset [ColorSwatchPicker](../ColorSwatchPicker), and a hex value [ColorField](../ColorField). You can compose these individual pieces together to create a full [ColorPicker](../ColorPicker) with whatever custom layout or configuration you need.
...
<video src={initialVideoUrl + '#t=0.1'} style={{maxWidth: 'min(100%, 700px)', display: 'block', margin: '20px auto'}} controls preload="metadata" playsInline aria-label="Demo of VoiceOver announcement in ColorArea before improvements" />
...
<img src={bundleSizeImageUrl} style={{maxWidth: 'min(100%, 700px)', display: 'block', margin: '20px auto'}} alt="Screenshot of bundle analysis showing over 1 MB of bundle size accounted for by color names in many languages" />
...
<video src={finalVideoUrl + '#t=0.1'} style={{maxWidth: 'min(100%, 700px)', display: 'block', margin: '20px auto'}} controls preload="metadata" playsInline aria-label="Demo of VoiceOver announcement in ColorArea after improvements" />

Check out our [ColorPicker](../ColorPicker) components in React Aria to build accessible, customizable, and styleable color pickers in your own applications.
