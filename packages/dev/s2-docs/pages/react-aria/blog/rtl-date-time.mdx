...
<video src={localeVideoURL} muted controls playsInline aria-label="Example of a React Spectrum DateField component in various locales" style={{width: '100%', display: 'block', margin: '40px auto'}} />
...
<video src={placeholderVideoURL} muted controls playsInline aria-label="Demo of the date segments shifting when backspacing" style={{width: '100%', display: 'block', margin: '40px auto'}} />
...
Date fields, on the other hand, were much more complicated to solve in comparison. Since we were relying on `display: flex` to format the date segments, the resulting format appeared to mirror the order in which the segments were stored, as returned by [DateFormatter](../internationalized/date/DateFormatter). This suggested that we could apply a similar approach to what we used for time fields — forcing a left-to-right direction on the date segments.
...
<video src={keyboardVideoURL} muted controls playsInline aria-label="Demo of keyboard navigation prior to fixes" style={{width: '100%', display: 'block', margin: '40px auto'}} />
