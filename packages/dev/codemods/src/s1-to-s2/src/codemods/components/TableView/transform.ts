 ...
 */
function addColumnsPropToRow(path: NodePath<t.JSXElement>): void {
  ...
  if (!tableHeaderPath) {
    addComment(
      path.node,
      ' TODO(S2-upgrade): Could not find TableHeader within Table to retrieve columns prop.'
    );
    return;
  }
  ...
  const columnsProp = tableHeaderPath
    .get('openingElement')
    .get('attributes')
    .find(attr => t.isJSXAttribute(attr.node) && attr.node.name.name === 'columns') as
...
function commentIfNestedColumns(path: NodePath<t.JSXElement>) {
...
function addRowHeader(path: NodePath<t.JSXElement>) {
  ...
  tableHeaderPath?.get('children').forEach(child => {
    if (
      ...
      let isRowHeaderProp = (child.get('openingElement') as NodePath)
        .get('attributes')
        .find(attr => t.isJSXAttribute(attr.node) && attr.node.name.name === 'isRowHeader') as
        | NodePath<t.JSXAttribute>
        | undefined;
      if (isRowHeaderProp) {
        hasRowHeader = true;
