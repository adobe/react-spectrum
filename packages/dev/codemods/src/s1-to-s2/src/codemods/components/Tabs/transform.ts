  ...
  tabListPath.traverse({
    JSXElement(itemPath) {
      if (
        ...
        updateComponentWithinCollection(itemPath as NodePath<t.JSXElement>, {
...
function transformTabPanels(
  tabPanelsPath: NodePath<t.JSXElement>,
  itemsProp: t.JSXAttribute | null
): t.JSXElement[] {
  // Dynamic case
  ...
  if (dynamicRender) {
    ...
    if (itemsProp) {
      tabPanelsPath.node.openingElement.attributes.push(
        t.jsxAttribute(t.jsxIdentifier('items'), itemsProp.value)
      );
    }
