...
Duration can often be expressed as an [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601#Durations) duration string (e.g. 'PT20H35M15S'). The <TypeLink links={docs.links} type={docs.exports.parseDuration} /> function can be used to convert an [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601#Durations) duration string into a <TypeLink links={docs.links} type={docs.exports.DateTimeDuration} /> object. The <TypeLink links={docs.links} type={docs.exports.parseDuration} /> function can parse [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601#Durations) duration strings that are expressed with decimals (using commas or periods), in negative values, or with strings that only contain time values (hours, minutes, seconds).

```tsx
let durationString = 'PT20H35M15S'
let duration = parseDuration(durationString)
// => duration = {years: 0, months: 0, weeks: 0, days: 0, hours: 20, minutes: 35, seconds: 15}

let durationString = '-PT20H35M15S'
let duration = parseDuration(durationString)
// => duration = {years: 0, months: 0, weeks: 0, days: 0, hours: -20, minutes: -35, seconds: -15}

let durationString = 'PT20H35M15,75S'
let duration = parseDuration(durationString)
// => duration = {years: 0, months: 0, weeks: 0, days: 0, hours: 20, minutes: 35, seconds: 15.75}
