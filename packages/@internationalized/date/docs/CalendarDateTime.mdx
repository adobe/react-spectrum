...
Duration can often be expressed as an [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601#Durations) duration string (e.g. 'P3Y6M4DT12H30M5S'). The <TypeLink links={docs.links} type={docs.exports.parseDuration} /> function can be used to convert an [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601#Durations) duration string into a <TypeLink links={docs.links} type={docs.exports.DateTimeDuration} /> object. The <TypeLink links={docs.links} type={docs.exports.parseDuration} /> function can parse [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601#Durations) duration strings that are expressed with decimals (using commas or periods) as well as negative values.

```tsx
let durationString = 'P3Y6M6W4DT12H30M5S'
let duration = parseDuration(durationString)
// => duration = {years: 3, months: 6, weeks: 6, days: 4, hours: 12, minutes: 30, seconds: 5}

let durationString = '-P3Y6M6W4DT12H30M5S'
let duration = parseDuration(durationString)
// => duration = {years: -3, months: -6, weeks: -6, days: -4, hours: -12, minutes: -30, seconds: -5}

let durationString = 'P3,25Y6,75M6,5W4,2DT12,2H30,5M5,5S'
let duration = parseDuration(durationString)
// => duration = {years: 3.25, months: 6.75, weeks: 6.5, days: 4.2, hours: 12.2, minutes: 30.5, seconds: 5.5}
