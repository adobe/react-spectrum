...
describe("NumberParser", function () {
  describe("parse", function () {
    it("should support basic numbers", function () {
      expect(new NumberParser("en-US", { style: "decimal" }).parse("10")).toBe(
        10
      );
      expect(new NumberParser("en-US", { style: "decimal" }).parse("-10")).toBe(
        -10
    ...
    it("should support decimals", function () {
      expect(
        new NumberParser("en-US", { style: "decimal" }).parse("10.5")
      ).toBe(10.5);
      expect(
        new NumberParser("en-US", { style: "decimal" }).parse("-10.5")
      ).toBe(-10.5);
      expect(new NumberParser("en-US", { style: "decimal" }).parse(".5")).toBe(
        0.5
      );
      expect(new NumberParser("en-US", { style: "decimal" }).parse("-.5")).toBe(
        -0.5
    ...
    it("should support group characters", function () {
      expect(
        new NumberParser("en-US", { style: "decimal" }).parse("1,000")
      ).toBe(1000);
      expect(
        new NumberParser("en-US", { style: "decimal" }).parse("-1,000")
      ).toBe(-1000);
      expect(
        new NumberParser("en-US", { style: "decimal" }).parse("1,000,000")
      ).toBe(1000000);
      expect(
        new NumberParser("en-US", { style: "decimal" }).parse("-1,000,000")
      ).toBe(-1000000);
    ...
    it("should support negative numbers with different minus signs", function () {
      ...
      expect(
        new NumberParser("en-US", { style: "decimal" }).parse("\u221210")
      ).toBe(NaN);

      expect(new NumberParser("fi-FI", { style: "decimal" }).parse("-10")).toBe(
        -10
      );
      expect(
        new NumberParser("fi-FI", { style: "decimal" }).parse("\u221210")
      ).toBe(-10);
    });

    it("should return NaN for random characters", function () {
      expect(new NumberParser("en-US", { style: "decimal" }).parse("g")).toBe(
        NaN
      );
      expect(
        new NumberParser("en-US", { style: "decimal" }).parse("1abc")
      ).toBe(NaN);
    });
    ...
    describe("units", function () {
      it("should parse with units", function () {
        expect(
          new NumberParser("en-US", { style: "unit", unit: "inch" }).parse(
            "23.5 in"
          )
        ).toBe(23.5);
      });
    ...
    describe("percents", function () {
      it("should parse a percent", function () {
        expect(
          new NumberParser("en-US", { style: "percent" }).parse("10%")
        ).toBe(0.1);
      });

      it("should parse a percent with decimals", function () {
        expect(
          new NumberParser("en-US", { style: "percent" }).parse("10.5%")
        ).toBe(0.11);
        expect(
          new NumberParser("en-US", {
            style: "percent",
    ...
    describe("NumberFormat options", function () {
      it("supports roundingIncrement", function () {
        expect(
          new NumberParser("en-US", { roundingIncrement: 2 }).parse("10")
        ).toBe(10);
    ...
    describe("round trips", function () {
      ...
      const numeralArb = fc.constantFrom(
        "latn",
        "arab",
        "hanidec",
        "deva",
      ...
      const inputsArb = fc
        ...
        .map(({ d, opts, locale, numerals }) => {
          ...
          if (
            ...
            adjustedNumberForFractions =
              d * 10 ** (opts.minimumFractionDigits || 2);
          } else if (
            Math.abs(d) > 1 &&
            opts.minimumFractionDigits &&
            opts.minimumFractionDigits > 1
          ...
          fc.property(
            ...
            function ({ adjustedNumberForFractions, locale, opts, numerals }) {
              const formatter = new Intl.NumberFormat(
                `${locale}-u-nu-${numerals}`,
                opts
              );
              const parser = new NumberParser(locale, opts);

              const formattedOnce = formatter.format(
                adjustedNumberForFractions
              );
              expect(formatter.format(parser.parse(formattedOnce))).toBe(
                formattedOnce
              );
            }
          )
  ...
  describe("isValidPartialNumber", function () {
    it("should support basic numbers", function () {
      expect(
        new NumberParser("en-US", { style: "decimal" }).isValidPartialNumber(
          "10"
        )
      ).toBe(true);
      expect(
    ...
    it("should support decimals", function () {
      ...
      expect(
        new NumberParser("ar-AE", { style: "decimal" }).isValidPartialNumber(
          "٫"
        )
      ).toBe(true);
      expect(
    ...
    it("should support group characters", function () {
      ...
      ).toBe(true); // en-US-u-nu-arab uses commas as the decimal point character
      expect(
        new NumberParser("en-US", { style: "decimal" }).isValidPartialNumber(
          ",000"
        )
      ).toBe(false); // latin numerals cannot follow arab decimal point
      expect(
        new NumberParser("en-US", { style: "decimal" }).isValidPartialNumber(
          "1,000"
        )
    ...
    it("should support negative numbers with different minus signs", function () {
      ...
      expect(
        new NumberParser("fi-FI", { style: "decimal" }).isValidPartialNumber(
          "-"
        )
      ).toBe(true);
      expect(
    ...
    it("should return false for negative numbers if minValue >= 0", function () {
      ...
      expect(
        new NumberParser("en-US", {
          style: "currency",
          currency: "USD",
        }).isValidPartialNumber("-$", 0)
      ).toBe(false);
      expect(
        new NumberParser("en-US", {
          style: "currency",
          currency: "USD",
        }).isValidPartialNumber("-$1", 0)
      ).toBe(false);
    });
    ...
    it("should support units", function () {
      ...
      expect(
        new NumberParser("en-US", {
          style: "unit",
          unit: "inch",
        }).isValidPartialNumber("10 in")
      ).toBe(true);
      expect(
        new NumberParser("en-US", {
          style: "unit",
          unit: "inch",
        }).isValidPartialNumber("10.5 in")
      ).toBe(true);
      expect(
        new NumberParser("en-US", {
          style: "unit",
          unit: "inch",
        }).isValidPartialNumber("10 i")
      ).toBe(false);
      expect(
        new NumberParser("en-US", {
          style: "unit",
          unit: "inch",
        }).isValidPartialNumber("10.5 i")
      ).toBe(false);
    });

    it("should support percents", function () {
      expect(
        new NumberParser("en-US", { style: "percent" }).isValidPartialNumber(
          "10"
        )
      ).toBe(true);
      expect(
  ...
  describe("getNumberingSystem", function () {
    it("should return the default numbering system for a locale", function () {
      ...
      expect(
        new NumberParser("en-US", { style: "decimal" }).getNumberingSystem("12")
      ).toBe("latn");
      expect(
        new NumberParser("en-US", { style: "decimal" }).getNumberingSystem(".")
      ).toBe("latn");
      ...
      expect(
        new NumberParser("ar-AE", { style: "decimal" }).getNumberingSystem("١٢")
      ).toBe("arab");
      expect(
        new NumberParser("ar-AE", { style: "decimal" }).getNumberingSystem("٫")
      ).toBe("arab");
      expect(
        new NumberParser("ar-AE", { style: "decimal" }).getNumberingSystem(
          "١٫٢"
    ...
    it("should support using non-default numbering systems for a locale", function () {
      expect(
        new NumberParser("en-US", { style: "decimal" }).getNumberingSystem("١٢")
      ).toBe("arab");
      expect(
        new NumberParser("en-US", { style: "decimal" }).getNumberingSystem("٫")
      ).toBe("arab");
      ...
      expect(
        new NumberParser("ar-AE", { style: "decimal" }).getNumberingSystem("12")
      ).toBe("latn");
      expect(
        new NumberParser("ar-AE", { style: "decimal" }).getNumberingSystem(".")
      ).toBe("latn");
