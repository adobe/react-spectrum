...
As mentioned on the Collection [page](./collections.html#unique-keys), there are two ways that keys can be defined for your collection items.
...
the `dataId`s provided as the `Item`'s `key`s are `number`s.
