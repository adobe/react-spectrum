...
For server side filtering, this can be implemented by using the `filterText` passed to the `load` as an API parameter.
...
The input text of the ComboBox is controlled by providing `list.filterText` as the `inputValue` prop. By passing `list.setFilterText` to
...
input value, ensuring that the list of items is properly updated. Finally, the state of the list (`loading`, `loadingMore`, `filtering`, etc) is
