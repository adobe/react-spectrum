...
import {AutocompleteProps, MenuTriggerAction} from '@react-types/autocomplete';
...
interface AutocompleteStateProps<T> extends AutocompleteProps<T> {
...
/**
 * Provides state management for an autocomplete component. Handles building a collection
 * of items from props and manages the option selection state. In addition, it tracks the input value,
 * focus state, and other properties of the autocomplete.
 */
export function useAutocompleteState<T extends object>(props: AutocompleteStateProps<T>): AutocompleteState<T> {
  let {
    defaultFilter,
    menuTrigger = 'input',
  ...
  let open = (focusStrategy?: FocusStrategy, trigger?: MenuTriggerAction) => {
    ...
    // This is to prevent autocomplete components with empty defaultItems from opening but allow controlled items autocompletes to open even if the inital list is empty (assumption is user will provide swap the empty list with a base list via onOpenChange returning `menuTrigger` manual)
  ...
  let commitSelection = () => {
    ...
    if (props.selectedKey !== undefined && props.inputValue !== undefined) {
      ...
      // If only a single aspect of autocomplete is controlled, reset input value and close menu for the user
      resetInputValue();
      triggerState.close();
    }
  };
