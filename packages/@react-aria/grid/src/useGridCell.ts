...
import {isFocusVisible, useLongPress} from '@react-aria/interactions';
...
export function useGridCell<T, C extends GridCollection<T>>(props: GridCellProps, state: GridState<T, C>, ref: RefObject<FocusableElement>): GridCellAria {
  ...
  useEffect(() => {
    if (isEditing !== wasEditing.current && (isEditing || (state.selectionManager.isFocused && state.selectionManager.focusedKey === node.key))) {
      focus();
