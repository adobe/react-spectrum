...
import {focusWithoutScrolling, getActiveElement, getEventTarget, nodeContains, useEffectEvent, useEvent, useId, useLayoutEffect} from '@react-aria/utils';
...
export function useSubmenuTrigger<T>(props: AriaSubmenuTriggerProps, state: SubmenuTriggerState, ref: RefObject<FocusableElement | null>): SubmenuTriggerAria<T> {
  ...
  let submenuKeyDown = (e: KeyboardEvent) => {
    ...
    if (!nodeContains(e.currentTarget, getActiveElement(document))) {
      return;
    }
      ...
      case 'ArrowRight':
        if (!isDisabled) {
          if (direction === 'ltr') {
            ...
            if (type === 'menu' && !!submenuRef?.current && getActiveElement(document) === ref?.current) {
