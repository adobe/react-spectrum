...
<p>{docs.exports.useSearchAutocomplete.description}</p>
...
`useSearchAutocomplete` helps achieve add accessible autocomplete to search components and can be styled as needed.
...
to and see search suggestions within the list box. The list box popup may be opened by a variety of input field interactions specified
...
If the search field does not have a visible label, an `aria-label` or `aria-labelledby` prop must be passed instead to
...
As seen below, an iterable list of options is passed to the SearchAutocomplete using the `defaultItems` prop. Each item accepts a `key` prop, which
...
The following example shows how you would create a controlled SearchAutocomplete, controlling everything from the selected value (`selectedKey`)
to the autocomplete options (`items`). By passing in `inputValue`, `selectedKey`, and `items` to the `SearchAutocomplete` you can control
...
It is important to note that you don't have to control every single aspect of a SearchAutocomplete. If you decide to only control a single property of the
SearchAutocomplete, be sure to provide the change handler for that prop as well e.g. controlling `selectedKey` would require `onSelectionChange` to be passed to `useSearchAutocomplete` as well.
