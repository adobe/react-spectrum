...
interface AriaSearchAutocompleteProps<T> extends ComboBoxProps<T> {
...
export function useSearchAutocomplete<T>(props: AriaSearchAutocompleteProps<T>, state: ComboBoxState<T>): SearchAutocompleteAria<T> {
  ...
  let listBoxProps = useLabels({
    id: menuProps.id,
    'aria-label': 'listBoxLabel', // TODO: formatMessage('listboxLabel'),
  ...
  useEffect(() => {
    if (isAppleDevice() && focusedItem != null && itemKey !== lastItem.current) {
      ...
      // TODO:  formatMessage('focusAnnouncement', {
  ...
  useEffect(() => {
    ...
    if (state.isOpen && (didOpenWithoutFocusedItem || optionCount !== lastSize.current)) {
      let announcement = 'countAnnouncement';// TODO: formatMessage('countAnnouncement', {optionCount});
      announce(announcement);
    }
  ...
  useEffect(() => {
    if (isAppleDevice() && state.isFocused && state.selectedItem && state.selectedKey !== lastSelectedKey.current) {
      ...
      let announcement = 'selectedAnnouncement'; // TODO: formatMessage('selectedAnnouncement', {optionText});
      announce(announcement);
    }
