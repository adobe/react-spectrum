...
can be used to build dialogs. However, it is not yet widely supported across browsers.
...
If a dialog not have a visible title element, an `aria-label` or `aria-labelledby`
...
and hides content ouside the dialog 
...
prop is set to true, or closed by pressing the Escape key.
...
function ModalDialog({title, children, isOpen, onClose, isDismissable}) {
