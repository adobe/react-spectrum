...
components such as a `Select` created with [useSelect](../Select/useSelect.html) or a `Dialog` popover created with [useDialog](../Dialog/useDialog.html).
...
ComboBox follows the [Collection Components API](v3:collections.html), accepting both static and dynamic collections.
