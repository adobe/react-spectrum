...
A ComboBox can be built using the [&lt;datalist&gt;](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/datalist) HTML element, but this is very limited in functionality and difficult to style.
`useComboBox` helps achieve accessible ComboBox components that can be styled as needed.
...
A ComboBox consists of a label, an input which displays the current value, a listbox popup, an optional button
...
function ListBoxPopup(props) {
  ...
  // Add hidden <DismissButton> components at the end of the list
...
The following example shows how you would create a uncontrolled ComboBox. The input value, selected option, and menu open state is completely
uncontrolled, with the default input text set by the `defaultInputValue` prop and handled by `useComboBoxState`.
...
function ControlledComboBox() {
  ...
  let onInputChange = (value) => {
    let filteredItems = optionList.filter(item => startsWith(item.name, value));
    setFieldState(prevState => ({
      isOpen: filteredItems.length > 0 ? true : false,
      inputValue: value,
...
function Option({item, state, shouldUseVirtualFocus}) {
  ...
  // Get props for the option element. Prevent options from receiving true focus via shouldUseVirtualFocus.
  let {optionProps} = useOption({
    key: item.key,
    isDisabled,
    isSelected,
...
* `manual`: ComboBox menu only opens when the user presses the trigger button.
