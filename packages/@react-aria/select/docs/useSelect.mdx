...
The option `hasA11yIgnoreDataAttr` exposes a data attribute `data-rsp-a11y-ignore` on the hidden select element that can be used to ignore the false positives.
