...
<HeaderInfo
  ...
  sourceData={[
    {type: 'W3C', url: 'https://www.w3.org/TR/wai-aria-1.1/#progressbar'}
  ]} />
...
assistive technology using the [progressbar](https://www.w3.org/TR/wai-aria-1.1/#progressbar)
ARIA role.

`useProgressBar` returns two sets of props that you should spread onto the appropriate element:

* `progressBarProps` - props to spread onto the wrapper element
* `labelProps` - props to spread onto the visual label (if any).

If you have no visual label, an `aria-label` or `aria-labelledby` prop must be passed instead 
...
function ProgressBar(props) {
  ...
  return (
    <div {...progressBarProps}>
      ...
      <div style={{width: 200, height: 10, background: 'gray'}}>
...
function Spinner() {
  ...
  return (
    <svg {...progressBarProps} viewBox="0 0 32 32" fill="none" strokeWidth={strokeWidth}>
      <circle
        role="presentation"
        cx={center}
        cy={center}
