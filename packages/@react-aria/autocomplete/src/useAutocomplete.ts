...
import {AutocompleteProps} from '@react-types/autocomplete';
...
interface AriaAutocompleteProps<T> extends AutocompleteProps<T> {
 ...
 * Autocomplete combines a text input with a listbox, allowing users to filter a list of options to items matching a query.
 * @param props - Props for the autocomplete.
 * @param state - State for the select, as returned by `useAutocompleteState`.
 */
export function useAutocomplete<T>(props: AriaAutocompleteProps<T>, state: AutocompleteState<T>): AutocompleteAria<T> {
  let {
    popoverRef,
    inputRef,
    listBoxRef,
  ...
  // If a touch happens on direct center of an Autocomplete input, might be virtual click from iPad so open Autocomplete menu
