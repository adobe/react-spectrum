...
`useHover` handles hover interactions for an element. A hover interactions begins when a user moves their pointer
...
Each of these handlers is fired with a `HoeverEvent`, which exposes information about the target and the
