...
```tsx example
import type {TextItem} from '@react-aria/dnd';
    ...
    async onDrop(e) {
      let items = await Promise.all(
        e.items
          ...
          .map((item: TextItem) => item.getText('text/plain'))
      );
...
function DropTarget() {
  ...
  let {dropProps, isDropTarget} = useDrop({
    ...
    /*- begin highlight -*/
    async onDrop(e) {
      let item = e.items.find(item => item.kind === 'text' && item.types.has('my-app-custom-type')) as TextItem;
      if (item) {
...
```tsx example
import type {FileItem} from '@react-aria/dnd';
...
function DropTarget() {
  ...
  let {dropProps, isDropTarget} = useDrop({
    ...
    /*- begin highlight -*/
    async onDrop(e) {
      let item = e.items.find(item => item.kind === 'file' && (item.type === 'image/jpeg' || item.type === 'image/png')) as FileItem;
      if (item) {
...
```tsx example
import type {DirectoryItem} from '@react-aria/dnd';
...
function DropTarget() {
  ...
  let {dropProps, isDropTarget} = useDrop({
    ...
    async onDrop(e) {
      ...
      let dir = e.items.find(item => item.kind === 'directory') as DirectoryItem;
      if (dir) {
...
function DropTarget() {
  ...
  let {dropProps, isDropTarget} = useDrop({
    ...
    /*- end highlight -*/
    async onDrop(e) {
      let item = e.items.find(item => item.kind === 'file' && item.type === 'image/png') as FileItem;
      if (item) {
        setFile(URL.createObjectURL(await item.getFile()));
      }
    }
...
function DropTarget(props) {
  ...
  let {dropProps, isDropTarget} = useDrop({
    ref,
    async onDrop(e) {
      let item = e.items.find(item => item.kind === 'text' && item.types.has('my-app-file')) as TextItem;
      if (!item) {
        return;
      }
