...
function DropTarget() {
  ...
  let {buttonProps} = useButton({elementType: 'div'}, ref);
  return (
    <div {...mergeProps(dropProps, buttonProps)} ref={ref} className={`droppable grid ${isDropTarget ? 'target' : ''}`}>
      {contents}
    </div>
  );
}
...
function DropTarget() {
  ...
  return (
    <ul {...mergeProps(dropProps, buttonProps)} ref={ref} className={`droppable ${isDropTarget ? 'target' : ''}`} style={{display: 'block', width: 'auto'}}>
