...
function Pasteable() {
  ...
  let {clipboardProps} = useClipboard({
    async onPaste(items) {
      let item = items.find(item => item.kind === 'file' && (item.type === 'image/jpeg' || item.type === 'image/png')) as FileItem;
      if (item) {
...
function Pasteable() {
  ...
  let {clipboardProps} = useClipboard({
    async onPaste(items) {
      ...
      let dir = items.find(item => item.kind === 'directory') as DirectoryItem;
      if (dir) {
