...
The examples above apply `dragProps` (used for traditional pointer-based drag and drop), and `dragButtonProps` (used for accessible keyboard-base drag and drop) to the same element. This means the draggable element is focusable, and represented semantically as a button.

However, there are cases where this is not possible. Buttons do not support nested interactive elements, so if you have a case where a draggable element must contain interactive elements such as buttons, text fields, checkboxes, etc., then the drag button can be split into a separate element. This is often represented as a visual affordance for dragging. The full element remains draggable via mouse or touch, but keyboard and screen reader accessible drag and drop is accomplished using this affordance instead.
