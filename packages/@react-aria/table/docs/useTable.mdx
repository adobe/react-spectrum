...
function Resizer(props) {
  ...
  return (
    <div
      role="presentation"
      className={`aria-table-resizer ${isFocusVisible ? 'focus' : ''} ${layoutState.resizingColumn === column.key ? 'resizing' : ''}`}
      {...resizerProps}>
      <VisuallyHidden>
        <input
          ref={ref}
          className="aria-table-resizer-input"
...
.aria-table-resizer.focus.resizing {
  border-color: orange;
  background-color: transparent;
}

.aria-table-resizer-input {
  margin: 0px;
}
```
