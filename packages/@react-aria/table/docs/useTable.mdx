...
`useTable` supports row actions via the `onRowAction` prop and cell actions via the `onCellAction` prop, both of which is useful for functionality such as navigating into items to open them or get more details.
In the default `"toggle"` selection behavior, when nothing is selected, clicking or tapping the row triggers the row action. When at least one item is selected, the table is in selection mode,
...
<div style={{display: 'flex', 'flex-wrap': 'wrap', gap: '24px'}}>
  <PokemonTable aria-label="Pokemon table with row actions and toggle selection behavior " selectionMode="multiple" onCellAction={key => alert(`Opening cell ${key}...`)} onRowAction={key => alert(`Opening item ${key}...`)} />
  <PokemonTable aria-label="Pokemon table with row actions and replace selection behavior" selectionMode="multiple" selectionBehavior="replace" onCellAction={key => alert(`Opening cell ${key}...`)} onRowAction={key => alert(`Opening item ${key}...`)} />
</div>
