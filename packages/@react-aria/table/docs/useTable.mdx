...
```tsx example export=true render=false
// Reuse the MenuTrigger from your component library. See below for details.
import {MenuTrigger} from 'your-component-library';
...
The `MenuTrigger` is used in the `ResizableTableColumnHeader` to display a list of available actions available to a column. It is built using the [useMenuTrigger](useMenu.html)
