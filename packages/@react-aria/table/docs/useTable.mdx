...
`useTable` also supports cell actions via the `onCellAction`. It follows the same interaction patterns as `onRowAction` in the default `"toggle"` selection mode, with the only difference being that it fires when clicking or tapping any cell within the
table.

Note: `onCellAction` is not meant to be used alongside `onRowAction` or with the `"replace"` `selectionMode`.

```tsx example
<PokemonTable aria-label="Pokemon table with cell actions and toggle selection behavior" selectionMode="multiple" onCellAction={key => alert(`Opening cell ${key}...`)} />
```
