...
You'll recall that in the implementation of [Example](#example) above, we passed all props to `useTableState`, so we can just pass `onRowAction` through our `PokemonTable`.
