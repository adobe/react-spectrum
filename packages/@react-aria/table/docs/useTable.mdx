...
[&lt;tbody&gt;](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/tbody), and other table specific HTML elements, but is very limited in functionality especially when it comes to user interactions.
...
* Column sorting
* Multiple [row header](https://www.w3.org/TR/wai-aria-1.1/#rowheader) support
...
If the table supports row selection, each row includes a selection checkbox in the first column. Additionally, a "select all" checkbox is displayed as the first column header if the table supports multiple row selection.
...
<TypeContext.Provider value={docs.links}>
  <InterfaceType properties={docs.links[docs.exports.useTableRowGroup.return.id].properties} />
</TypeContext.Provider>
...
<TypeContext.Provider value={docs.links}>
  <InterfaceType properties={docs.links[docs.exports.useTableCell.return.id].properties} />
</TypeContext.Provider>
...
See [useTableState](/react-stately/useTableState.html) for more info on the props they support.
...
function Table(props) {
  ...
  let headerRows = collection.rows.filter(item => item.type === 'headerrow' && item);
  let rows = collection.rows.filter(item => item.type === 'item' && item);
  ...
  let renderRowNode = (node) => {
    ...
    if (collection.rowHeaderColumnKeys.has(node.column.key)) {
...
function Example() {
  ...
  return (
    <Table aria-label="Example dynamic collection table" style={{height: '210px', maxWidth: '400px'}}>
...
```tsx example
<Table aria-label="Example table with single selection (uncontrolled)" selectionMode="single" defaultSelectedKeys={['2']} style={{height: '210px', maxWidth: '400px'}}>
  <TableHeader>
    <Column>Name</Column>
    <Column>Type</Column>
    <Column>Level</Column>
...
function PokemonTable(props) {
  ...
  return (
    <Table aria-label="Example table with single selection (controlled)" selectionMode="single" selectedKeys={selected} onSelectionChange={setSelected} style={{height: '210px', maxWidth: '400px'}} {...props}>
      <TableHeader columns={columns}>
        {column => (
...
function AsyncSortTable() {
  ...
  return (
    <Table aria-label="Example table with client side sorting" sortDescriptor={list.sortDescriptor} onSortChange={list.sort} style={{height: '300px', maxWidth: '600px'}}>
      <TableHeader columns={columns}>
        {(column) => (
