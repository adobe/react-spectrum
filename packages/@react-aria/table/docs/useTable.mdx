...
function ResizableTableColumnHeader({column, state, layoutState, onResizeStart, onResize, onResizeEnd}) {
 let {widths} = layoutState;
  ...
  return (
    <th
      {...mergeProps(columnHeaderProps, focusProps, hoverProps, pressProps, descriptionProps)}
      colSpan={column.colspan}
      style={{
        textAlign: column.colspan > 1 ? 'center' : 'left',
        padding: '5px 10px',
      ...
      <div style={{display: 'flex', position: 'relative'}}>
        ...
        {allowsResizing &&
          <Resizer showResizer={showResizer} ref={resizerRef} triggerRef={ref} column={column} layoutState={layoutState} onResizeStart={onResizeStart} onResize={onResize} onResizeEnd={onResizeEnd} />
        }
      </div>
    </th>
  );
