          ...
          <TableHeaderRow key={headerRow.key} item={headerRow} state={state} className={classNames(ariaStyles, 'aria-table-row', 'aria-table-headerRow')}>
            {[...headerRow.childNodes].map(column =>
              column.props.isSelectionCell
                ...
                : <TableColumnHeader key={column.key} column={column} state={state} widths={widths} layoutState={layoutState} onResize={props.onResize} />
            )}
          </TableHeaderRow>
        ))}
      </TableRowGroup>
...
function Resizer({column, state, layoutState, onResize}) {
  let ref = useRef(null);
...
export function TableColumnHeader({column, state, widths, layoutState, onResize}) {
  let ref = useRef();
    ...
    <th
      ...
      <div style={{display: 'flex', position: 'relative'}}>
        ...
        </div>
        {
          ...
          <Resizer column={column} state={state} layoutState={layoutState} onResize={onResize} />
        }
      </div>
    </th>
  );
