...
with [useDialog](../Dialog/useDialog.html) to create fully accessible modal dialogs. Other overlays
...
This example shows how to build a typical modal dialog, by combining `useModalOverlay` with [useDialog](../Dialog/useDialog.html). The `Dialog` component used in this example can also be reused within a [popover](../Popover/usePopover.html) or other types of overlays.
...
The `Dialog` component is rendered within the `ModalOverlay` component. It is built using the [useDialog](../Dialog/useDialog.html) hook, and can also be used in other overlay containers such as [popovers](../Popover/usePopover.html).
