...
see [useMenu](useMenu.html), which builds on `useOverlayTrigger` and provides
...
const Popover = React.forwardRef(({
  ...
  style,
  ...otherProps
}: PopoverProps, ref: React.RefObject<HTMLDivElement>) => {
...
function Example() {
  ...
  return <>
    <Button {...triggerProps} buttonRef={triggerRef}>Open Popover</Button>
    {state.isOpen &&
      <OverlayContainer>
        <Popover
          {...overlayProps}
