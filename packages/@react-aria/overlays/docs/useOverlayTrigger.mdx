...
* Handles closing the overlay when interacting outside and pressing the Escape key, when combined with <TypeLink links={docs.links} type={docs.exports.useOverlay} />
...
Focus is restored back to the button that triggered the popover it when it closes by a
...
into a portal at the end of the document, and the content just before it will be
...
// Application must be wrapped in an OverlayContainer so that it can be
