...
* Positions the overlay relative to the trigger, automatically flipping when needed
...
function Dialog({title, children, ...props}) {
  ...
  return (
    <div {...dialogProps} ref={ref} style={{padding: 30, maxWidth: 200}}>
      {title &&
...
```tsx example
<PopoverTrigger placement="right top" label="Trigger">
  <Dialog>
    This is a popover placed to the right of its trigger and aligned to its top edge.
  </Dialog>
</PopoverTrigger>
```
