...
export function useColorSlider(props: ColorSliderAriaOptions, state: ColorSliderState): ColorSliderAria {
  ...
  if (props.label == null &&
    (!props['aria-label'] || props['aria-label'] === channel)) {
    defaultLabel = formatMessage(channel);
  ...
  let {inputProps, thumbProps} = useSliderThumb({
    ...props,
    /* Since labelProps have been generated by useSlider, 
    pass label prop as undefined so that an unused label id
    is not added to inputProps['aria-labelledby']. */
    label: undefined,
    index: 0
  }, state);

  // Avoid redundant self-referential labelling of the input
  if (defaultLabel && groupProps['aria-label'] === inputProps['aria-label']) {
    inputProps['aria-labelledby'] = inputProps['aria-labelledby'].replace(inputProps.id, '').trim();
  }
