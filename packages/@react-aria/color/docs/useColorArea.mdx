...
[useColorWheel](useColorWheel.html) hook, or to display a the color value stored in a state using a preview swatch.
  ...
  return (
    <>
      ...
      <div style={{display: 'flex', flexWrap: 'wrap'}}>
        ...
        <ColorSwatch
          value={color}
          aria-label={`color swatch: ${color.toString('hsla')}`} />
      </div>
...
The `ColorSlider` component is used in the example above to control the channel value not controlled by the `ColorArea`, in this case, the `brightness` channel, or the `alpha` channel. It is built using the [useColorSlider](useColorSlider.html) hook, and can be shared with other color components.
...
function Example() {
  ...
  return (
    <>
      ...
      <p>Current RGB color value: <ColorSwatch value={color} aria-hidden="true" style={{width: '1em', height: '1em', verticalAlign: 'middle'}} /> {color.toString('rgb')}</p>
    </>
  );
}
```
...
function Example() {
  ...
  return (
    <>
      ...
      <p>Current HSL color value: <ColorSwatch value={color} aria-hidden="true" style={{width: '1em', height: '1em', verticalAlign: 'middle'}} /> {color.toString('hsl')}</p>
    </>
  );
}
```
...
function Example() {
  ...
  return (
    <>
      ...
      <p>Current HSB color value: <ColorSwatch value={color} aria-hidden="true" style={{width: '1em', height: '1em', verticalAlign: 'middle'}} /> {color.toString('hsb')}</p>
    </>
  );
}
```
