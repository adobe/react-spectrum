...
touch-friendly color area that can be styled as needed.

* Support for adjusting two channels values of an HSL, HSB or RGB color value
...
the background gradient and positioning of the thumb are provided by `useColorArea` in the returned  prop for each element.
The two `<input>` elements inside the thumb represent the color channel values to assistive technologies, and are hidden
from view, within `useColorArea`, using the [useVisuallyHidden](useVisuallyHidden.html) hook. The thumb also uses the
...
"Text color",  which `useColorArea` will return as the `aria-label` prop "Text color, Color picker"
for each of the `<input>` elements, or on mobile devices, the group containing them. When a custom `aria-label`
