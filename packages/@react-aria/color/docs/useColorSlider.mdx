...
Additionally, the vertical slider does not have a visible label or `<output>` element. This is done by simply not using the returned
...
This example shows how to build a similar color picker to the one above, but for HSLA colors.
