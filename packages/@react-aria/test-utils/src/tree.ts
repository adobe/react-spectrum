...
export class TreeTester {
  ...
  private async keyboardNavigateToRow(opts: {row: HTMLElement, selectionOnNav?: 'default' | 'none'}) {
    ...
    if (document.activeElement !== this._tree && !this._tree.matches(':focus-within')) {
    ...
    } else if (this._tree.matches(':focus-within') && document.activeElement!.getAttribute('role') !== 'row') {
      do {
  ...
  async toggleRowExpansion(opts: TreeToggleExpansionOpts): Promise<void> {
    ...
    if (!this.tree.matches(':focus-within')) {
      await act(async () => {
        this.tree.focus();
      });
    }
