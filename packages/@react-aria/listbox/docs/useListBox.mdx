...
import {useListBox, useOption, useFocusRing, mergeProps, useObjectRef} from 'react-aria';
import {useListState} from 'react-stately';
import {CollectionBuilder, Collection, createLeafComponent} from '@react-aria/collections';

// Context to pass state from ListBox to items.
const ListStateContext = React.createContext<ListState | null>(null);
...
function ListBoxInner<T extends object>(props: AriaListBoxProps<T>) {
...
const Option = createLeafComponent('item', function Option(props: OptionProps, forwardedRef: ForwardedRef<HTMLElement>, item: Node<any>) {
  ...
  return (
    <>
      <ListBox label="Choose sandwich contents" selectionMode="multiple" selectedKeys={selected} onSelectionChange={setSelected}>
        ...
        <Option key="cheese">Cheese</Option>
        <Option key="tuna">Tuna Salad</Option>
        <Option key="egg">Egg Salad</Option>
        <Option key="ham">Ham</Option>
      </ListBox>
...
const ListBoxSection = createBranchComponent('section', function ListBoxSection(props, ref, section) {
  let state = React.useContext(ListStateContext);
  ...
  return <>
    ...
    <li {...itemProps}>
      ...
      <ul
        ...
        {[...state.collection.getChildren(section.key)].map(item => item.render(item))}
      </ul>
    </li>
  </>;
});
...
<ListBox label="Choose sandwich contents" selectionMode="multiple">
  <ListBoxSection title="Veggies">
    <Option id="lettuce">Lettuce</Option>
    <Option id="tomato">Tomato</Option>
    <Option id="onion">Onion</Option>
  </ListBoxSection>
  <ListBoxSection title="Protein">
    <Option id="ham">Ham</Option>
    <Option id="tuna">Tuna</Option>
    <Option id="tofu">Tofu</Option>
  </ListBoxSection>
  <ListBoxSection title="Condiments">
    <Option id="mayo">Mayonaise</Option>
    <Option id="mustard">Mustard</Option>
    <Option id="ranch">Ranch</Option>
  </ListBoxSection>
</ListBox>
```
...
Similarly to the props on ListBox, `<ListBoxSection>` takes an array of data using the `items` prop.
...
function Example() {
  ...
  return (
    <ListBox
      ...
      {item => (
        <ListBoxSection id={item.name} items={item.children} title={item.name}>
          {item => <Option>{item.name}</Option>}
        </ListBoxSection>
      )}
    </ListBox>
...
function AsyncLoadingExample() {
  ...
  return (
    <ListBox label="Pick a Pokemon" items={list.items} selectionMode="single">
      {(item) => <Option key={item.name}>{item.name}</Option>}
    </ListBox>
  );
}
```
...
By default, interacting with an item in a ListBox triggers `onSelectionChange`. Alternatively, items may be links to another page or website. This can be achieved by passing the `href` prop to the `<Option>` component.
...
<ListBox aria-label="Links">
  <Option href="https://adobe.com/" target="_blank">Adobe</Option>
  <Option href="https://apple.com/" target="_blank">Apple</Option>
  <Option href="https://google.com/" target="_blank">Google</Option>
  <Option href="https://microsoft.com/" target="_blank">Microsoft</Option>
</ListBox>
```
...
The `<Option>` component works with frameworks and client side routers like [Next.js](https://nextjs.org/) and [React Router](https://reactrouter.com/en/main). As with other React Aria components that support links, this works via the <TypeLink links={utilsDocs.links} type={utilsDocs.exports.RouterProvider} /> component at the root of your app. See the [client side routing guide](routing.html) to learn how to set this up.
