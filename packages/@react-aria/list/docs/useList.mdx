...
<HeaderInfo
  ...
  componentNames={['useList', 'useListItem', 'useListSelectionCheckbox']}
  sourceData={[
...
<FunctionAPI function={docs.exports.useList} links={docs.links} />
<FunctionAPI function={docs.exports.useListItem} links={docs.links} />
<FunctionAPI function={docs.exports.useListSelectionCheckbox} links={docs.links} />

## Features

A list can be built using [&lt;ul&gt;](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/ul) or [&lt;ol&gt;](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/ol) HTML elements, but is very limited in functionality especially when it comes to user interactions.
...
`useList` helps achieve accessible and interactive list components that can be styled as needed.
...
A list consists of a container element, with rows of data inside. The rows within a list may contain focusable elements or plain text content.
If the list supports row selection, each row can optionally include a selection checkbox.

The <TypeLink links={docs.links} type={docs.exports.useList} /> and <TypeLink links={docs.links} type={docs.exports.useListItem} /> hooks handle keyboard, mouse, and other interactions to support
row selection, in list navigation, and overall focus behavior. Those hooks handle exposing the list and its contents to assistive technology using ARIA. <TypeLink links={docs.links} type={docs.exports.useListSelectionCheckbox} /> handles row selection and associating each checkbox with its respective rows
...
`useList` requires knowledge of the rows in the list in order to handle keyboard
...
The <TypeLink links={docs.links} type={docs.exports.useList} /> hook will be used to render the outer most list element. It uses
...
You may notice the extra `<div>` with `gridCellProps` in our example. This is needed because we are following the [ARIA grid pattern](https://www.w3.org/WAI/ARIA/apg/patterns/grid/).

You may also notice that we included an info button in the rows in our example. This is to highlight `useList`'s ability to handle interactive elements inside a row.
...
import {useList, useListItem} from '@react-aria/list';
...
function List(props) {
  ...
  let { gridProps } = useList(props, state, ref);
  ...
  return (
    ...
    >
      <div {...gridCellProps}>
        ...
        {item.rendered}
        <button style={{float: 'right'}} {...buttonProps} ref={buttonRef}>Info</button>
      </div>
    </li>
  );
}
...
This is done using the <TypeLink links={docs.links} type={docs.exports.useListSelectionCheckbox} />
...
import {useListSelectionCheckbox} from '@react-aria/list';
...
function ExampleList(props) {
  ...
  return (
    <List aria-label="Example dynamic collection List" selectionMode="single" items={rows} {...props}>
...
<ExampleList aria-label="List with single selection" selectionMode="single" defaultSelectedKeys={[2]} />
```
...
```tsx example
// Using the example above
<ExampleList aria-label="List with disallowed empty selection" selectionMode="single" defaultSelectedKeys={[2]} disallowEmptySelection />
```
...
`useList` supports row actions via the `onAction` prop, which is useful for functionality such as navigation. In the default `"toggle"` selection behavior, when nothing is selected, clicking or tapping the row triggers the row action.
When at least one item is selected, the list is in selection mode, and clicking or tapping a row toggles the selection. Actions may also be triggered via the <Keyboard>Enter</Keyboard> key, and selection using the <Keyboard>Space</Keyboard> key.
...
```tsx example
<PokemonList aria-label="List with replace selection behavior and row actions" selectionMode="multiple" selectionBehavior="replace" onAction={key => alert(`Opening item ${key}...`)} />
```

With `"toggle"` selection behavior, we can show checkboxes for selection while still having row actions. In this case, clicking the checkbox itself will toggle selection of that row, while single clicking anywhere else on the row will trigger the action.
Once one row has been selected, clicking anywhere the rows will select or deselect them.

```tsx example
<PokemonList aria-label="List with toggle selection behavior and row actions" selectionMode="multiple" selectionBehavior="toggle" onAction={key => alert(`Opening item ${key}...`)} />
```

## Internationalization

`useList` handles some aspects of internationalization automatically.
