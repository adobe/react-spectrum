  ...
  function getFormattedValue(valueParam: number | number[] = values) {
    ...
    switch (valueParam.length) {
      ...
      default: {
        ...
        if (!listFormat || listFormat.resolvedOptions().locale !== formatter.resolvedOptions().locale) {
          listFormat = listFormatRef.current = new Intl.ListFormat(formatter.resolvedOptions().locale, {type: 'unit'});
        }
