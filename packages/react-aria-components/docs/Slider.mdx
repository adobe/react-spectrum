...
The `SliderOutput` component can be targeted with the `output` CSS selector, or by adding a custom `className`. It provides a <TypeLink links={statelyDocs.links} type={statelyDocs.exports.SliderState} /> object to its render props, which can be used to customize the `className`, `style`, or `children`.
