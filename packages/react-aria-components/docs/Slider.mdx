...
import Anatomy from '/packages/@react-aria/slider/docs/anatomy.svg';
...
The `Slider` component can be targeted using the `[role=group]` selector, or by adding a custom `className`. It provides a <TypeLink links={statelyDocs.links} type={statelyDocs.exports.SliderState} /> object to its render props, which can be used to customize the `className`, `style`, or `children`. In addition, it provides the following states as DOM attributes.
...
The `SliderOutput` component can be targeted using the `output` selector, or by adding a custom `className`. It provides a <TypeLink links={statelyDocs.links} type={statelyDocs.exports.SliderState} /> object to its render props, which can be used to customize the `className`, `style`, or `children`.

### SliderTrack

The `SliderTrack` component can be targeted by adding a custom `className`. It provides a <TypeLink links={statelyDocs.links} type={statelyDocs.exports.SliderState} /> object to its render props, which can be used to customize the `className`, `style`, or `children`.

### SliderThumb

The `SliderTrack` component can be targeted by adding a custom `className`. It provides the following render props and states:
    ...
    <Slider {...props} className="slider">
      ...
      <SliderTrack className="track">
        {state => state.values.map((_, i) => (
          <SliderThumb key={i} index={i} className="thumb" />
        ))}
      </SliderTrack>
    </Slider>
  );
