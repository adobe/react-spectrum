...
import Anatomy from '/packages/@react-aria/breadcrumbs/docs/anatomy.svg';
...
As seen below, an iterable list of options is passed to the Breadcrumbs using the `items` prop. A function provided as `children` of the `<Breadcrumbs>` is called to render each item. When a breadcrum is pressed, the `onPress` event is triggered and the breadcrumbs array is updated.
...
The last breadcrumb may be used as a heading for the page by placing a `<Heading>` element within the `<Item>` instead of a `<Link>`.
...
The `<Link>` component can wrap a custom link element provided by a router like [React Router](https://reacttraining.com/react-router/).
