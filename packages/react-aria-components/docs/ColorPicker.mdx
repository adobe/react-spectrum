...
<ColorPicker>{/* Color components here */}</ColorPicker>;
```
...
```tsx example
<MyColorPicker label="Color" defaultValue="hsl(25, 100%, 50%)" />
```
...
function Example() {
  ...
  return <MyColorPicker label="Color" value={value} onChange={setValue} />;
}
```
...
function Example() {
  ...
  return (
    <div>
      <MyColorPicker label="Color" value={value} onChange={setValue} />
