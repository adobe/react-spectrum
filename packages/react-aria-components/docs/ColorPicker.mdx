...
function Example() {
  let [space, setSpace] = React.useState('rgb');

  return (
    <MyColorPicker label="Fill color" defaultValue="#184">
      <MySelect aria-label="Color space" selectedKey={space} onSelectionChange={setSpace}>
...
<MyColorPicker label="Stroke color" defaultValue="#345">
  <MyColorWheel colorSpace="hsb" outerRadius={100} innerRadius={74} />
  <MyColorArea 
    colorSpace="hsb"
    xChannel="saturation"
    yChannel="brightness"
