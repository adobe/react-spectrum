...
function MyItem({children, ...props}) {
  return (
    <Item textValue={children} {...props}>
      {({selectionMode, selectionBehavior, isDraggable}) => <>
        {isDraggable && <Button slot="drag">≡</Button>}
...
function DraggableGridList() {
  ...
  return (
    <MyGridList aria-label="Draggable list" selectionMode="multiple" items={items} dragAndDropHooks={dragAndDropHooks}>
      {([id, item]) => <MyItem id={id} textValue={item.name}><item.style>{item.name}</item.style></MyItem>}
    </MyGridList>
  );
}
...
function Example() {
  ...
  let { dragAndDropHooks } = useDragAndDrop({
    ...
    renderDropIndicator(target) {
      return <DropIndicator target={target} className="my-drop-indicator" />;
    }
    ///- end highlight -///
  });
...
function DroppableGridList() {
  ...
  return (
    <MyGridList aria-label="Draggable list" items={items} dragAndDropHooks={dragAndDropHooks} renderEmptyState={() => "Drop items here"}>
...
function Example() {
  ...
  return (
    <MyGridList aria-label="Draggable list" items={items} dragAndDropHooks={dragAndDropHooks} renderEmptyState={() => "Drop images here"}>
      {item => (
        <MyItem>
...
function Example() {
  ...
  return (
    <MyGridList aria-label="Draggable list" items={files} dragAndDropHooks={dragAndDropHooks} renderEmptyState={() => "Drop directory here"}>
      {item => (
        <MyItem id={item.name}>
...
function Example() {
  ...
  return (
    <MyGridList aria-label="Draggable list" items={items} dragAndDropHooks={dragAndDropHooks} renderEmptyState={() => "Drop PNGs here"}>
      {item => (
        <MyItem>
          <div className="image-item">
            <img src={item.url} />
...
function DndGridList(props) {
  ...
  let { dragAndDropHooks } = useDragAndDrop({
    ...
    onDragEnd(e) {
      // Remove the items from the source list on drop if they were moved.
      if (e.dropOperation === 'move') {
        list.remove(...e.keys);
      }
    }
