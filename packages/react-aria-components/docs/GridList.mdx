...
<details>
  <summary style={{fontWeight: 'bold'}}><ChevronRight size="S" /> Show CSS</summary>
```css hidden
.react-aria-GridListItem .react-aria-Button[slot='drag'] {
  display: inline-flex;
  align-items: center;
...
<GridList aria-label="Sandwich contents" selectionMode="multiple">
  ...
  <GridListSection>
    ...
    <GridListItem id="tuna">Tuna</GridListItem>
    <GridListItem id="tofu">Tofu</GridListItem>
  </GridListSection>
  <GridListSection>
    <GridListHeader>Condiments</GridListHeader>
    <GridListItem id="mayo">Mayonaise</GridListItem>
    <GridListItem id="mustard">Mustard</GridListItem>
    <GridListItem id="ranch">Ranch</GridListItem>
  </GridListSection>
</GridList>
```
...
This example uses the [useAsyncList](../react-stately/useAsyncList.html) hook to handle asynchronous loading of data from a server.
Use the [renderEmptyState](./empty-state) prop to display a spinner during initial load.
...
<PropTable component={docs.exports.GridListLoadMoreItem} links={docs.links} />
