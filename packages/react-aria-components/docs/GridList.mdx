...
infinite scroll to load more data, be sure to include the UNSTABLE_GridListLoadingSentinel after the GridList's items render function as shown in the example below. Note that UNSTABLE_GridListLoadingSentinel is meant to render the spinner specifically when loading additional items,
the initial loading state and empty state should instead be handled via [renderEmptyState](./empty-state).

```tsx example
import {Collection, UNSTABLE_GridListLoadingSentinel} from 'react-aria-components';
...
### GridListLoadingSentinel

<PropTable component={docs.exports.UNSTABLE_GridListLoadingSentinel} links={docs.links} />
