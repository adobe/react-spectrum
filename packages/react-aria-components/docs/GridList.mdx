...
infinite scroll to load more data, be sure to include the `<UNSTABLE_GridListLoadingSentinel>` after the `GridList`'s items render function as shown in the example below.

Note that `<UNSTABLE_GridListLoadingSentinel>` is meant to render the spinner specifically when loading additional items,
