...
Rows may also have a row action specified by directly applying `onAction` on the `GridListItem` itself. If `onAction` is also provided to the `GridList`, both the grid list's and the row's `onAction` are called.
...
Rows may also be disabled by directly applying `isDisabled` on the `GridListItem` itself. A row is considered disabled if its key exists in `disabledKeys` or if it has `isDisabled`.

```tsx example
<MyGridList aria-label="List with isDisabled applied on the rows directly" selectionMode="multiple">
