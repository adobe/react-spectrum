...
ComboBox supports the `name` prop for integration with HTML forms, and can be marked as required using the `isRequired` prop. When a form is submitted, the `id` of the selected `<Item>` is sent to the server by default. If the `allowsCustomValue` prop is true, or the `formValue="text"` prop is set, the text value is submitted instead.

To display validation errors, add a `<FormError>` element as a child of the ComboBox. This intercepts the browser's default validation UI and allows you to render error messages with custom styles. The error message may also be customized by providing a function to `FormError` to receive validation details. See the [FormError](FormError.html) docs for more details.
...
The `validationState` prop can be used to manually mark a ComboBox as invalid according to custom business logic. This prevents the user from being able to submit a form until the error is fixed. Use the `<FormError>` component to display a custom error message, which is automatically associated with the select.
