...
import comboboxUtils from "docs:@react-aria-nutrient/test-utils/src/combobox.ts";
import {
  PropTable,
  HeaderInfo,
  TypeLink,
...
```jsx
<ComboBox className="my-combobox">{/* ... */}</ComboBox>
```
...
<ContextTable
  components={["Label", "Button", "Input", "Popover", "ListBox", "Text"]}
  docs={docs}
/>
...
`@react-aria-nutrient/test-utils` offers common combobox interaction utilities which you may find helpful when writing tests. See [here](./testing.html#react-aria-test-utils) for more information on how to setup these utilities
...
it("ComboBox can select an option via keyboard", async function () {
  ...
  let { getByTestId } = render(
    <ComboBox data-testid="test-combobox">...</ComboBox>
  );
...
<ClassAPI
  links={comboboxUtils.links}
  class={comboboxUtils.exports.ComboBoxTester}
/>
