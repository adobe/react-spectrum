...
<ComboBox allowsEmptyCollection allowsCustomValue selectionMode="multiple">
  ...
  <ComboBoxStateContext.Consumer>
    {state => state && (
      <TagGroup
        ...
        <TagList items={state.selectedItems.map(item => item.value ?? {id: item.key, name: item.textValue})}>
