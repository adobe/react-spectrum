...
This example shows a basic tree that allows users to reorder items via drag and drop. This is enabled using the `onReorder` event handler, provided to the `useDragAndDrop` hook. The `getItems` function must also be implemented for items to become draggable. [See below](#drag-data) for more details.
...
<InlineAlert variant="info" maxWidth={600}>
  <Heading>Reordering vs. Moving</Heading>
  <Content>
    `onReorder` only allows reordering items within the same level of the tree hierarchy. To move items between different levels, use `onMove` instead (see [Moving between levels](#moving-between-levels)).
  </Content>
</InlineAlert>
    ...
    initialItems: [
      {id: 'documents', name: 'Documents', children: [
        {id: 'project', name: 'Project', children: [
          {id: 'report', name: 'Weekly Report', children: []}
        ]}
      ]},
      {id: 'photos', name: 'Photos', children: [
        {id: 'image1', name: 'Image 1', children: []},
        {id: 'image2', name: 'Image 2', children: []}
      ]},
...
The `onMove` event handler allows moving items between different levels of the tree hierarchy, as well as within the same level. Unlike `onReorder`, which only works within the same parent, `onMove` can handle drops "on" items to make them children of the target item.
  ...
  let tree = useTreeData({
    initialItems: [
      {id: 'documents', name: 'Documents', type: 'folder', children: [
        {id: 'project', name: 'Project', type: 'folder', children: [
          {id: 'report', name: 'Weekly Report', type: 'document', children: []}
        ]}
      ]},
      {id: 'photos', name: 'Photos', type: 'folder', children: [
        {id: 'image1', name: 'Image 1', type: 'image', children: []},
        {id: 'image2', name: 'Image 2', type: 'image', children: []}
      ]},
...
function Example() {
  ...
  return (
    <div style={{display: 'flex', gap: 12, flexWrap: 'wrap'}}>
      <DraggableTree />
      <PokemonEvolutionTree dragAndDropHooks={dragAndDropHooks} />
    </div>
  );
}
```
  ...
  let tree = useTreeData({
    initialItems: [
      {id: 'folder1', name: 'Folder 1', children: [
        {id: 'file1', name: 'File 1', children: []},
        {id: 'file2', name: 'File 2', children: []}
      ]},
...
function Example() {
  let tree = useTreeData({
    initialItems: [
      {id: 1, name: 'Adobe Photoshop', children: []},
      {id: 2, name: 'Adobe XD', children: []},
      {id: 3, name: 'Adobe Dreamweaver', children: []},
      {id: 4, name: 'Adobe InDesign', children: []},
      {id: 5, name: 'Adobe Connect', children: []}
    ],
    getKey: item => item.id,
    getChildren: item => item.children || []
  });
...
function DndTree(props: DndTreeProps) {
  ...
  let {dragAndDropHooks} = useDragAndDrop({
    ...
    getItems(keys) {
      let draggedKeys = new Map();
      for (let key of keys) {
        let treeNode = tree.getItem(key);
        draggedKeys.set(key, treeNode);
      }

      let isChildOfNode = (node, potentialParent) => {
        if (node.parentKey == null) {
          ...
          while (parentKey && !isChild) {
            let parentNode = tree.getItem(parentKey)!;
            isChild = potentialParent.key === parentNode.key;
      ...
      // Get rid of any keys that would already be included by a selected parent key
      for (let key of draggedKeys.keys()) {
        let node = tree.getItem(key)!;
        let keysToCheck = [...draggedKeys.keys()];
        for (let potentialParentKey of keysToCheck) {
          if (node.key === potentialParentKey) {
            continue;
          } else {
            if (isChildOfNode(node, tree.getItem(potentialParentKey)!)) {
              draggedKeys.delete(node.key);
            }
          }
        }
      }
      ...
      return (
        [...draggedKeys.keys()].map(key => {
          let item = draggedKeys.get(key);
...
<div style={{display: 'flex', gap: 12, flexWrap: 'wrap'}}>
  ...
  <DndTree
    initialItems={[
      {id: '8', name: 'Videos', type: 'folder', children: [
        {id: '9', name: 'Movie.mp4', type: 'file'}
      ]},
      {id: '10', name: 'Music', type: 'folder', children: [
        {id: '11', name: 'Song.mp3', type: 'file'},
        {id: '12', name: 'Album', type: 'folder', children: [
          {id: '13', name: 'Track1.mp3', type: 'file'}
        ]}
      ]}
    ]}
