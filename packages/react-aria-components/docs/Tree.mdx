...
infinite scroll to load more data, be sure to include the `<TreeLoadMoreItem>` after the `Tree's`'s items render function as shown in the example below.
This can be done at any arbitrary Tree row level, allowing you to handle independent asynchronous loading for any number of rows.
  ...
  return (
    <Tree
      ...
      <MyTreeItem id="pokemon" title="Pokemon">
        ...
        <MyTreeLoader isLoading={pokemonList.items} onLoadMore={pokemonList.loadMore} />
        {/*- end highlight -*/}
      </MyTreeItem>
      <MyTreeItem id="starwars" title="Star Wars">
        ...
        <MyTreeLoader isLoading={starWarsList.items} onLoadMore={starWarsList.loadMore} />
        {/*- end highlight -*/}
      </MyTreeItem>
    </Tree>
  );
