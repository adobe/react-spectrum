...
<TimeField>
  ...
  <Text slot="description">Please enter a time between 9 AM and 5 PM</Text>
</TimeField>
```

### Form validation

TimeField supports the `name` prop for integration with HTML forms, and can be marked as required using the `isRequired` prop. When a form is submitted, the time is serialized as an [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) string.
...
The `minValue` and `maxValue` props can also be used to perform builtin validation. This marks the time field as invalid if the user enters a value outside this range, and blocks form submission until a valid value is entered.

This example only accepts times between 9 AM and 5 PM, and renders a custom error message using the `<FormError>` component.
...
>
  ...
  {/*- begin highlight -*/}
  <FormError>
    {({validationDetails}) => validationDetails.rangeUnderflow || validationDetails.rangeOverflow ? 'Please enter a time between 9 AM and 5 PM' : null}
  </FormError>
  {/*- end highlight -*/}
</TimeField>
```
...
The `validationState` prop can be used to manually mark a time field as invalid according to custom business logic. This prevents the user from being able to submit a form until the error is fixed. Use the `<FormError>` component to display a custom error message, which is automatically associated with the field.
...
function Example() {
  ...
  let isInvalid = Boolean(time.minute % 15);
  ...
  return (
    ...
    >
      ...
      {isInvalid && <FormError>Meetings start every 15 minutes.</FormError>}
      {/*- end highlight -*/}
    </TimeField>
  );
}
