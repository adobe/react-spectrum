...
A dropzone consists of a target element for the dropped objects. Users may drop objects via mouse, keyboard, or touch. In addition, a `FileTrigger` may be passed as a child to allow users to upload files. 

If a visual label is not provided, then an `aria-label` or
`aria-labelledby` prop must be passed to identify the visually hidden button to assistive technology.

### Composed Components
A dropzone can pass `FileTrigger` as a child, which may also be used standalone or reused in other components.

{/* TODO: Add an Example Card of FileTrigger */}
