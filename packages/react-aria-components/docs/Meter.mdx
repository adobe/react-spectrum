...
import Anatomy from "@react-aria-nutrient/meter/docs/anatomy.svg";
...
```tsx example
<MyMeter label="Storage space" value={25} />
```
...
```tsx example
<MyMeter label="Widgets Used" minValue={50} maxValue={150} value={100} />
```
...
```tsx example
<MyMeter label="Space used" valueLabel="54 of 60GB" value={90} />
```
...
```jsx
<Meter className="my-meter">{/* ... */}</Meter>
```
