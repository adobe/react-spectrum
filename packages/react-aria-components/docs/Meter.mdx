...
See [ProgressBar](useProgressBar.html) for more details about progress bars.
...
A `Button` can be targeted with the `.react-aria-Meter` CSS selector, or by overriding with a custom `className`. It supports the following states and render props:
