...
The `NumberField` component itself does not render any DOM elements (it only passes through its children) so it does not support styling. If you need a wrapper element, add one yourself inside the `<NumberField>`.

```jsx
<NumberField>
  <div className="my-number-field">
    {/* ... */}
  </div>
</NumberField>
```
...
A `Label` can be targeted using the `label` CSS selector, or by adding a custom `className`.
...
A `Button` can be targeted with the `button` CSS selector, or by adding a custom `className`. Within a `NumberField`, there are two slots, which can be targeted using the `[slot=increment]` and `[slot=decrement]` CSS selectors. Buttons support the following states:
...
This example wraps `NumberField` and all of its children together into a single component which accepts a `label` prop, which is passed to the right place.

```tsx example export=true
function MyNumberField({label, ...props}) {
  return (
    <NumberField {...props}>
      <Label>{label}</Label>
      <Group className="number-field">
