...
<NumberField>
  ...
  <Text slot="description">Enter a width in pixels.</Text>
</NumberField>
```

### Form validation

NumberField supports the `name` prop for integration with HTML forms, and can be marked as required using the `isRequired` prop. When a form is submitted, the numeric value is sent to the server rather than the formatted string shown in the input.

To display validation errors, add a `<FormError>` element as a child of the NumberField. This intercepts the browser's default validation UI and allows you to render error messages with custom styles. The error message may also be customized by providing a function to `FormError` to receive validation details. See the [FormError](FormError.html) docs for more details.
...
For more advanced use cases, the `onValidationChange` event may also be used to implement custom logic when the number field's validation state changes. You can also disable native HTML form validation entirely by setting `validationBehavior="aria"`. This will use ARIA attributes to mark the input as required and invalid rather than blocking form submission.

### Custom errors

The `validationState` prop can be used to manually mark a number field as invalid according to custom business logic. This prevents the user from being able to submit a form until the error is fixed. Use the `<FormError>` component to display a custom error message, which is automatically associated with the field.
...
function Example() {
  ...
  return (
    <NumberField
      ...
      value={value}
      onChange={setValue}>
      <Label>Positive numbers only</Label>
      <Group>
        <Button slot="decrement">-</Button>
        <Input />
        <Button slot="increment">+</Button>
      </Group>
      {isValid && <Text slot="description">Enter a positive number.</Text>}
      {!isValid && <FormError>{value === 0 ? 'Is zero positive?' : 'Positive numbers are bigger than 0.'}</FormError>}
    </NumberField>
  );
}
```
