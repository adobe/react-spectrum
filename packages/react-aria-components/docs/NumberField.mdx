...
* **International** – Support for the Latin, Arabic, and Han, Devanagari and Bengali positional decimal numbering systems in [over 30 locales](internationalization.html#supported-locales). The numbering system is automatically detected from user input, and input method editors such as Pinyin are supported.
