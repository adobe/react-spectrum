...
```tsx example
import {RangeCalendar, Button, Heading, Button, CalendarGrid, CalendarCell} from 'react-aria-components';

<RangeCalendar aria-label="Trip dates" className="calendar">
  <header>
...
A `RangeCalendar` can be targeted by adding a custom `className`. The next and previous buttons can be targeted using the `[slot=previous]` and `[slot=next]` selectors.
...
```tsx example
<RangeCalendar aria-label="Trip dates" visibleDuration={{months: 2}} className="calendar">
  <header>
    <Button slot="previous">◀</Button>
    <Heading />
    <Button slot="next">▶</Button>
...
function MyRangeCalendar(props) {
  return (
    <RangeCalendar {...props} className="calendar">
      <header>
        <Button slot="previous">◀</Button>
        <Heading />
        <Button slot="next">▶</Button>
