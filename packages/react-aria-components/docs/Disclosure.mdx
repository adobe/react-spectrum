...
A disclosure consists of a button and panel of content. The button is wrapped in a heading element with the appropriate level based on the architecture of the page, and it contains the label representing content within the panel. The panel is the section of content that is associated with the button which is either expanded or collapsed.
...
function MyDisclosure({label, children, ...props}: MyDisclosureProps) {
  return (
    <Disclosure {...props}>
      <h3>
        <Button slot="trigger">
...
<MyDisclosure label="Manage your account">
...
```tsx example
<MyDisclosure label="Download, Install, and Set Up" defaultExpanded >
...
function ControlledExpanded() {
  ...
  return (
    <MyDisclosure label="Download, Install, and Set Up" isExpanded={isExpanded} onExpandedChange={setIsExpanded} >
      Instructions on how to download, install, and set up
    </MyDisclosure>
  )
}
...
```tsx example
<MyDisclosure label="Introduction to Knitting" isDisabled >
...
<DisclosureGroup isDisabled>
  <MyDisclosure label="How to make a return" >
    Details about returning items
  </MyDisclosure>
  <MyDisclosure label="How to cancel an order" >
