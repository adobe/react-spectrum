...
import ariaDocs from "docs:@react-aria-nutrient/calendar";
...
import Anatomy from "@react-aria-nutrient/calendar/docs/calendar-anatomy.svg";
...
function Example() {
  return (
    <MyCalendar firstDayOfWeek="sun" createCalendar={() => new Custom454()} />
  );
}
...
```jsx
<CalendarGrid className="my-calendar-grid">{/* ... */}</CalendarGrid>
```
...
function CalendarPicker({ children }) {
  ...
  return (
    ...
    >
      {/*- end highlight -*/}
      <div className="calendar-picker">{children}</div>
    </CalendarContext.Provider>
  );
}
...
function Example() {
  ...
  return (
    <CalendarPicker>
      ...
      <Preset date={startOfWeek(now.add({ weeks: 1 }), locale)}>
        Next week
      </Preset>
