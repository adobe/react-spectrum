...
function DraggableTable() {
  let items = new Map([
    [1, {name: 'Charizard', type: 'Fire, Flying', level: '67'}],
    [2, {name: 'Blastoise', type: 'Water', level: '56'}],
    [3, {name: 'Venusaur', type: 'Grass, Poison', level: '83'}],
    [4, {name: 'Pikachu', type: 'Electric', level: '100'}]
  ]);
  ...
  let { dragAndDropHooks } = useDragAndDrop({
    ...
    async onInsert(e) {
      let items = await Promise.all(e.items.map(async item => {
        let {name, type, level} = JSON.parse(await item.getText('custom-app-type'));
...
```css
.my-drop-indicator[data-drop-target] {
