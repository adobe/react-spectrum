...
```tsx
import {Table, TableHeader, TableBody, Column, Row, Cell, Button, Checkbox, ResizableTableContainer, ColumnResizer, SelectionIndicator} from 'react-aria-components';
...
export function MyColumn(props: Omit<ColumnProps, 'children'> & {children?: React.ReactNode}) {
  return (
    <Column {...props}>
      {({allowsSorting, sortDirection}) => <div className="column-header">
        {props.children}
        {allowsSorting && (
...
export function MyRow<T extends object>({id, columns, children, ...otherProps}: RowProps<T>) {
  ...
  return (
    <Row id={id} {...otherProps}>
      {allowsDragging && <Cell><Button slot="drag"><GripVertical size={18} /></Button></Cell>}
...
To enable infinite scrolling, render a `<TableLoadMoreItem>` element at the end of the table.
