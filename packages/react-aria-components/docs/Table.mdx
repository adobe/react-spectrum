...
@layer table.layout {
  ...
  /* could remove, but then we're at the mercy of import order, also contains a lot of knowledge of structure of checkbox */
  /* we'd need to check for circular dependencies and we'd need to declare this at the top like the checkbox layers, and it'd be project wide? */
  /* would be nice if checkboxes had a variant for this set of overrides instead, then table wouldn't need to know about it */
  /* or at the very least customize layer + variables that are api? */
  @layer checkbox {
    .react-aria-Checkbox {
...
You can disable specific rows by providing an array of keys to `Table` via the `disabledKeys` prop. This will prevent rows from being selectable as shown in the example below.
