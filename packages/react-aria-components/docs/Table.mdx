...
Rows may also have a row action specified by directly applying `onAction` on the `Row` itself. If `onAction` is also provided to the `Table`, both the table's and the row's `onAction` are called.
...
Rows may also be disabled by directly applying `isDisabled` on the `Row` itself. A row is considered disabled if its key exists in `disabledKeys` or if it has `isDisabled`.

```tsx example
<Table aria-label="Table with isDisabled applied on the rows directly" selectionMode="multiple">
  <MyTableHeader>
    <Column isRowHeader>Name</Column>
    <Column>Type</Column>
    <Column>Level</Column>
