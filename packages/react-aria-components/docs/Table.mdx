...
infinite scroll to load more data, be sure to include the `<TableLoadMoreItem>` after the `Table`'s items render function as shown in the example below.

Note that `<TableLoadMoreItem>` is meant to render the spinner specifically when loading additional items,
...
<PropTable component={docs.exports.TableLoadMoreItem} links={docs.links} />
