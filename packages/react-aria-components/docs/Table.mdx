...
export function MyColumn(props: Omit<ColumnProps, 'children'> & {children?: React.ReactNode}) {
  return (
    <Column {...props}>
...
The `dependencies` prop can be used to ensure that separate parts of the collection update in response to each other, for example, column additions and removals.
Be careful using it too frequently as it will cause the table to re-render more often than necessary which can cause performance degradation.
...
Tables support dynamically adding and removing columns. This is useful to change the density of information in a table.
Columns are handled separately from rows when it comes to caching. As a result, the `TableBody` component accepts a `dependencies` prop much like React's various hooks.

Note, the `dependencies` array is only needed for the dynamic items case. When using static items, the table will always re-render when the children change.
    ...
    <div style={{display: 'flex', flexDirection: 'column', gap: 12, alignItems: 'flex-start'}}>
      ...
      <Table aria-label="Files" selectionMode="multiple">
        <MyTableHeader>
          ...
          <MyColumn>Type</MyColumn>
          {/*- begin highlight -*/}
          {!hide && <MyColumn>Date Modified</MyColumn>}
          {/*- end highlight -*/}
        </MyTableHeader>
        <TableBody
          /*- begin highlight -*/
