...
interface PokemonTableProps extends TableProps {
  items?: Pokemon[],
  renderEmptyState?: (props: {isDropTarget: boolean}) => string
}
...
<Table aria-label="Search results">
  ...
  {/*- begin highlight -*/}
  <TableBody renderEmptyState={({isDropTarget, state}) => 'No results found.'}>
  {/*- end highlight -*/}
    {[]}
  </TableBody>
</Table>
  ...
  return (
    <div style={{display: 'flex', gap: 12, flexWrap: 'wrap'}}>
      ...
      <PokemonTable
        items={items}
        dragAndDropHooks={dragAndDropHooks}
        renderEmptyState={({isDropTarget}) => isDropTarget ? 'Items will be dropped' : 'Drop items here'} />
    </div>
  );
}
```
