...
function Example() {
  ...
  return (
    <form onSubmit={onSubmit} ref={formRef}>
      <SearchField value={value} onChange={v => {setValue(v); setServerInvalid(false)}} validationState={validationState}>
        <Label>Tracking number</Label>
        <Input />
        <Button>✕</Button>
        <FormError>{isServerInvalid ? 'Invalid tracking number.' : 'Please enter an 8-digit tracking number.'}</FormError>
      </SearchField>
      <Button type="submit">Submit</Button>
    </form>
  );
