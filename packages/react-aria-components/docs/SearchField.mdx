...
<SearchField>
  ...
  <Input />
  <Button>✕</Button>
  <Text slot="description">Enter a tracking number.</Text>
</SearchField>
```

### Form validation

SearchField supports the `name` prop for integration with HTML forms, and can be marked as required using the `isRequired` prop. In addition, attributes such as `type`, `pattern`, `inputMode`, and others are passed through to the underlying `<input>` element.

To display validation errors, add a `<FormError>` element as a child of the SearchField. This intercepts the browser's default validation UI and allows you to render error messages with custom styles. The error message may also be customized by providing a function to `FormError` to receive validation details. See the [FormError](FormError.html) docs for more details.
...
For more advanced use cases, the `onValidationChange` event may also be used to implement custom logic when the search field's validation state changes. You can also disable native HTML form validation entirely by setting `validationBehavior="aria"`. This will use ARIA attributes to mark the input as required and invalid rather than blocking form submission.

### Custom errors

The `validationState` prop can be used to manually mark a search field as invalid according to custom business logic. This prevents the user from being able to submit a form until the error is fixed. Use the `<FormError>` component to display a custom error message when the field is invalid.

```tsx example
function Example() {
  let [value, setValue] = React.useState('');
  let validationState = value.length === 8 ? 'valid' : 'invalid';

  return (
    <form>
      <SearchField value={value} onChange={setValue} validationState={validationState}>
...
function Example() {
  // let [value, setValue] = React.useState('');
  // let [isServerInvalid, setServerInvalid] = React.useState(false);
  // let validationState = value.length === 8 && !isServerInvalid ? 'valid' : 'invalid';
  let validationState = useValidationState(value => value.length === 8 ? null : 'invalid');
  ...
  return (
    <form onSubmit={onSubmit} ref={formRef}>
      <SearchField validationState={validationState}>
        ...
        <Input />
        <Button>✕</Button>
        <FormError>{validationState.errorMessage || 'Please enter an 8-digit tracking number.'}</FormError>
      </SearchField>
      <Button type="submit">Submit</Button>
    </form>
  );
