...
**Note:**
The `ProgressBar` must be in the accessibility tree when the button becomes pending.
This can be achieved by using opacity instead of visibility or display to hide the spinner if you'd like it wait a moment before showing.
The `Text` can remain in the button when the button is pending, in which case you should decide if it stays in the accessiblity
tree or if you should hide it in favor of the label on the `ProgressBar`.

Try the above example and the one below with a screen reader to see the difference in behavior.

<details>
  <summary style={{fontWeight: 'bold'}}><ChevronRight size="S" /> Show other styles of implementation</summary>
...
function PendingDelayed(props) {
  let [isPending, setPending] = useState(false);
  let [isPendingVisible, setPendingVisible] = useState(false);
  ...
  return (
    <Button
      ...
      style={{position: 'relative'}}>
        {({isPending}) => (
          <>
            <Text className={isPendingVisible ? 'pending' : undefined}>Save</Text>
            {isPending && <ProgressBar
              aria-label="in progress"
              isIndeterminate
              className={['spinner', (isPendingVisible ? 'spinner-pending' : '')].join(' ')}>
              <svg width="24" height="24" viewBox="0 0 24 24" style={{display: 'block'}}>
                <path fill="currentColor" d="M12,1A11,11,0,1,0,23,12,11,11,0,0,0,12,1Zm0,19a8,8,0,1,1,8-8A8,8,0,0,1,12,20Z" opacity=".25" />
                <path fill="currentColor" d="M10.14,1.16a11,11,0,0,0-9,8.92A1.59,1.59,0,0,0,2.46,12,1.52,1.52,0,0,0,4.11,10.7a8,8,0,0,1,6.66-6.61A1.42,1.42,0,0,0,12,2.69h0A1.57,1.57,0,0,0,10.14,1.16Z">
                  <animateTransform attributeName="transform" type="rotate" dur="0.75s" values="0 12 12;360 12 12" repeatCount="indefinite"/>
