...
This example wraps `ColorSwatch` and all of its children together into a single component which accepts an optional `label` prop, which is passed to the right place. It also shows how to use the `color` [render prop](styling.html#render-props) to add a checkerboard pattern behind partially transparent colors.
...
function MyColorSwatch({label, ...props}: MyColorSwatchProps) {
  return (
    <ColorSwatch 
      {...props}
      style={({color}) => ({
