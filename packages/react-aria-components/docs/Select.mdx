...
Select supports the `isRequired` prop to ensure the user selects an option, as well as custom client and server-side validation. It can also be integrated with other form libraries such as React Hook Form. See the [Forms](forms.html) guide to learn more.
