...
<Select>
  ...
  {/*- begin highlight -*/}
  <Text slot="description">Select an animal.</Text>
  {/*- end highlight -*/}
  <Popover>
    <ListBox>
      <Item>Cat</Item>
...
Select supports the `name` prop for integration with HTML forms, and can be marked as required using the `isRequired` prop. When a form is submitted, the `id` of the selected `<Item>` is sent to the server.

To display validation errors, add a `<FormError>` element as a child of the Select. This intercepts the browser's default validation UI and allows you to render error messages with custom styles. The error message may also be customized by providing a function to `FormError` to receive validation details. See the [FormError](FormError.html) docs for more details.
...
For more advanced use cases, the `onValidationChange` event may also be used to implement custom logic when the select's validation state changes. You can also disable native HTML form validation entirely by setting `validationBehavior="aria"`. This will use ARIA attributes to mark the input as required and invalid rather than blocking form submission.

### Custom errors

The `validationState` prop can be used to manually mark a Select as invalid according to custom business logic. This prevents the user from being able to submit a form until the error is fixed. Use the `<FormError>` component to display a custom error message, which is automatically associated with the select.

```tsx example
function Example() {
  let [animal, setAnimal] = React.useState<React.Key | null>(null);
  let isValid = animal !== 'cat' && animal !== 'snake';
