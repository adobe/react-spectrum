...
import Anatomy from '/packages/@react-aria/select/docs/anatomy.svg';
...
[role=option] [slot=label] {
  font-weight: bold;
}

[role=option] [slot=description] {
  font-size: small;
}
...
The `Select` component itself does not render any DOM elements (it only passes through its children) so it does not support styling. If you need a wrapper element, add one yourself inside the `<Select>`.
...
A `SelectValue` can be targed with a custom `className`. It supports the following states and render props:

<StateTable properties={docs.exports.SelectValueRenderProps.properties} />

### Popover

The `Popover` component can be targeted with a custom `className`. Note that it renders in a [React Portal](https://reactjs.org/docs/portals.html), so it will not appear as a descendant of the Select in the DOM.
...
A `Section` can be targeted with the `[role=group]` CSS selector, or by adding a custom `className`. The section title can be targeted with the `header` selector. The `title` prop also allows JSX elements and not just strings, which can enable custom formatting. However, keep in mind that interactive elements within a listbox are not allowed. See [sections](#sections) for examples.
...
This example wraps `Select` and all of its children together into a single component which accepts a `label` prop and `children`, which are passed through to the right places. The `Item` component is also wrapped to apply class names based on the current state, as descibed [above](#styling).

```tsx example export=true
function MySelect({label, children, ...props}) {
  return (
    <Select {...props}>
      <Label>{label}</Label>
      <Button className="my-select">
        <SelectValue />
        <span aria-hidden="true">▼</span>
      </Button>
...
<MySelect label="Choose frequency" isDisabled>
  <MyItem id="rarely">Rarely</MyItem>
  <MyItem id="sometimes">Sometimes</MyItem>
  <MyItem id="always">Always</MyItem>
</MySelect>
```
...
<MySelect label="Favorite Animal" disabledKeys={['cat', 'kangaroo']}>
  <MyItem id="red panda">Red Panda</MyItem>
  <MyItem id="cat">Cat</MyItem>
  <MyItem id="dog">Dog</MyItem>
  <MyItem id="aardvark">Aardvark</MyItem>
  <MyItem id="kangaroo">Kangaroo</MyItem>
  <MyItem id="snake">Snake</MyItem>
</MySelect>
```
