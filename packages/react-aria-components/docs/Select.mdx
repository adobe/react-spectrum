...
import selectUtil from "docs:@react-aria-nutrient/test-utils/src/select.ts";
import {
  PropTable,
  HeaderInfo,
  TypeLink,
...
<ContextTable
  components={["Label", "Button", "Popover", "ListBox", "Text"]}
  docs={docs}
/>
...
`@react-aria-nutrient/test-utils` offers common select interaction utilities which you may find helpful when writing tests. See [here](./testing.html#react-aria-test-utils) for more information on how to setup these utilities
...
it("Select can select an option via keyboard", async function () {
  ...
  let { getByTestId } = render(<Select data-testid="test-select">...</Select>);
