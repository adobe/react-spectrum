...
Tabs may also be disabled by directly applying `isDisabled` on the `Tab` itself. A tab is considered disabled if its key exists in `disabledKeys` or if it has `isDisabled`.
