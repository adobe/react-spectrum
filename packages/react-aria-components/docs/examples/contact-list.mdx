...
function Contact({id, avatar, name, handle}) {
  return (
    <Item id={id} textValue={name} className="group relative py-1 px-2 outline-none cursor-default grid grid-rows-2 grid-flow-col auto-cols-max gap-x-3 rounded selected:bg-blue-500 text-slate-700 selected:text-white selected:[&:has(+[data-selected])]:rounded-b-none [&[data-selected]+[data-selected]]:rounded-t-none focus-visible:ring-2 ring-offset-2 ring-blue-500">
      ...
      <Text slot="description" className="truncate text-sm text-slate-600 group-selected:text-white">{handle}</Text>
      <div className="absolute left-12 right-2 bottom-0 h-px bg-gray-200 group-selected:bg-blue-400 group-[[data-selected]:not(:has(+[data-selected]))]:hidden group-[:not([data-selected]):has(+[data-selected])]:hidden" />
    </Item>
  );
}
```
