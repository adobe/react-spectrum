...
import {Button, Collection, UNSTABLE_Tree as Tree, UNSTABLE_TreeItem as TreeItem, UNSTABLE_TreeItemContent as TreeItemContent} from 'react-aria-components';
      ...
      <Tree aria-label="File system" selectionMode="multiple" selectionBehavior="replace" items={filesystem} defaultExpandedKeys={['documents']} className={`
        ...
        overflow-auto rounded-lg shadow-lg`}>
          {function renderItem(item) {
            return (
              <TreeItem className={`selected:bg-slate-600 text-white
                ...
                <TreeItemContent>
                  {({hasChildRows}) => (
                    <div className="flex items-center space-x-2 py-2 ps-[calc(calc(var(--tree-item-level)_-_1)_*_calc(var(--spacing)_*_3))]">
                      {hasChildRows ? <Button slot="chevron" className={`shrink-0 w-8 h-8
                        ...
                        cursor-default outline-hidden`}><ChevronIcon /></Button> : <div className="shrink-0 w-8 h-8" />}
                      <div>{item.name}</div>
                    </div>
                  )}
                </TreeItemContent>
