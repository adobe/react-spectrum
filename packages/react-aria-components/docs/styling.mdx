...
Entry and exit animations can also be done using the `isExiting` prop, which can be set to true to keep an overlay element in the DOM until an animation is complete. Framer Motion's [variants](https://www.framer.com/motion/animation/#variants) can be a good way to setup named animation states.
