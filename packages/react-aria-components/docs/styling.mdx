...
```jsx
<Item className={({isSelected}) => isSelected ? 'selected' : 'unselected'}>
  Item
</Item>
```
...
```jsx
<Item className="data-[selected]:bg-blue-400 data-[disabled]:bg-gray-100">
  Item
</Item>
```

Alternatively, you can use [render props](#render-props) to control which Tailwind classes are applied based on states. This can be useful if you need to apply multiple clases based on a single state:
...
```jsx
<Item className="selected:bg-blue-400 disabled:bg-gray-100">
  Item
</Item>
```
...
```jsx
<Item className="rac-selected:bg-blue-400 rac-disabled:bg-gray-100">
  Item
</Item>
```
...
React Aria Components support [CSS keyframe animations](https://developer.mozilla.org/en-US/docs/Web/CSS/@keyframes), and work with JavaScript animation libraries like [Framer Motion](https://www.framer.com/motion/).
