...
```css
@layer datefield;
@layer datefield.layout, datefield.hcm;
@layer datefield.layout {
...
```tsx
import {DateField, Label, DateInput, DateSegment, Text} from 'react-aria-components';
...
function Example() {
  ...
  return (
    <DateField value={date} onChange={setDate} isInvalid={isInvalid}>
      ...
      {isInvalid && <Text slot="errorMessage">We are closed on weekends.</Text>}
      {/*- end highlight -*/}
    </DateField>
  );
}
