...
The `DateField` component itself does not render any DOM elements (it only passes through its children) so it does not support styling. If you need a wrapper element, add one yourself inside the `<DateField>`.

```jsx
<DateField>
  <div className="my-date-field">
    {/* ... */}
  </div>
</DateField>
```
...
A `DateInput` can be targeted using the `[role=group]` selector, or by adding a custom `className`.
...
This example wraps `DateField` and all of its children together into a single component which accepts a `label` prop, which is passed to the right place.

```tsx example export=true
function MyDateField({label, ...props}) {
  return (
    <DateField {...props}>
      <Label>{label}</Label>
      <DateInput className="input">
