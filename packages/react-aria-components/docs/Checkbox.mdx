...
```tsx example
<MyCheckbox validationState="invalid">I accept the terms and conditions</MyCheckbox>
```
...
Checkbox supports the following context:
...
<CheckboxDescription description="You will receive our newsletter once per week. You can unsubscribe at any time.">
  <MyCheckbox defaultSelected>Subscribe</MyCheckbox>
</CheckboxDescription>
```
...
If you need to customize things further, such as accessing internal state or customizing DOM structure, you can drop down to the lower level Hook-based API. See [useCheckbox](useCheckbox.html) for more details.
