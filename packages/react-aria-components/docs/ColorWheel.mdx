...
function MyColorWheel(props: ColorWheelProps) {
  return (
    <ColorWheel outerRadius={100} innerRadius={74} {...props}>
      <ColorWheelTrack />
      <ColorThumb />
    </ColorWheel>
  );
...
```tsx example
import {ColorWheelStateContext, NumberField, Input, useSlottedContext, useLocale} from 'react-aria-components';
