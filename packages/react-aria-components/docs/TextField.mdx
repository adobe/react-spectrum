...
TextField supports the `name` prop for integration with HTML forms, and can be marked as required using the `isRequired` prop. In addition, attributes such as `type`, `pattern`, `minLength`, `maxLength`, and others are passed through to the underlying `<input>` element.

To display validation errors, add a `<FormError>` element as a child of the TextField. This intercepts the browser's default validation UI and allows you to render error messages with custom styles. The error message may also be customized by providing a function to `FormError` to receive validation details. See the [FormError](FormError.html) docs for more details.
...
For more advanced use cases, the `onValidationChange` event may also be used to implement custom logic when the text field's validation state changes. You can also disable native HTML form validation entirely by setting `validationBehavior="aria"`. This will use ARIA attributes to mark the input as required and invalid rather than blocking form submission.

### Custom errors

The `validationState` prop can be used to manually mark a text field as invalid according to custom business logic. This prevents the user from being able to submit a form until the error is fixed. Use the `<FormError>` component to display a custom error message when the field is invalid.

```tsx example
<form>
  <TextField validate={value => value.length === 8 || 'Please enter an 8-digit tracking number.'}>
...
function Example() {
  let validationState = useValidationState(value =>
    value.length === 8 || 'Please enter an 8-digit tracking number.'
  );

  let onSubmit = e => {
    e.preventDefault();
    validationState.setError('Invalid tracking number.');
  };

  return (
    <form onSubmit={onSubmit}>
      <TextField validate={validationState}>
        <Label>Tracking number</Label>
        <Input />
        <FormError />
      </TextField>
