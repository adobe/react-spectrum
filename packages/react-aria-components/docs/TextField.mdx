...
TextField supports the `name` prop for integration with HTML forms, and can be marked as required using the `isRequired` prop. In addition, attributes such as `type`, `pattern`, `inputMode`, and others are passed through to the underlying `<input>` element.
