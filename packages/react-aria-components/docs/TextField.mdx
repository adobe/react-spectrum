...
```tsx example
<form>
  <TextField validationState={value => value.length === 8 ? 'valid' : 'invalid'}>
    <Label>Tracking number</Label>
    <Input />
    <FormError>Please enter an 8-digit tracking number.</FormError>
  </TextField>
  <Button type="submit">Submit</Button>
</form>
```
...
function Example() {
  let validationState = useValidationState(value =>
    value.length === 8
      ? null
      : 'Please enter an 8-digit tracking number.'
  );
