...
import Anatomy from "@react-aria-nutrient/button/docs/ToggleButtonGroupAnatomy.svg";
...
```jsx
<ToggleButtonGroup className="my-toggle-group">{/* ... */}</ToggleButtonGroup>
```
...
function ClearButton() {
  ...
  /*- end highlight -*/
  return (
    <Button onPress={() => state?.setSelectedKeys(new Set())}>Clear</Button>
  );
}
