...
By default popovers are rendered in a portal inside document body. 
This can be overridden by passing a `portalContainer` prop to `Popover` component. 
The example below will insert the popover into the element with id `portal-root`.

```tsx example
<DialogTrigger>
  <Button>Trigger</Button>
  <MyPopover portalContainer={document.getElementById('portal-root')}>
    This is a popover.
  </MyPopover>
</DialogTrigger>
```
