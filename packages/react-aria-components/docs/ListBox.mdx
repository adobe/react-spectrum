...
```tsx
import {ListBox, Item, Section, Header, Text} from 'react-aria-components';
...
This example wraps `ListBox` and all of its children together into a single component which accepts a `label` prop and `children`, which are passed through to the right places. The `Item` component is also wrapped to apply class names based on the current state, as described in the [styling](#styling) section.
...
<ListBox
  ...
  /*- begin highlight -*/
  renderEmptyState={({isDropTarget, state, isFocused, isFocusVisible}) => 'No results found'}
  /*- end highlight -*/
>
  {[]}
</ListBox>
...
function Example() {
  ...
  return (
    <div style={{display: 'flex', gap: 12, flexWrap: 'wrap'}}>
      <DraggableListBox />
      <ListBox aria-label="Droppable list" items={items} dragAndDropHooks={dragAndDropHooks} renderEmptyState={({isDropTarget}) => isDropTarget ? 'This will not be uploaded' : 'Drop items here'}>
        {item => <Item>{item.name}</Item>}
      </ListBox>
    </div>
  );
