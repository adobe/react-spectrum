...
```tsx
import {ListBox, ListBoxItem, Section, Header, Text} from 'react-aria-components';
...
Rows may also be disabled by directly applying `isDisabled` on the `ListBoxItem` itself. A row is considered disabled if its key exists in `disabledKeys` or if it has `isDisabled`.

```tsx example
<ListBox
  aria-label="Choose sandwich contents example, isDisabled applied on row directly"
  selectionMode="multiple">
  <ListBoxItem id="lettuce">Lettuce</ListBoxItem>
  <ListBoxItem id="tomato">Tomato</ListBoxItem>
  <ListBoxItem id="cheese">Cheese</ListBoxItem>
...
If you need to customize one of the components within a `ListBox`, such as `ListBoxItem` or `Section`, in many cases you can create a wrapper component. This lets you customize the props passed to the component.
