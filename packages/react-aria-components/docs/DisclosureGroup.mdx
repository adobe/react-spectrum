...
```tsx example
import {DisclosureGroup as DisclosureGroup, Disclosure as Disclosure, Button, DisclosurePanel as DisclosurePanel, Heading} from 'react-aria-components';
...
If you need to customize things even further, such as accessing internal state, you can drop down to the lower level Hook-based API. See [useDisclosureGroupState](useDisclosureGroupState.html) for more details.
