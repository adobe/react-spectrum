...
```tsx example
<MyDateRangePicker label="Trip dates" description="Please select the dates for your trip." />
```

### Form validation

DateRangePicker supports the `startName` and `endName` props for integration with HTML forms, and can be marked as required using the `isRequired` prop. When a form is submitted, the date is serialized as an [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) string matching the granularity of the displayed value.

To display validation errors, add a `<FormError>` element as a child of the DateRangePicker. This intercepts the browser's default validation UI and allows you to render error messages with custom styles. The error message may also be customized by providing a function to `FormError` to receive validation details. See the [FormError](FormError.html) docs for more details.

```tsx example
import {FormError, Button} from 'react-aria-components';

<form>
  <MyDateRangePicker label="Trip dates" startName="start" endName="end" isRequired />
...
For more advanced use cases, the `onValidationChange` event may also be used to implement custom logic when the date range picker's validation state changes. You can also disable native HTML form validation entirely by setting `validationBehavior="aria"`. This will use ARIA attributes to mark the input as required and invalid rather than blocking form submission.

### Minimum and maximum values

The `minValue` and `maxValue` props can also be used to perform builtin validation. `DateRangePicker` also validates that the end date is after the start date. This prevents the user from selecting dates outside the valid range in the calendar, marks the date range picker as invalid if the user enters a value outside this range, and blocks form submission until a valid value is selected.
...
<MyDateRangePicker
  ...
  errorMessage={({validationDetails}) => validationDetails.rangeUnderflow ? 'Please select a future date range' : null} />
```
...
The `validationState` prop can be used to manually mark a date range picker as invalid according to custom business logic. This prevents the user from being able to submit a form until the error is fixed. Use the `<FormError>` component to display a custom error message, which is automatically associated with the field.
