...
See the [Collections](collections.html) guide for more details.
...
Tags may also be disabled by directly applying `isDisabled` on the `Tag` itself. A tag is considered disabled if its key exists in `disabledKeys` or if it has `isDisabled`.
...
>
  ...
  <MyTag id="cheese">Cheese</MyTag>
  {/*- begin highlight -*/}
  <MyTag id="tuna" isDisabled>Tuna Salad</MyTag>
  {/*- end highlight -*/}
  <MyTag id="egg">Egg Salad</MyTag>
  <MyTag id="ham">Ham</MyTag>
</MyTagGroup>
