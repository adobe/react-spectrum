...
import Anatomy from "@react-aria-nutrient/tag/docs/anatomy.svg";
...
```tsx example
<MyTagGroup label="Sandwich contents" selectionMode="multiple">
  <MyTag>Lettuce</MyTag>
  <MyTag>Tomato</MyTag>
  <MyTag>Cheese</MyTag>
  {/*- begin highlight -*/}
...
```jsx
<TagGroup className="my-tag-group">{/* ... */}</TagGroup>
```
