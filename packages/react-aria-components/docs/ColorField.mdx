...
of a complete color picker rather than a single field for editing a hex value. `ColorField` helps achieve accessible
...
* **Validation** – Keyboard input is validated as the user types so that only valid hex characters are accepted. Custom client and server-side validation is also supported.
...
function MyColorField({label, description, errorMessage, ...props}: MyColorFieldProps) {
  return (
    <ColorField {...props}>
      <Label>{label}</Label>
      <Input />
