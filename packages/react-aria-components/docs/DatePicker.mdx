...
By default, the first day of the week is Sunday. This can be changed by setting the `firstDayOfWeek` prop to a number between 0 (Sunday) and 6 (Saturday).

```tsx example
<MyDatePicker aria-label="Appointment time" firstDayOfWeek={1} />
```
