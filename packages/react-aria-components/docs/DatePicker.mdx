...
```css
@layer datepicker;
@layer datepicker.layout, datepicker.hcm;
@layer datepicker.layout {
...
The `description` slot can be used to associate additional help text with a date picker. Additionally, the `errorMessage` slot can be used to help the user fix a validation error. It should be combined with the `isInvalid` prop to semantically mark the date picker as invalid for assistive technologies.
...
```css
  /* technically covered in DateField... but should it? */
```
