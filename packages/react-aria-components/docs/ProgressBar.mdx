...
import Anatomy from "@react-aria-nutrient/progress/docs/anatomy.svg";
...
```tsx example
<MyProgressBar label="Loading…" minValue={50} maxValue={150} value={100} />
```
...
```tsx example
<MyProgressBar aria-label="Loading…" isIndeterminate />
```
...
```tsx example
<MyProgressBar label="Feeding…" valueLabel="30 of 100 dogs" value={30} />
```
...
```jsx
<ProgressBar className="my-progressbar">{/* ... */}</ProgressBar>
```
