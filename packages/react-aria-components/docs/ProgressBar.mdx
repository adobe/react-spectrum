...
import Anatomy from '/packages/@react-aria/progress/docs/anatomy.svg';
...
A `Button` can be targeted with the `.react-aria-ProgressBar` CSS selector, or by overriding with a custom `className`. It supports the following states and render props:
...
<ProgressBar aria-label="Loading…" value={60}>
  {({percentage}) => <>
    <svg width={64} height={64} viewBox="0 0 32 32" fill="none" strokeWidth={strokeWidth}>
      <circle cx={center} cy={center} r={r} stroke="var(--spectrum-global-color-gray-400)" />
      <circle
        cx={center}
        cy={center}
        r={r}
