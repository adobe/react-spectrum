...
function Example() {
  return (
    <Virtualizer 
      /*- begin highlight -*/
      layout={useMemo(() => new WaterfallLayout(), [])}
      layoutOptions={{
...
function Example() {
  return (
    <Virtualizer
      /*- begin highlight -*/
      layout={useMemo(() => new TableLayout(), [])}
      layoutOptions={{
        rowHeight: 32,
        headingHeight: 32,
        padding: 4,
...
function Example() {
  let layout = useMemo(() => new HorizontalLayout(), []);
  ...
  return (
    <Virtualizer layout={layout}>
      <ListBox aria-label="Favorite animal" items={items} style={{height: 102}}>
