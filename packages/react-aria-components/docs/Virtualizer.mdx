...
<PageDescription>{docs.exports.UNSTABLE_Virtualizer.description}</PageDescription>
...
```tsx example
import {UNSTABLE_Virtualizer as Virtualizer, UNSTABLE_ListLayout as ListLayout, ListBox, ListBoxItem} from 'react-aria-components';
...
function Example() {
  ...
  return (
    <Virtualizer layout={layout}>
      <ListBox aria-label="Favorite animal" selectionMode="multiple" items={items}>
...
function Example() {
  ...
  return (
    <Virtualizer layout={layout}>
      <MyGridList aria-label="Favorite animal" selectionMode="multiple" items={items}>
...
```tsx example
import {UNSTABLE_GridLayout as GridLayout, Size, Text} from 'react-aria-components';
...
```tsx example
import {UNSTABLE_TableLayout as TableLayout, Table, TableHeader, Column, TableBody, Row, Cell} from 'react-aria-components';
...
<TypeContext.Provider value={docs.links}>
  <InterfaceType {...docs.exports.Layout} />
</TypeContext.Provider>
...
class HorizontalLayout extends Layout {
  getVisibleLayoutInfos(rect: Rect): LayoutInfo[] {
    // Determine which range of indices are visible within the given rectangle.
    let keys = Array.from(this.virtualizer!.collection.getKeys());
...
function Example() {
  ...
  return (
    <Virtualizer layout={layout}>
      <ListBox aria-label="Favorite animal" selectionMode="multiple" items={items} style={{height: 102}}>
