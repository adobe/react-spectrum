...
import menuUtil from "docs:@react-aria-nutrient/test-utils/src/menu.ts";
import {
  PropTable,
  HeaderInfo,
  TypeLink,
...
function Example() {
  ...
  return (
    <>
      ...
      <MyMenuButton label="View" isOpen={open} onOpenChange={setOpen}>
...
```jsx
<MenuTrigger>
  <div className="my-menu-trigger">{/* ... */}</div>
</MenuTrigger>
```
...
<ContextTable
  components={["Button", "Popover", "Menu", "Separator", "Text", "Keyboard"]}
  docs={docs}
/>
...
`@react-aria-nutrient/test-utils` offers common menu interaction utilities which you may find helpful when writing tests. See [here](./testing.html#react-aria-test-utils) for more information on how to setup these utilities
