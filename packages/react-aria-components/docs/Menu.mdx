...
import menuUtil from 'docs:@react-aria/test-utils';
...
<MenuTrigger>
  <Button aria-label="Menu"><MenuIcon size={18} /></Button>
  <Popover>
    <Menu>
...
```tsx
import {MenuTrigger, Button, Popover, Menu, MenuItem, MenuSection, Separator, Header, Text, Keyboard, SelectionIndicator} from 'react-aria-components';
...
export function MyItem(props: Omit<MenuItemProps, 'children'> & {children?: React.ReactNode}) {
