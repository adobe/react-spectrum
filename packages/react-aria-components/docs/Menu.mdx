...
Items may also be disabled by directly applying `isDisabled` on the `MenuItem` itself. A item is considered disabled if its key exists in `disabledKeys` or if it has `isDisabled`.
