...
`@react-aria/test-utils` also offers common menu interaction utilities which you may find helpful when writing tests. See [here](./testing.html#react-aria-test-utils) for more information on how to setup these utilities
...
it('Menu can open its submenu via keyboard', async function () {
  ...
  let submenuTester = await menuTester.openSubmenu({submenuTriggerText: 'Share…'});
