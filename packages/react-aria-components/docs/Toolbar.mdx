...
```tsx example
import {Toolbar, Button, ToggleButton, Separator, Popover, MenuTrigger, Menu, Item, Checkbox} from 'react-aria-components';
...
Toobar provides keyboard navigation and announcements for groups of interactive elements.
`Toolbar` helps implement these in an accessible way.

* **Flexible** – Support for various RAC implemented components.
* **Accessible** – Implemented as a roving tab index with arrow key navigation.
