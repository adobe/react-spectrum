...
import Anatomy from '/packages/@react-aria/dialog/docs/anatomy.svg';
...
A `Heading` can be targeted with the `h2` CSS selector, or by adding a custom `className`.
...
```tsx example
import {Popover, OverlayArrow, NumberField} from 'react-aria-components';
