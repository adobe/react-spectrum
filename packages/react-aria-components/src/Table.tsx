  ...
  commit(firstKey: Key, lastKey: Key, isSSR = false) {
    ...
    for (let row of this.getChildren(this.body.key)) {
      ...
      if (lastChildKey != null) {
        ...
        let numberOfCellsInRow =
          (lastCell.colIndex ?? lastCell.index) + (lastCell.colSpan ?? 1);
...
const ResizableTableContainerContext =
  createContext<ResizableTableContainerContextValue | null>(null);
...
export const TableContext =
  createContext<ContextValue<TableProps, HTMLTableElement>>(null);
export const TableStateContext = createContext<TableState<any> | null>(null);
export const TableColumnResizeStateContext =
...
export interface TableProps
  extends Omit<SharedTableProps<any>, "children">,
    StyleRenderProps<TableRenderProps>,
    SlotProps,
    AriaLabelingProps,
    ScrollableProps<HTMLTableElement> {
...
export const Table = forwardRef(function Table(
  ...
  let { selectionBehavior, selectionMode, disallowEmptySelection } =
    selectionState;
...
function useElementType<E extends keyof JSX.IntrinsicElements>(
  element: E
): E | "div" {
...
export interface TableHeaderProps<T>
  extends StyleRenderProps<TableHeaderRenderProps>,
    HoverEvents {
...
export const TableHeader = /*#__PURE__*/ createBranchComponent(
  ...
  <T extends object>(
    ...
    let collection = useContext(TableStateContext)!
      .collection as TableCollection<unknown>;
    let headerRows = useCachedChildren({
      ...
      children: useCallback((item: Node<unknown>) => {
        switch (item.type) {
          ...
          default:
            throw new Error(
              "Unsupported node type in TableHeader: " + item.type
            );
        }
      }, []),
    });
...
function TableHeaderRow({ item }: { item: GridNode<any> }) {
  ...
  let { isVirtualized, CollectionBranch } = useContext(
    CollectionRendererContext
  );
...
export interface ColumnResizerProps
  extends HoverEvents,
    RenderProps<ColumnResizerRenderProps> {
...
const ColumnResizerContext = createContext<ColumnResizerContextValue | null>(
  null
);
...
export const ColumnResizer = forwardRef(function ColumnResizer(
  ...
  let { onResizeStart, onResize, onResizeEnd } = useContext(
  ...
  let { resizerProps, inputProps, isResizing } = useTableColumnResize(
    {
      column,
      "aria-label":
        props["aria-label"] || stringFormatter.format("tableResizer"),
      onResizeStart,
      onResize,
      onResizeEnd,
      triggerRef,
  ...
  let isEResizable =
    layoutState.getColumnMinWidth(column.key) >=
    layoutState.getColumnWidth(column.key);
  let isWResizable =
    layoutState.getColumnMaxWidth(column.key) <=
    layoutState.getColumnWidth(column.key);
  let { direction } = useLocale();
  let resizableDirection: ColumnResizerRenderProps["resizableDirection"] =
    "both";
  if (isEResizable) {
...
export interface TableBodyProps<T>
  extends Omit<CollectionProps<T>, "disabledKeys">,
    StyleRenderProps<TableBodyRenderProps> {
...
export const TableBody = /*#__PURE__*/ createBranchComponent(
  ...
  <T extends object>(
    ...
    let isDroppable =
      !!dragAndDropHooks?.useDroppableCollectionState && !dropState?.isDisabled;
    let isRootDropTarget =
      isDroppable &&
      !!dropState &&
      (dropState.isDropTarget({ type: "root" }) ?? false);
    ...
    return (
      ...
      >
        ...
        <CollectionBranch
          ...
          renderDropIndicator={useRenderDropIndicator(
            dragAndDropHooks,
            dropState
          )}
        />
...
export interface RowProps<T>
  extends StyleRenderProps<RowRenderProps>,
    LinkDOMProps,
    HoverEvents {
...
export const Row = /*#__PURE__*/ createBranchComponent(
  ...
  <T extends object>(
    ...
    let state = useContext(TableStateContext)!;
    let { dragAndDropHooks, dragState, dropState } =
...
function TableDropIndicatorWrapper(
  ...
  let { dropIndicatorProps, isHidden, isDropTarget } = dragAndDropHooks!
    .useDropIndicator!(props, dropState!, buttonRef);
...
function TableDropIndicator(
  props: TableDropIndicatorProps,
  ref: ForwardedRef<HTMLElement>
) {
...
export const UNSTABLE_TableLoadingIndicator = createLeafComponent(
  "loader",
  function TableLoadingIndicator<T extends object>(
    props: TableLoadingIndicatorProps,
    ...
    if (isVirtualized) {
      rowProps["aria-rowindex"] =
        state.collection.headerRows.length + state.collection.size;
      rowHeaderProps["aria-colspan"] = numColumns;
      style = { display: "contents" };
    } else {
      rowHeaderProps["colSpan"] = numColumns;
