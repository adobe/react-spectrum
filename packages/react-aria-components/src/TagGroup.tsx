...
export interface TagListProps<T>
  extends Omit<CollectionProps<T>, "disabledKeys">,
    StyleRenderProps<TagListRenderProps> {
  /** Provides content to display when there are no items in the tag list. */
  renderEmptyState?: (props: TagListRenderProps) => ReactNode;
}

export const TagGroupContext =
  createContext<ContextValue<TagGroupProps, HTMLDivElement>>(null);
export const TagListContext =
  createContext<ContextValue<TagListProps<any>, HTMLDivElement>>(null);

/**
 * A tag group is a focusable list of labels, categories, keywords, filters, or other items, with support for keyboard navigation, selection, and removal.
 */
export const TagGroup = /*#__PURE__*/ (forwardRef as forwardRefType)(
...
function TagGroupInner({
  ...
  let domPropOverrides = Object.fromEntries(
    Object.entries(domProps).map(([k]) => [k, undefined])
  );
 ...
 */
export const TagList = /*#__PURE__*/ (forwardRef as forwardRefType)(
...
function TagListInner<T extends object>({
  props,
  forwardedRef,
}: TagListInnerProps<T>) {
  let state = useContext(ListStateContext)!;
...
export interface TagProps
  extends RenderProps<TagRenderProps>,
    LinkDOMProps,
    HoverEvents {
