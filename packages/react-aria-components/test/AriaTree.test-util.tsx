...
interface AriaTreeTestProps extends AriaBaseTestProps {
  renderers: {
    ...
    // must have an aria-label
    allInteractionsDisabled?: (props?: {
      name: string;
    }) => ReturnType<typeof render>;
  };
}
export const AriaTreeTests = ({
  renderers,
  setup,
  prefix,
}: AriaTreeTestProps) => {
      ...
      describe("single selection", function () {
        describeInteractions("interaction", function ({ interactionType }) {
          ...
          it("can select items", async () => {
            ...
            expect(within(rows[2]).getByRole("checkbox")).toHaveAttribute(
              "disabled"
            );
            ...
            expect(
              within(treeTester.rows[0]).getByRole("checkbox")
            ).toBeChecked();
            ...
            expect(
              within(treeTester.rows[1]).getByRole("checkbox")
            ).toBeChecked();
