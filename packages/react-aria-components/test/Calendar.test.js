...
describe('Calendar', () => {
  ...
  it('should support multi-month calendars', () => {
    ...
    let expectedStartDate = new CalendarDate(2026, 4, 1);
    let expectedNextDate = expectedStartDate.add({months: 1});
    expect(grids[0]).toHaveAttribute('aria-label', 'Appointment date, ' + formatter.format(expectedStartDate.toDate(getLocalTimeZone())));
    expect(grids[1]).toHaveAttribute('aria-label', 'Appointment date, ' + formatter.format(expectedNextDate.toDate(getLocalTimeZone())));
