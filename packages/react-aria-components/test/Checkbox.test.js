...
describe('Checkbox', () => {
  ...
  it('should support hover on readonly checkboxes', async () => {
    let hoverStartSpy = jest.fn();
    let hoverChangeSpy = jest.fn();
    let hoverEndSpy = jest.fn();
    let {getByRole} = render(<Checkbox isReadOnly className={({isHovered}) => isHovered ? 'hover' : ''} onHoverStart={hoverStartSpy} onHoverChange={hoverChangeSpy} onHoverEnd={hoverEndSpy}>Test</Checkbox>);
    let checkbox = getByRole('checkbox').closest('label');
