...
function InteractiveCoworkerStepList(args: any) {
  const keys = useMemo(() => (args.items || []).map(o => o.id), [args.items]);
  let [stepNumber, setStepNumber] = useState(
    () => keys.indexOf(args.selectedKey || args.defaultSelectedKey) + 1
  );
...
export const CoworkerStepListExample: StepListStory = {
  render: (args: any) => (
    <InteractiveCoworkerStepList {...args} items={coworkerItems} defaultSelectedKey="governance" className={styles['coworker-StepList']}>
      {(item: CoworkerItemValue) => {
        let Icon = item.Icon;
        return (
          <StepListItem id={item.id} className={styles['coworker-StepListItem']}>
            ...
            <span className={styles['coworker-StepListItem-labels']}>
              ...
              <span className={styles['coworker-StepListItem-stage']}>{`Stage ${item.stage}`}</span>
            </span>
          </StepListItem>
        );
      }}
    </InteractiveCoworkerStepList>
  )
};
