...
const StaticTreeItem = (props: StaticTreeItemProps) => {
  return (
    <TreeItem
      {...props}
      className={({isFocused, isSelected, isHovered, isFocusVisible}) =>
  ...
  return (
    <Tree
      ...
      <TreeItem
        ...
        <TreeItemContent>Reports</TreeItemContent>
      </TreeItem>
      <TreeItem
        id="Tests"
        textValue="Tests"
        ...
        <TreeItemContent>{({isFocused}) => <Text>{`${isFocused} Tests`}</Text>}</TreeItemContent>
      </TreeItem>
    </Tree>
  );
}
...
const TreeExampleStaticNoActionsRender = <T extends object>(args: TreeProps<T>): JSX.Element => (
  <Tree
    ...
    onExpandedChange={action('onExpandedChange')}
    onSelectionChange={action('onSelectionChange')}>
    <StaticTreeItemNoActions id="Photos" textValue="Photos">
      Photos
...
const TreeExampleSectionRender = args => (
  <Tree
    ...
    <TreeSection>
      ...
      <StaticTreeItem id="edited photos" textValue="Edited photos">
        Edited Photos
      </StaticTreeItem>
    </TreeSection>
    <TreeSection>
      ...
      <StaticTreeItem id="projects-4" textValue="Projects-4">
        Project-4
      </StaticTreeItem>
    </TreeSection>
    <TreeItem
...
const MyTreeLoader = props => {
  ...
  return (
    <TreeLoadMoreItem className={classNames(styles, 'tree-loader')} {...props}>
      {({level}) => {
        if (omitChildren) {
          return;
        }
...
const DynamicTreeItem = (props: DynamicTreeItemProps) => {
  ...
  return (
    <>
      <TreeItem
        {...props}
        className={({isFocused, isSelected, isHovered, isFocusVisible, isDropTarget}) =>
          classNames(styles, 'tree-item', {
            focused: isFocused,
            'focus-visible': isFocusVisible,
            selected: isSelected,
        ...
        <Collection items={childItems}>
          {(item: any) => (
            <DynamicTreeItem
              ...
              textValue={item.name ?? item.value.name}
              href={props.href}>
              {item.name ?? item.value.name}
            </DynamicTreeItem>
          )}
        </Collection>
        {renderLoader?.(props.id) && (
          <MyTreeLoader isLoading={props.isLoading} onLoadMore={props.onLoadMore} />
        )}
      </TreeItem>
      {props.isLastInRoot && (
        <MyTreeLoader isLoading={props.isLoading} onLoadMore={props.onLoadMore} />
...
let defaultExpandedKeys = new Set([
  'projects',
  'project-2',
  'project-5',
  'reports',
...
export const TreeSectionDynamic: StoryObj<typeof TreeExampleDynamicRender> = {
  ...TreeExampleStatic,
  render: args => <TreeSectionExampleDynamicRender {...args} />,
  parameters: undefined
};
...
function LoadingStoryDepOnTop<T extends object>(
  ...
  return (
    <Tree
      ...
      {item => (
        <DynamicTreeItem
          isLastInRoot={item.key === 'reports'}
          renderLoader={id => id === 'root' || id === 'project-2'}
          isLoading={args.isLoading}
          id={item.key}
          childItems={item.children ?? []}
          textValue={item.value.name}>
...
function ExpandButton(props) {
  ...
  return <Button slot="chevron">{contents}</Button>;
}
          ...
          {({isExpanded, hasChildItems, level, selectionBehavior, selectionMode}) => (
            <>
              ...
              <div
                ...
                {hasChildItems && (
                  <ExpandButton
                    isLoading={isLoading && renderLoader && renderLoader(props.id)}
...
function ButtonLoadingIndicator<T extends object>(
  ...
  return (
    <Tree
      ...
      {item => (
        <DynamicTreeItemWithButtonLoader
          renderLoader={id => id === 'project-2' || id === 'project-5'}
          isLoading={args.isLoading}
          id={item.key}
          childItems={item.children ?? []}
          textValue={item.value.name}>
...
function MultiLoaderTreeMockAsync(args) {
  ...
  let onProjectsLoadMore = useCallback(() => {
    if (!isProjectsLoading && projectsData.items.length < 30) {
      ...
      setTimeout(() => {
        ...
        for (let i = 0; i < 5; i++) {
          dataToAppend.push({
            id: `projects-${i + projectsLength}`,
            value: `Projects-${i + projectsLength}`
          });
        }
  ...
  let onProjectsLevel3LoadMore = useCallback(() => {
    if (!isProjectsLevel3Loading && projects3Data.items.length < 30) {
      ...
      setTimeout(() => {
        ...
        for (let i = 0; i < 5; i++) {
          dataToAppend.push({
            id: `project-1-${i + projects3Length}`,
            value: `Project-1-${i + projects3Length}`
          });
        }
  ...
  let onDocumentsLoadMore = useCallback(() => {
    if (!isDocumentsLoading && documentsData.items.length < 30) {
      ...
      setTimeout(() => {
        ...
        for (let i = 0; i < 5; i++) {
          dataToAppend.push({
            id: `document-${i + documentsLength}`,
            value: `Document-${i + documentsLength}`
          });
        }
  ...
  return (
    <Virtualizer layout={ListLayout} layoutOptions={{rowHeight: 30}}>
      <Tree aria-label="multi loader tree" className={styles.tree}>
        ...
        <Collection
          ...
          dependencies={[isProjectsLoading, isDocumentsLoading, isProjectsLevel3Loading]}>
          {(item: any) => {
            if (item.id === 'projects') {
              return (
                <StaticTreeItem id="projects" textValue="Projects" title="Projects">
                    ...
                    {(item: any) => {
                      return item.id !== 'projects-1' ? (
                        ...
                        <StaticTreeItem id="projects-1" textValue="Projects-1" title="Projects-1">
                          ...
                          <MyTreeLoader
                            isLoading={isProjectsLevel3Loading}
                            onLoadMore={onProjectsLevel3LoadMore}
                          />
                        </StaticTreeItem>
                      );
                    }}
...
function SecondTree(args) {
  ...
  let {dragAndDropHooks} = useDragAndDrop({
    ...
    [args.dropFunction]: (e: DroppableCollectionReorderEvent) => {
      console.log(
        `moving [${[...e.keys].join(',')}] ${e.target.dropPosition} ${e.target.key} in SecondTree`
      );
      try {
        if (e.target.dropPosition === 'before') {
          treeData.moveBefore(e.target.key, e.keys);
...
export const VirtualizedTreeSectionRender = {
  render: VirtualizedTreeExampleSectionRender,
  args: {
    selectionMode: 'none',
    selectionBehavior: 'toggle',
    disabledBehavior: 'selection',
