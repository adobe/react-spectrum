...
export const DropzoneExampleWithDraggableObject: DropzoneStory = props => (
  <div>
    ...
    <DropZone
      ...
      onDropEnter={action('OnDropEnter')}
      onDropExit={action('OnDropExit')}>
      <Text slot="label">DropZone Area</Text>
    </DropZone>
  </div>
);
  ...
  <div>
    ...
    <DropZone
      ...
      {({isHovered, isFocused, isFocusVisible, isDropTarget, isDisabled}) => (
        <div>
          <Text slot="label">DropzoneArea</Text>
...
export const Scrolling = () => (
  <div
    ...
    <div style={{height: 500, background: '#eef', padding: 8}}>
      Filler 1 — scroll down past this so the DropZone's top is above the visible region but the
      FileTrigger button below is still visible.
    </div>
    ...
    <DropZone
      ...
      <div style={{height: 300, background: '#cce', padding: 8}}>
        Inner filler — the DropZone's hidden accessibility button sits at the very top of this
        region.
      </div>
      <FileTrigger
