...
import {ColorArea} from '../../src/color/ColorArea';
...
import {ProviderProps} from '../../src/provider/types';
import React from 'react';
import * as stories from '../../stories/color/ColorArea.stories';
import userEvent from '@testing-library/user-event';

let {
  XRedYGreen: DefaultColorArea,
  XBlueYGreen,
  XSaturationYBrightness,
  XSaturationYLightness
...
describe('ColorArea', () => {
  ...
  `('$Name', ({Component, groupIndex}) => {
    describe('attributes', () => {
      it('sets input props', () => {
        ...
        expect(xSlider).toHaveAttribute(
          'aria-valuetext',
          'Red: 255, Green: 0, Blue: 255, light vibrant magenta'
        );
        ...
        expect(ySlider).toHaveAttribute(
          'aria-valuetext',
          'Green: 0, Red: 255, Blue: 255, light vibrant magenta'
        );
      ...
      describe('labelling', () => {
        it('should support a custom aria-label', () => {
          let {getAllByRole} = render(
            <Component defaultValue={'#ff00ff'} aria-label="Color hue" />
          );
          let [xSlider, ySlider] = getAllByRole('slider', {hidden: true});
        ...
        it('should support a custom aria-labelledby', () => {
          let {getAllByRole} = render(
            <Component defaultValue={'#ff00ff'} aria-labelledby="label-id" />
          );
          let [xSlider, ySlider] = getAllByRole('slider', {hidden: true});
    ...
    describe('behaviors', () => {
      ...
      describe('keyboard events', () => {
        ...
        `('$Name', async ({props, actions: {forward, backward}, result}) => {
          let {getAllByRole} = render(
            <Component {...props} onChange={onChangeSpy} onChangeEnd={onChangeEndSpy} />
          );
          let [xSlider, ySlider] = getAllByRole('slider', {hidden: true});
          ...
          expect(xSlider.getAttribute('aria-valuetext')).toBe(
            `${result.getChannelName('red', 'en-US')}: ${result.formatChannelValue('red', 'en-US')}, ${result.getColorName('en-US')}`
          );
          ...
          expect(document.activeElement === xSlider ? ySlider : xSlider).toHaveAttribute(
            'tabindex',
            '-1'
          );
          expect(document.activeElement === xSlider ? ySlider : xSlider).not.toHaveAttribute(
            'aria-hidden',
            'true'
          );
          ...
          expect(onChangeSpy.mock.calls[1][0].toString('rgba')).toBe(
            props.defaultValue.toString('rgba')
          );
          expect(onChangeEndSpy).toHaveBeenCalledTimes(2);
          expect(onChangeEndSpy.mock.calls[1][0].toString('rgba')).toBe(
            props.defaultValue.toString('rgba')
          );
          expect(ySlider.getAttribute('aria-valuetext')).toBe(
            `${props.defaultValue.getChannelName('green', 'en-US')}: ${props.defaultValue.formatChannelValue('green', 'en-US')}, ${props.defaultValue.getColorName('en-US')}`
          );
          expect(document.activeElement).not.toHaveAttribute('tabindex', '0');
          expect(document.activeElement).not.toHaveAttribute('aria-hidden');
          expect(document.activeElement === xSlider ? ySlider : xSlider).toHaveAttribute(
        ...
        `('$Name RTL', async ({props, actions: {forward, backward}, result}) => {
          ...
          expect(onChangeEndSpy.mock.calls[0][0].toString('rgba')).toBe(result.toString('rgba'));
          expect(xSlider.getAttribute('aria-valuetext')).toBe(
            `${result.getChannelName('red', 'ar-AE')}: ${result.formatChannelValue('red', 'ar-AE')}, ${result.getColorName('ar-AE')}`
          );
          ...
          expect(ySlider.getAttribute('aria-valuetext')).toBe(
            `${props.defaultValue.getChannelName('green', 'ar-AE')}: ${props.defaultValue.formatChannelValue('green', 'ar-AE')}, ${props.defaultValue.getColorName('ar-AE')}`
          );
          expect(document.activeElement).not.toHaveAttribute('tabindex');
          expect(document.activeElement).not.toHaveAttribute('aria-hidden');
          expect(document.activeElement === xSlider ? ySlider : xSlider).toHaveAttribute(
      ...
      `('$type', ({actions: [start, move, end], prepare}) => {
        ...
        it('clicking on the area chooses the color at that point', () => {
          ...
          expect(onChangeSpy.mock.calls[0][0].toString('rgba')).toBe(
            parseColor('#EC80FF').toString('rgba')
          );
          ...
          expect(onChangeSpy).toHaveBeenCalledTimes(1);
          expect(onChangeEndSpy).toHaveBeenCalledTimes(1);
          expect(onChangeEndSpy.mock.calls[0][0].toString('rgba')).toBe(
            parseColor('#EC80FF').toString('rgba')
          );
          expect(document.activeElement).toBe(xSlider);
        });
        ...
        it('dragging the thumb works', () => {
          ...
          expect(onChangeSpy).toHaveBeenCalledTimes(1);
          expect(onChangeSpy.mock.calls[0][0].toString('rgba')).toBe(
            parseColor('#9300FF').toString('rgba')
          );
          expect(onChangeEndSpy).toHaveBeenCalledTimes(0);
          expect(document.activeElement).toBe(xSlider);
        ...
        it('clicking and dragging on the track works', () => {
          ...
          expect(onChangeSpy).toHaveBeenCalledTimes(2);
          expect(onChangeSpy.mock.calls[1][0].toString('rgba')).toBe(
            parseColor('#8014FF').toString('rgba')
          );
          expect(document.activeElement).toBe(xSlider);
  ...
  describe('defaults uncontrolled', () => {
    ...
    it('the slider is focusable', async () => {
      ...
      expect(xSlider).toHaveAttribute(
        'aria-valuetext',
        'Red: 254, Green: 0, Blue: 255, light vibrant magenta'
      );
      expect(ySlider).toHaveAttribute(
        'aria-valuetext',
        'Green: 0, Red: 254, Blue: 255, light vibrant magenta'
      );
      await user.tab({shift: true});
      expect(document.activeElement).toBe(xSlider);
  ...
  describe('full implementation controlled', () => {
    it('sets input props rgb', () => {
      ...
      expect(xSlider).toHaveAttribute(
        'aria-valuetext',
        'Blue: 255, Green: 255, Red: 0, very light vibrant cyan'
      );
      expect(xSlider).not.toHaveAttribute('tabindex');
      expect(xSlider).not.toHaveAttribute('aria-hidden');
      ...
      expect(ySlider).toHaveAttribute(
        'aria-valuetext',
        'Green: 255, Blue: 255, Red: 0, very light vibrant cyan'
      );
      expect(ySlider).toHaveAttribute('tabindex', '-1');
      expect(ySlider).toHaveAttribute('aria-hidden', 'true');
    ...
    it('sets input props hsb', () => {
      let {getAllByRole, getByLabelText} = render(
        <XSaturationYBrightness {...XSaturationYBrightness.args} />
      );
      let sliders = getAllByRole('slider');
      ...
      expect(xSlider).toHaveAttribute(
        'aria-valuetext',
        'Saturation: 100%, Brightness: 100%, Hue: 0°, vibrant red'
      );
      expect(xSlider).not.toHaveAttribute('tabindex');
      expect(xSlider).not.toHaveAttribute('aria-hidden');
      ...
      expect(ySlider).toHaveAttribute(
        'aria-valuetext',
        'Brightness: 100%, Saturation: 100%, Hue: 0°, vibrant red'
      );
      expect(ySlider).toHaveAttribute('tabindex', '-1');
      expect(ySlider).toHaveAttribute('aria-hidden', 'true');
    ...
    it('sets input props hsl', () => {
      let {getAllByRole, getByLabelText} = render(
        <XSaturationYLightness {...XSaturationYLightness.args} />
      );
      let sliders = getAllByRole('slider');
      ...
      expect(xSlider).toHaveAttribute(
        'aria-valuetext',
        'Saturation: 100%, Lightness: 50%, Hue: 0°, vibrant red'
      );
      expect(xSlider).not.toHaveAttribute('tabindex');
      expect(xSlider).not.toHaveAttribute('aria-hidden');
      ...
      expect(ySlider).toHaveAttribute(
        'aria-valuetext',
        'Lightness: 50%, Saturation: 100%, Hue: 0°, vibrant red'
      );
      expect(ySlider).toHaveAttribute('tabIndex', '-1');
      expect(ySlider).toHaveAttribute('aria-hidden', 'true');
