...
import {ColorWheel} from '../../src/color/ColorWheel';
import {ControlledHSLRender} from '../../stories/color/ColorWheel.stories';
...
describe('ColorWheel', () => {
  ...
  describe('keyboard events', () => {
    it('works', async () => {
      ...
      expect(onChangeEndSpy.mock.calls[0][0].toString('hsla')).toBe(
        defaultColor.withChannelValue('hue', 1).toString('hsla')
      );
      ...
      expect(onChangeEndSpy.mock.calls[1][0].toString('hsla')).toBe(
        defaultColor.withChannelValue('hue', 0).toString('hsla')
      );
    });
  ...
  `('$type', ({actions: [start, move, end], prepare}) => {
    ...
    it('dragging the thumb works', () => {
      let defaultColor = parseColor('hsl(0, 100%, 50%)');
      let {container: _container, getByRole} = render(
        <ColorWheel
          defaultValue={defaultColor}
          onChange={onChangeSpy}
          onChangeEnd={onChangeEndSpy}
  ...
  it('thumb background color should not include alpha channel', () => {
    ...
    */
    let handleColorElement = _container?.firstChild?.firstChild?.firstChild?.nextSibling
