...
describe('ProgressCircle', function () {
  ...
  `('$Name handles defaults', function ({Component, props}) {
    let {getByRole} = render(
      <Component aria-label="Progress" {...props}>
        Progress Circle
      </Component>
    );
  ...
  it('handles submask defaults', () => {
    let {getByRole, getByTestId} = render(
      <ProgressCircle aria-label="Progress" value={0}>
        Progress Bar
      </ProgressCircle>
    );
    let progressCircle = getByRole('progressbar');
  ...
  it('shows quarter of the circle for 25%', () => {
    ...
    expect(getByTestId('fillSubMask1')).toHaveAttribute('style', 'transform: rotate(-90deg);');
    expect(getByTestId('fillSubMask2')).toHaveAttribute('style', 'transform: rotate(-180deg);');
  });

  it('shows half the circle for 50%', () => {
    let {getByTestId} = render(<ProgressCircle aria-label="Progress" value={50} />);
    expect(getByTestId('fillSubMask1')).toHaveAttribute('style', 'transform: rotate(0deg);');
    expect(getByTestId('fillSubMask2')).toHaveAttribute('style', 'transform: rotate(-180deg);');
  });
  ...
  it('shows quarter of the circle for 75%', () => {
    ...
    expect(getByTestId('fillSubMask1')).toHaveAttribute('style', 'transform: rotate(0deg);');
    expect(getByTestId('fillSubMask2')).toHaveAttribute('style', 'transform: rotate(-90deg);');
  });

  it('shows all of the circle for 100%', () => {
    let {getByTestId} = render(<ProgressCircle aria-label="Progress" value={100} />);
    expect(getByTestId('fillSubMask1')).toHaveAttribute('style', 'transform: rotate(0deg);');
    expect(getByTestId('fillSubMask2')).toHaveAttribute('style', 'transform: rotate(0deg);');
  });

  it('can handle negative values with minValue and maxValue', () => {
    let {getByRole} = render(
      <ProgressCircle aria-label="Progress" value={0} minValue={-5} maxValue={5} />
    );
