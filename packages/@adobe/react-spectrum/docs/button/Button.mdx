...
Buttons can indicate that a quick progress task is taking place (e.g., saving settings on a server). After a delay, an indeterminate spinner will be displayed in place of the button label and icon. You can trigger this behavior with the `isPending` prop. Button events are disabled while `isPending` is true. By default, the spinner appears after 1 second; use `pendingDelay` to customize this (set to `0` for an instant spinner).
...
### Custom pending delay

Use `pendingDelay` to control how long to wait before showing the spinner. Interaction is still disabled immediately when `isPending` is true.
...
function Example() {
  ...
  return (
    <Button variant="primary" isPending={isLoading} pendingDelay={0} onPress={handlePress}>
      Save
    </Button>
  );
}
