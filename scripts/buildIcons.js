...
const PACKAGES = {
  ...
  workflow: path.dirname(require.resolve('@adobe/react-spectrum-workflow/dist/')),
  color: path.dirname(require.resolve('@adobe/react-spectrum-workflow-color/dist/')),
...
(async function () {
  ...
  for (let [pkg, srcFolder] of Object.entries(PACKAGES)) {
    ...
    for (let srcFile of await glob([path.join(srcFolder, '*.{js,tsx,svg}').replace(/\\/g, '/')])) {
      let filename = path.basename(srcFile);
      if (filename === 'index.js' || filename === 'util.js') {
        continue;
      }

      let distFile = path.join(distFolder, path.basename(srcFile, path.extname(srcFile))) + '.js';
      if (!(await exists(distFile)) || (await isNewerThan(srcFile, distFile))) {
        commandPromises.push(
          pkg !== 'illustrations'
            ? {
                command: 'yarn make-icons && yarn build-icons',
                name: `Making and building icons for @spectrum-icons/${pkg}`,
                cwd: distFolder
              }
            : {
                command: 'yarn build-icons',
                name: `Building icons for @spectrum-icons/${pkg}`,
                cwd: distFolder
              }
        );
        break;
...
function getIconPackageFolder(name) {
  return path.resolve(path.join(__dirname, '..', 'packages', '@spectrum-icons', name));
}
