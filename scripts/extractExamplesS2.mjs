      ...
      code = code.replace(
        ...
        (m, start, examples, inner) => {
          if (inner.includes('COMPONENT')) {
            ...
            inner = components
              .map(component => inner.replace(/COMPONENT/g, component))
              .join('\n\n');
      ...
      code = code.replace(
        ...
        (m, meta, propsStr, inner) => {
          ...
          if (inner.includes('/* PROPS */')) {
            ...
            let props = Object.entries(parsed)
              .map(([key, value]) => {
      ...
      for (let asset of res.mdx_assets) {
        if (asset.lang === 'tsx' || asset.lang === 'ts') {
          let relative = path.relative(
            path.join(process.cwd(), 'packages/dev/s2-docs/pages'),
            page
          );
          ...
          await fs.writeFile(
            path.join(distDir, relative + ':' + asset.position.start.line + '.' + asset.lang),
            code
          );
        }
      }
